...
- For orchestration telemetry, inspect `Game.get_orchestration_audit_events(...)` and `Game.get_orchestration_audit_counters()` for compact General, Deployment, Commander, dirty/repair, phase-report, and context-attachment events. These events are bounded operational audit metadata, not replay authority.
