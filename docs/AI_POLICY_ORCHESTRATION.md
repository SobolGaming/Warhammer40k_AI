...
- For strategic-flow audits, inspect `plan_id`, `turn_plan`, `battle_round_plan_id`, `commander_dirty_flags`, `commander_replan_scope`, `last_commander_phase_report`, `score_window_state`, `opportunity_catalog`, `cp_reserve_policy`, `tier2_task`, `movement_intent`, `unit_battle_task`, `commander_movement_task`, `commander_fire_assignment`, `preferred_target_unit_ids`, `preferred_declarations`, `target_fire_plan_summary`, `general_limited_resource_policy`, `general_cp_policy`, `commander_charge_assignment`, `commander_fight_assignment`, `commander_candidate_charge_assignments`, `commander_candidate_fight_assignments`, and `compute_tier` inside `request_context`; these explain which orchestration context was available when candidates were generated or scored. The full `battle_round_plan` and `general_plan` dicts are present only when audit/debug payload attachment is explicitly enabled.
