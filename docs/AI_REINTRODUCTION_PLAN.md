...
| `PR-AI-015` | ML dependency onboarding boundary PR | DONE | Closed on March 3, 2026: ML dependency onboarding is now isolated behind an explicit `warhammer40k_ai/ml` boundary with optional packaging extras and runtime guards, while core engine/replay imports remain ML-stack independent. | Enforced by `src/warhammer40k_ai/ml/dependency_boundary.py`, `pyproject.toml` `project.optional-dependencies.ml`, and regression coverage in `tests/rules/test_ml_dependency_boundary.py` plus boundary documentation in `docs/ML_DEPENDENCY_BOUNDARY.md`. |
