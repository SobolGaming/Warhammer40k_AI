...
- Fight-stage transitions clear unresolved prompts from the stage being left,
  including older fight target-selection prompts that predate `phase_step`
  metadata. Fight target-selection requests now carry `phase_step`, and
  fight-phase completion clears remaining fight-phase prompts. This prevents
  stale Fight First or Remaining Combatants requests from carrying into later
  fight activations or the next battle phase during replay.
