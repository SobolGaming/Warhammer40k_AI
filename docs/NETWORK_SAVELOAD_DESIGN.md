...
- deployment_placement_dialog: MOVE_UNIT {unit_id, model_positions} (context `placement_kind="deployment"`; request options can include deterministic exact-placement candidates via `placement_candidate_id`/`deployment_anchor`; engine finalizes deployment + advances deployment turn)
