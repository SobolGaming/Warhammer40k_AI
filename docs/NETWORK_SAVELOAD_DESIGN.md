...
- deployment_placement_dialog: MOVE_UNIT {unit_id, model_positions} (context `placement_kind="deployment"`; request options can include deterministic exact-placement candidates via `placement_candidate_id`/`deployment_anchor`; optional `deployment_lookahead` config can enable bounded rollout scoring metadata; engine finalizes deployment + advances deployment turn)
...
- stratagem_tool_action_dialog: SELECT_TOOL_ACTION {tool_name, ability_key, ability_name, tool_id, tool_descriptor_id?, resolved_kwargs | skip} (context `ability="tool_action"`, `tool_action_signature`, `reactions_only`; authoritative fallback for headless/non-local stratagem windows not covered by bespoke decision kinds. `resolved_kwargs` must already be a fully bound legal payload for that stratagem; target-required malformed options are omitted rather than emitted with missing fields. A shared preflight firewall validates descriptor-derived eligibility, entity-reference roundtrip resolution when available, and `can_use(...)` before emitting options. Candidate-build omissions caused by missing required context produce visible tool-probe diagnostics; malformed/illegal emitted specs caught before exposure use `malformed_tool_candidate_filtered_preflight` or `illegal_tool_candidate_filtered_preflight`, and any malformed candidate that still escapes preflight is rejected with `malformed_tool_candidate_escaped_preflight`.)
- overwatch_shooter_dialog: SELECT_OVERWATCH_SHOOTER {unit_id | skip} (also used for stratagem unit selection; context may include `enemy_unit_id` and abilities such as `fire_overwatch`, `claimed_for_the_dark_gods`, `crushed_like_vermin`, or `titanic_duel`)
