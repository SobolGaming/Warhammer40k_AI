...
- secondary_discard_dialog: DISCARD_SECONDARY {card_name, card_slot | skip} (context `ability="new_orders"`, `discard_source="new_orders"`, `discard_timing="command_phase_stratagem"` for `NEW ORDERS`; achieved-secondary discard at end of turn is automatic scoring cleanup and does not emit this player-choice dialog)
