...
- stratagem_tool_action_dialog: SELECT_TOOL_ACTION {tool_name, ability_key, ability_name, tool_id, tool_descriptor_id?, resolved_kwargs | skip} (context `ability="tool_action"`, `tool_action_signature`, `reactions_only`; authoritative fallback for headless/non-local stratagem windows not covered by bespoke decision kinds)
