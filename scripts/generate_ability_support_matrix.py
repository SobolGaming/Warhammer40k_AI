...
def _detachment_full_consumption_patterns() -> Dict[str, Tuple[str, ...]]:
    ...
    raw: Dict[str, Tuple[str, ...]] = {
        ...
        "Slaves to None": (
            ...
            r"ranged weapons equipped by heretic astartes models from your army have the assault ability",
            r"when mustering your army you cannot use the cults of the dark gods rule",
        ),
        "Vendetta": (
        ...
        "Relentless Onslaught": (
            r"each time a necrons model excluding monster models from your army makes an attack that targets a unit within range of one or more objective markers add \d+ to the hit roll",
...
def _detachment_ability_support_by_name() -> Dict[str, Tuple[str, str]]:
    raw = {
        ...
        "Slaves to None": (
            "Supported",
            "Renegade Warband: HERETIC ASTARTES units from your army lose access to Dark Pacts, ranged weapons equipped by HERETIC ASTARTES models from your army gain [ASSAULT], and army validation blocks Cults of the Dark Gods allies while this detachment is selected.",
        ),
        "Vendetta": (
            "Supported",
        ...
        "The Great Wolf Watches": (
            "Supported",
            "Champions of Fenris: at the end of the opponent Charge phase, eligible ADEPTUS ASTARTES INFANTRY/WALKER units within 3\" of enemy units can declare out-of-turn charges that do not receive the Charge bonus (Wolves' Wisdom increases this to 6\" for the bearer's unit), and ADEPTUS ASTARTES TERMINATOR units gain +1 Objective Control while not Battle-shocked; Space Wolves-only chapter restriction enforced.",
        ),
        "Vulkan's Quest": (
            "Supported",
        ...
        "Relentless Onslaught": ("Supported", "Starshatter Arsenal: non-MONSTER NECRONS gain +1 to hit vs targets within objective range; VEHICLE/MOUNTED (non-TITANIC) ranged weapons gain Assault."),
        "Ruthless Discipline": (
            "Supported",
...
def _datasheet_ability_support_by_name_faction() -> Dict[Tuple[str, str], Tuple[str, str]]:
    raw = {
        ...
        ("AM", "LOYAL PROTECTOR"): (
            "Supported",
            "Declare Battle Formations: must join one COMMAND SQUAD if eligible, otherwise removed as destroyed; joined model counts as part of the unit, can embark with its bodyguard host, and uses 3 transport slots. Warlord and Enhancements restrictions from LOYAL PROTECTOR text variants are parsed.",
        ),
        ("AM", "Ogryn Bodyguard"): (
            "Supported",
...
def _bearer_move_and_save_characteristics_support(description: str) -> Optional[Tuple[str, str]]:
    ...
    if not m:
        return None
    return ("Supported", f"Bearer has Move characteristic {m.group('move')}\" and Save characteristic {m.group('save')}+.")
...
def _conditional_lone_operative_support(description: str) -> Optional[Tuple[str, str]]:
    ...
    not_leading = bool(m.group("not_leading"))
    note = f"Conditional Lone Operative within {rng}\" of friendly {keywords.upper()} units."
    if not_leading:
        note += " Disabled while leading a unit."
    return (
        "Supported",
        note,
    )
...
def _enhancement_support(name: str, enh_id: str, description: str) -> Tuple[str, str]:
    explicit = {
        ...
        "000009037002": "Litanies of Faith: start of your Command phase, while bearer is on the battlefield, pass Leadership test for the bearer to gain 1 Miracle die.",
        ...
        "000009033005": "Iron Surplice of Saint Istalela: bearer Save characteristic is set to 2+ and the bearer has Feel No Pain 5+.",
        "000009831002": "Triptych of Judgement: while the bearer is alive, attacks made by models in the bearer's unit can ignore Ballistic Skill/Weapon Skill modifiers and Hit roll modifiers.",
        "000009831003": "Mark of Devotion: bearer melee weapons gain +1 Attacks, or +2 Attacks and +1 Damage while the bearer's unit is Righteous.",
        "000009831004": "Eyes of the Oracle: bearer weapons gain [PRECISION], and while the bearer is alive the bearer's unit gains 1CP when it destroys an enemy CHARACTER model.",
        "000009831005": "Sanctified Amulet: while the bearer is alive, enemy units arriving from Reserves cannot be set up within 12\" of the bearer.",
        "000009029002": "Psalm of Righteous Judgement: while the bearer is on the battlefield, each time an enemy unit is destroyed by a friendly PENITENT unit you can discard 1 Miracle die to gain 1 Miracle die showing 6.",
        "000009029003": "Verse of Holy Piety: once per battle at the start of a battle round, select one Vow of Atonement to also be active for the bearer's unit for that battle round (in addition to the army's active vow).",
        "000009029004": "Refrain of Enduring Faith: while the bearer is leading, models in the bearer's unit have a 5+ invulnerable save.",
        "000009029005": "Catechism of Divine Penitence: bearer gains the PENITENT keyword and can attach to REPENTIA SQUAD units during Declare Battle Formations.",
        "000008494002": "The Imperium's Sword: bearer melee weapons gain +1 Attacks; once per battle at start of any phase, optional activation grants +1 Attacks to melee weapons of other models in the bearer's unit until end of phase.",
        "000008494003": "Fear Made Manifest (Aura): enemy non-MONSTER/non-VEHICLE units within 6\" that fail Battle-shock destroy one model (target owner selects); once per battle choose to destroy D3 models instead.",
        "000008494004": "Rites of War: bearer gains +1 Objective Control; once per battle at start of any phase, optional activation grants +1 Objective Control to other models in the bearer's unit until end of phase.",
        "000008494005": "Iron Resolve: bearer has Feel No Pain 5+; once per battle after bearer unit is selected as target in Shooting/Fight, optional activation grants Feel No Pain 5+ to models in that unit until end of phase.",
        "000009835002": "Prescient Flash: models in the bearer's unit gain Scouts 6\".",
        "000009835003": "Troubling Visions: once per battle in your Command phase, optional activation makes all Angelic Legacy abilities active for the bearer's unit until the start of your next Command phase.",
        "000009835004": "Blazing Icon: enemy units cannot use Fire Overwatch to shoot at the bearer's unit while the bearer is alive.",
        "000009835005": "Ordained Sacrifice: first time the bearer is destroyed, end-of-phase 2+ return at the destroyed position with 3 wounds remaining.",
        "000009190002": "Artisan of War: while the bearer is alive, improve AP by 1 for ranged and melee weapons equipped by the bearer and set the bearer's Save characteristic to 2+.",
        "000009190003": "Visage of Death: in your opponent's Command phase, enemy non-MONSTER/non-VEHICLE units within Engagement Range of the bearer must take a Battle-shock test.",
        "000009190004": "Archangel's Shard: while the bearer is alive, the bearer's melee weapons gain [ANTI-CHAOS 5+] and [LANCE].",
        "000009190005": "Gleaming Pinions: once per turn, when an enemy unit ends a Normal/Advance/Fall Back move within 9\" of the bearer's unit and that unit is not within Engagement Range, the bearer's unit can make a reactive Normal move up to 6\".",
        "000009186002": "Sanguinius' Grace: once per battle at the end of the Fight phase, if the bearer is within Engagement Range of three or more enemy models, the bearer can fight one additional time.",
        "000009186003": "Blood Shard: first time the bearer is destroyed, end-of-phase 2+ return at the destroyed position with 3 wounds remaining.",
        "000009186004": "To Slay the Warmaster: once per battle at the start of the Fight phase, select one enemy CHARACTER unit within Engagement Range of the bearer and roll six D6; each 4+ inflicts 1 mortal wound on a CHARACTER model in that unit.",
        "000009186005": "Vengeful Onslaught: if the bearer is destroyed, until the end of your next turn friendly DEATH COMPANY models gain +1 to Hit rolls for attacks.",
        "000008771002": "Shroud of Heroes: first time the bearer is destroyed, end-of-phase 2+ return with 3 wounds, or full wounds if the bearer was Battle-shocked when destroyed.",
        "000008771003": "Stubborn Tenacity: while the bearer is leading, attacks by models in that unit gain +1 to Hit while below Starting Strength, and gain +1 to Wound as well while below Starting Strength and Battle-shocked.",
        "000008771004": "Weapons of the First Legion: bearer's melee weapons gain +1 Attacks/Strength/Damage, or +2 Attacks/Strength/Damage while the bearer is Battle-shocked.",
        "000008771005": "Pennant of Remembrance: while the bearer is leading, models in the bearer's unit gain Feel No Pain 6+, or Feel No Pain 4+ while that unit is Battle-shocked.",
        "000009851002": "Wolves' Wisdom: while the bearer is alive, the bearer's unit can use a 6\" range instead of 3\" when declaring out-of-turn The Great Wolf Watches charges.",
        "000009851003": "Foes' Fate: enemy non-MONSTER/non-VEHICLE units within Engagement Range of the bearer's unit that Fall Back must take Desperate Escape tests, suffering an additional -1 when Battle-shocked.",
        "000009851004": "Fangrune Pendant: while the bearer is alive, the bearer's unit is eligible to shoot and declare a charge in a turn in which it Fell Back.",
        "000009851005": "Longstrider: re-roll Charge rolls for the bearer's unit.",
        "000010269002": "Wolf-touched: bearer gains +2\" Move and can attach to WULFEN INFANTRY units during Declare Battle Formations.",
        "000010269003": "Hunter's Guile: after deployment, select up to three friendly THUNDERWOLF CAVALRY, WULFEN, and/or BLOOD CLAWS units to redeploy, with optional placement into Strategic Reserves regardless of current reserves limits.",
        "000010269004": "Elder's Guidance: once per battle at the start of the Fight phase, if the bearer is leading BLOOD CLAWS, optional activation improves AP by 1 for melee weapons equipped by models in that unit until end of phase.",
        "000010269005": "Helm of the Beastslayer: while the bearer is alive, attacks made by CHARACTER, MONSTER, or VEHICLE models that target the bearer's unit have their AP worsened by 1.",
        "000010265002": "Braggart's Steel: bearer melee weapons gain +2 Strength, and gain +1 Damage while the bearer's unit has achieved one or more Boasts.",
        "000010265003": "Skjald: each time a Space Wolves CHARACTER unit from your army achieves a Boast, if the bearer is on the battlefield, gain 1CP.",
        "000010265004": "Hordeslayer: at the start of the Fight phase, if enemy models outnumber friendly models wholly within 6\" of the bearer, the bearer's melee weapons gain +2 Attacks, or +3 if the bearer's unit has achieved one or more Boasts, until end of phase.",
        "000010265005": "Thunderwolf's Fortitude: first time the bearer is destroyed, at end of phase return on 2+ with 3 wounds remaining.",
        "000010261002": "Swift Hunter: models in the bearer's unit gain Scouts 7\".",
        "000010261003": "Fenrisian Grit: bearer gains Feel No Pain 4+.",
        "000010261004": "Wolf Master: in your Command phase, select one friendly SPACE WOLVES unit within 9\" of the bearer; until your next Command phase, teeth and claws and Tyrnak and Fenrir weapons in that unit gain [LETHAL HITS].",
        "000010261005": "Feral Rage: bearer melee weapons gain +1 Attacks, and gain an additional +1 Attacks while the bearer has charged this round.",
        "000010660002": "Grimnar's Mark: from battle round 2 onwards, once per battle round target the bearer's unit with Rapid Ingress or Heroic Intervention for 0CP, and the bearer can attach to Wolf Guard Terminators during Declare Battle Formations.",
        "000010660003": "Howlmaw: at the start of the Fight phase, optional selection of one enemy unit within 6\" of the bearer to take a Battle-shock test at -1.",
        "000010660004": "Chariots of the Storm: after deployment, select up to three friendly ADEPTUS ASTARTES units to redeploy, with optional placement into Strategic Reserves regardless of current reserves limits.",
        "000010660005": "Skjald's Foretelling: while the bearer is leading a unit, melee weapons equipped by models in that unit gain [LANCE].",
        "000008486002": "Fury of the Storm: bearer melee weapons gain +1 Strength and +1 AP, and after the bearer ends a Charge move this turn the bearer instead gains +2 Strength and +2 AP.",
        "000008486003": "Portents of Wisdom: while the bearer is leading and alive, the bearer's attached unit can re-roll Advance rolls.",
        "000008486004": "Feinting Withdrawal: while the bearer is leading and alive, the bearer's attached unit is eligible to shoot in turns in which it Fell Back.",
        "000008486005": "Hunter's Instincts: while the bearer is alive and the bearer's unit is in Strategic Reserves, that unit treats the current battle round as one higher for Strategic Reserves setup timing.",
        "000008490002": "The Blade Driven Deep: while the bearer is leading, models in that attached unit gain Infiltrators.",
        "000008490003": "Ghostweave Cloak: bearer model gains Stealth and Lone Operative.",
        "000008490004": "Execute and Redeploy: in your Shooting phase after the bearer's unit shoots and while not in Engagement Range, optional reactive Normal move up to 6\" with no-charge until end of turn; cannot move more than once in the same Shooting phase.",
        "000008490005": "Shadow War Veteran: while the bearer is alive, enemy units within 12\" of the bearer model increase the CP cost of targeting Stratagems by 1.",
        "000010396002": "Imperialis of the Eternal Crusade: while the bearer is alive, enemy units targeting the bearer's unit with a charge suffer -2 to the Charge roll; this is not cumulative with other negative Charge roll modifiers.",
        "000010396003": "Consecrating Aura: while the bearer is alive, models in the bearer's unit have a 5+ invulnerable save.",
        "000010396004": "Orb of the Emperor's Aegis: while the bearer is alive, models in the bearer's unit have the Deep Strike ability.",
        "000010396005": "Warden of Honour: while the bearer is leading, add 1 to the result of Vengeful Exhortation rolls made for the bearer's unit.",
        "000008778002": "Master-crafted Weapon: melee weapons equipped by the bearer gain Precision.",
        "000008778003": "Mounted Strategist: while the bearer is alive, the bearer's unit can re-roll Advance and Charge rolls.",
        "000008778004": "Master of Manoeuvre: while the bearer is alive, the bearer's unit ignores Strategic Reserves points-cap contribution and treats the battle round as one higher for Strategic Reserves setup timing.",
        "000008778005": "Recon Hunter: models in the bearer's unit gain Scouts 9\".",
        "000010629002": "Spearpoint Paragon: bearer melee weapons gain +1 Strength and +1 AP, and after the bearer ends a Charge move this turn the bearer instead gains +2 Strength and +2 AP.",
        ...
        "000010392005": "Zealous Vanguard: models in the bearer's unit gain Scouts 6\".",
        "000010460002": "Champion of the Feast: bearer melee weapons gain +1 Attacks; once per battle at start of any phase, optional activation grants +1 Attacks to melee weapons of other models in the bearer's unit until end of phase.",
        "000010460003": "Disciple of Rhetoricus: bearer gains +1 Objective Control; once per battle at start of any phase, optional activation grants +1 Objective Control to other models in the bearer's unit until end of phase.",
        "000010460004": "Indomitable Champion: first time the bearer is destroyed, end-of-phase 2+ return at the destroyed position with 3 wounds remaining.",
        "000010460005": "Malodraxian Standard: while the bearer is alive, each time an attack targets the bearer's unit, if attack Strength is greater than that unit's Toughness subtract 1 from the Wound roll.",
        "000010466002": "Blackwing Shroud: while the bearer is leading, models in that attached unit gain Infiltrators.",
        "000010466003": "Coronal Susurrant: while the bearer is alive, enemy units within 12\" of the bearer model increase the CP cost of targeting Stratagems by 1.",
        "000010466004": "Umbral Raptor: bearer model gains Stealth and Lone Operative.",
        "000010466005": "Hunter's Instincts: while the bearer is alive and the bearer's unit is in Strategic Reserves, that unit treats the current battle round as one higher for Strategic Reserves setup timing.",
        "000010155002": "Tempered in Battle (Aura): while the bearer is alive, friendly ADEPTUS ASTARTES units within 6\" of the bearer can re-roll Battle-shock and Leadership tests.",
        "000010155003": "Ancient Weapons: the bearer's melee weapons gain +2 Strength, +1 AP, and +1 Damage.",
        "000010155004": "Deathwing Assault: while the bearer is alive and leading a Deep Strike unit in reserves, that unit treats Strategic Reserves setup timing as one battle round higher (allowing first-turn setup).",
        "000010155005": "Lord of the Ravenwing: while the bearer is alive, the bearer's unit can re-roll Advance and Charge rolls.",
        "000009843002": "Pyrebrand: while the bearer is alive, models in the bearer's unit have Stealth.",
        "000009843003": "Sacred Rage: once per battle at the start of the Fight phase, optional activation grants Fights First to the bearer's unit until end of phase.",
        "000009843004": "Taramond's Censer: at the start of the Fight phase, each enemy unit within Engagement Range of the bearer's unit must take a Battle-shock test with a -1 modifier.",
        "000009843005": "Benediction of Fury: the bearer's melee weapons gain [DEVASTATING WOUNDS].",
        "000008478002": "Target Augury Web: in your Command phase, select one friendly ADEPTUS ASTARTES VEHICLE model within 6\" of the bearer; until your next Command phase, weapons equipped by that VEHICLE model gain [LETHAL HITS].",
        "000008478003": "The Flesh is Weak: the bearer has the Feel No Pain 4+ ability.",
        "000008478004": "Adept of the Omnissiah: once per battle round, the first failed save for a friendly ADEPTUS ASTARTES VEHICLE model within 6\" of the bearer sets that attack's Damage characteristic to 0.",
        "000008478005": "Master of Machine War: in your Command phase, select one friendly ADEPTUS ASTARTES VEHICLE model within 6\" of the bearer; until your next Command phase, that VEHICLE can shoot after Advancing or Falling Back.",
        "000008560002": "Cantic Thrallnet: at the start of each battle round, optional selection of one friendly SKITARII unit within 12\" of the bearer grants both Protector and Conqueror Imperatives to that unit until the start of the next battle round.",
        ...
        "000008376005": "Gift of Foresight: once per battle round, after a Hit roll, Wound roll, or saving throw for the bearer, that roll can be changed to an unmodified 6.",
        "000009785002": "Prescience: once per turn, when an enemy unit ends a Normal/Advance/Fall Back move within 9\" of the bearer's unit, optional reactive Normal move up to D6\", or up to 6\" while Divination is active.",
        "000009785003": "Celerity: while the bearer is alive, the bearer's unit can declare a charge after Advancing, and can also charge after Falling Back while Biomancy is active.",
        "000009785004": "Obfuscation: while the bearer is alive, enemy units cannot Fire Overwatch the bearer's unit; while Telepathy is active, that unit can only be targeted by ranged attacks from within 18\".",
        "000009785005": "Fusillade: while the bearer is alive, ranged weapons in the bearer's unit gain [ANTI-MONSTER 5+] and [ANTI-VEHICLE 5+], gain [SUSTAINED HITS 1] while Pyromancy is active, and gain +6\" range while Telekinesis is active.",
        "000009733002": "Calibanite Armaments: while the bearer is alive, the bearer's melee weapons have +1 Damage.",
        "000009733003": "Lord of the Hunt: while the bearer is alive, the bearer's unit can shoot and charge after Falling Back and can re-roll Desperate Escape tests.",
        "000009733004": "Stalwart Champion: while the bearer is alive and the bearer's unit is not Battle-shocked, models in that unit have Objective Control +1.",
        "000009733005": "Fulgus Magna: once per battle at the end of your opponent's turn, if the bearer's unit is not within Engagement Range, optional move into Strategic Reserves.",
        "000010680002": "Laurels of Thunder: while the bearer is alive, the bearer's unit can re-roll Charge rolls on turns in which that unit was set up on the battlefield.",
        "000010680003": "Veteran of the Vanguard: while the bearer is alive, models in the bearer's unit gain Scouts 6\".",
        "000010680004": "Orbital Uplink Reliquary: after deployment, select up to three friendly ADEPTUS ASTARTES units to redeploy, with optional placement into Strategic Reserves regardless of current reserves limits.",
        "000010680005": "Dedicated Gunship: once per battle at the end of your opponent's Fight phase, if the bearer's unit is not within Engagement Range, optional move into Strategic Reserves.",
        "000010684002": "Seals of Reconquest: while the bearer is alive, models in the bearer's unit gain a 5+ invulnerable save.",
        "000010684003": "Avenging Avatar (Aura): in your opponent's Command phase, enemy units below Starting Strength within 9\" of the bearer must take Battle-shock tests.",
        "000010684004": "Scroll of Proclamation: the bearer's unit can re-roll Charge rolls when charging enemy units within objective range.",
        "000010684005": "Liberatum: bearer attacks can re-roll Hit and Wound rolls against targets within objective range.",
        "000008474002": "Indomitable Fury: bearer-only return on a 2+ at end of phase after first destruction with full wounds restored.",
        "000008474003": "Fleet Commander: start of Shooting phase first-marker point pick, next Shooting phase mandatory second-marker pick within 12\" of first marker, then deterministic line-crossing unit checks with per-unit D6 (3+) for D3 mortal wounds.",
        "000008474004": "Stoic Defender: while bearer is leading and unit is within a controlled objective, models in that unit gain Feel No Pain 6+; if Battle-shocked while leading, Objective Control is halved instead of set to 0.",
        "000008474005": "Architect of War: while bearer is leading, ranged attacks made by models in that unit gain [IGNORES COVER].",
        "000008482002": "Champion of Humanity: while the bearer is leading and alive, models in that unit can ignore selected Movement/Advance/Charge characteristic modifiers and Hit/Wound/Charge roll modifiers (excluding save modifiers).",
        "000008482003": "War-tempered Artifice: bearer melee weapons gain +3 Strength.",
        "000008482004": "Forged in Battle: while the bearer is leading, once per turn after a Hit roll or saving throw for a model in that unit, that roll can be changed to an unmodified 6.",
        "000008482005": "Adamantine Mantle: attacks allocated to the bearer are reduced by 1 Damage; attacks from MELTA or TORRENT weapons instead have their Damage set to 1.",
        "000010368002": "Immolator: while the bearer is alive, add 1 to the Attacks characteristic of TORRENT weapons equipped by models in the bearer's unit.",
        "000010368003": "War-tempered Artifice: bearer melee weapons gain +3 Strength.",
        "000010368004": "Forged in Battle: while the bearer is leading, once per turn after a Hit roll or saving throw for a model in that unit, that roll can be changed to an unmodified 6.",
        "000010368005": "Adamantine Mantle: attacks allocated to the bearer are reduced by 1 Damage; attacks from MELTA or TORRENT weapons instead have their Damage set to 1.",
        "000010676002": "Eye of the Primarch: ranged weapons equipped by the bearer and Battleline models in the bearer's unit gain [PRECISION].",
        ...
        "000010623005": "Steel Font: while bearer is leading a unit, in your Command phase you can return one destroyed Bodyguard model to that unit.",
        "000008774002": "Champion of the Deathwing: bearer's melee weapons gain [LETHAL HITS], and while the bearer is within range of your Vowed objective marker the bearer scores critical hits in melee on unmodified Hit rolls of 5+.",
        "000008774003": "Eye of the Unseen: each time the bearer's unit is targeted by a Stratagem, roll D6 (+1 while the bearer is within range of your Vowed objective marker); on 5+ gain 1CP.",
        "000008774004": "Singular Will: while the bearer is alive, the bearer's unit can move an additional 3\" when it Piles In and when it Consolidates.",
        "000008774005": "Deathwing Assault: while the bearer is alive and leading a Deep Strike unit in reserves, that unit treats Strategic Reserves setup timing as one battle round higher (allowing first-turn setup).",
        "000008353002": "Artificer Armour: bearer Save characteristic is set to 2+ and bearer gains Feel No Pain 5+.",
        "000008353003": "The Honour Vehement: add 1 to Attacks and Strength of the bearer's melee weapons, or add 2 while the bearer is under Assault Doctrine.",
        "000008353004": "Adept of the Codex: at the start of your Command phase, optional activation makes Tactical Doctrine active for the bearer's unit until the start of your next Command phase.",
        "000008353005": "Fire Discipline: while the bearer is leading a unit, ranged weapons in that unit gain [SUSTAINED HITS 1]; while under Devastator Doctrine, that unit can re-roll Advance rolls.",
        "000010400002": "Paragon of Fury: bearer melee weapons gain +2 Strength, and while the bearer disembarked from a Transport this turn those melee attacks gain +1 Damage.",
        "000010400003": "Battle-psalm Precentor: when the bearer's unit triggers Shock and Awe Battle-shock, that test is modified by -1.",
        "000010400004": "Augury Servo-host: at the start of your Shooting phase, select one visible enemy within 12\" of the bearer; it cannot gain Benefit of Cover until end of phase.",
        "000010400005": "Herald of Sacred Slaughter: if the bearer starts embarked in a Dedicated Transport during Declare Battle Formations, that Dedicated Transport gains Scouts 9\".",
        "000010633002": "Armour of Antoninus: bearer Save characteristic is set to 2+ and bearer gains Feel No Pain 5+.",
        "000010633003": "Oath of Macragge: add 1 to Attacks and Strength of the bearer's melee weapons, or add 2 while the bearer is under Assault Doctrine.",
        "000010633004": "Student of the Codex: at the start of your Command phase, optional activation makes Tactical Doctrine active for the bearer's unit until the start of your next Command phase.",
        "000010633005": "Veteran of Behemoth: while the bearer is leading a unit, ranged weapons in that unit gain [SUSTAINED HITS 1]; while under Devastator Doctrine, that unit can re-roll Advance rolls.",
        "000008522002": "Thief of Secrets: bearer melee weapons gain +1 Strength, +1 Damage and +1 AP; at end of Fight phase after a bearer melee kill, those bonuses upgrade to +2 for the rest of the battle.",
        ...
        "000008981002": "Amulet of Tainted Vigour: in your Command phase while the bearer is leading, optional return of up to D3 destroyed DAMNED non-CHARACTER models to the bearer's unit via deterministic bodyguard-return selection.",
        "000008981003": "Cultist's Brand: while the bearer is alive and every other model in the bearer's unit (excluding Dark Disciples) is DAMNED, the bearer's unit can re-roll Advance and Charge rolls.",
        "000008981004": "Incendiary Goad: while the bearer's unit is below Starting Strength, melee weapons equipped by DAMNED models in that unit gain +1 Strength; while that unit is Below Half-strength, those melee weapons also gain +1 Attacks.",
        "000008981005": "Warped Foresight: while the bearer is leading a unit with Scouts 6\", the bearer's unit gains Scouts 6\" (applied to the attached unit via leader scout propagation hooks).",
        "000009773002": "Surgical Precision: the bearer's melee weapons gain [PRECISION].",
        "000009773003": "Living Carapace: the bearer gains +1 Wounds and Feel No Pain 5+ (scoped to bearer model only).",
        "000009773004": "Helm of All-seeing: enemy units arriving from Reserves cannot be set up within 12\" of the bearer while it is alive.",
        "000009773005": "Prime Test Subject: the bearer's melee weapons gain +1 Damage and the bearer can re-roll Hit rolls for melee attacks.",
        ...
        "000008412003": "Instinctive Defence: while the bearer is within 6\" of a friendly HARVESTER, the bearer's unit has Fights First and can be targeted with Heroic Intervention for 0CP.",
        ...
        "000009084003": "Adaptive Reprisal: once per turn while the bearer is on the battlefield, a friendly GENESTEALER CULTS unit within 9\" can be targeted with Heroic Intervention for 0CP.",
        "000009134004": "Gift of the Prescient: once per battle, if the bearer is on the battlefield, Rapid Ingress can target a friendly GREY KNIGHTS TERMINATOR SQUAD for 0CP and that setup is legal if it is more than 3\" from enemy models.",
        "000009138005": "Fleetmaster: once per battle round, the bearer's unit can be targeted with Violent Acquisition, Masters of the Void, or Close-quarters Barrage for 0CP.",
        ...
        "000009130004": "No Escape (Aura): enemy units within 6\" selected to Fall Back must pass a Leadership test or remain stationary.",
        "000008543004": "Soulless Reaper: enemy units within Engagement Range selected to Fall Back roll D6; on 3+ they cannot Fall Back and must remain stationary.",
        ...
        "000008380002": "Death Mask of Ollanius: while the bearer is leading and the unit is Battle-shocked, Objective Control is reduced by 1 instead of being set to 0.",
        "000008380003": "Drill Commander: while the bearer is leading and the unit Remained Stationary, ranged attacks by models in that unit score critical hits on unmodified Hit rolls of 5+.",
        "000008380004": "Grand Strategist: bearer issues one additional Order each Command phase.",
        "000008380005": "Reactive Command: each time an enemy unit is set up within 9\" of the bearer's unit, the bearer gains one setup-triggered Order that does not count against issued Orders.",
        "000009801002": "Bombast-class Vox-array: if bearer unit includes a Master Vox, each issued Order can be applied to up to three different REGIMENT units (same order key, one order consumed).",
        "000009801003": "Priority-drop Beacon: while bearer unit starts in Strategic Reserves and has Deep Strike, it treats setup timing as one battle round higher (allowing first-turn setup).",
        "000009801004": "Shroud Projector: enemy units cannot use Fire Overwatch to shoot at the bearer's unit while the bearer is alive.",
        "000009801005": "Advance Augury: after deployment, redeploy up to three friendly REGIMENT units; selected units may be placed into Strategic Reserves regardless of limits.",
        "000009839002": "Student of Kauyon: in pre-battle setup, optional single-dialog selection of up to three friendly Kroot Carnivores or Kroot Farstalkers units grants selected units Deep Strike for the battle.",
        "000009839005": "Transponder Lock Module: while bearer unit starts in Strategic Reserves and has Deep Strike, it treats setup timing as one battle round higher; if set up via Deep Strike in your first Movement phase, it must be within 12\" of a friendly KROOT or VESPID STINGWINGS unit.",
        "000009857002": "Eager Advance: while the bearer is leading a REGIMENT unit and the bearer is alive, that attached unit gains Scouts 6\".",
        "000009857003": "Flash Grenades: enemy units cannot use Fire Overwatch to shoot at the bearer's unit while the bearer is alive.",
        "000009857004": "Legacy Sidearm: add 2 to the Attacks characteristic of the bearer's Pistol weapons.",
        "000009857005": "Stalwart's Honours: while the bearer is leading and alive, when that attached unit is issued an Order it is also affected by Take Cover!.",
        "000009861002": "Bold Leadership: at end of your Command phase, objective markers you control remain sticky when the bearer's unit is in range, including while embarked if its Transport is in range.",
        "000009861003": "Sacred Unguents: at the start of your Shooting phase, select one friendly non-AIRCRAFT/non-TITANIC TRANSPORT within 3\" of the bearer; that TRANSPORT can re-roll Hit rolls until end of phase.",
        "000009861004": "Smoke Grenades: while the bearer's unit is wholly within 3\" of one or more friendly TRANSPORT models, it has Benefit of Cover and Stealth.",
        "000009861005": "Vanguard Honours: the bearer's unit can disembark after its TRANSPORT Advanced, counts as having made a Normal move that phase, and cannot charge that turn.",
        "000009869002": "Guerrilla Honours: after deployment, while the bearer is on the battlefield, redeploy up to three other friendly ASTRA MILITARUM INFANTRY units; selected units may be placed into Strategic Reserves regardless of limits.",
        "000009869003": "Scare Gas Grenades: once per battle at the start of any phase, bearer can select one enemy unit within 8\" of the bearer's unit (excluding MONSTER/VEHICLE) to take a Battle-shock test via deterministic target selection.",
        "000009869004": "Survival Gear: bearer model gains Scouts 6\".",
        "000009869005": "Tripwires: each time an enemy INFANTRY or MOUNTED unit ends a Normal/Advance/Charge/Fall Back move within 9\" of the bearer's unit, roll D6 and on 4+ apply stunned (-1 to Hit rolls) until the start of your next turn.",
        "000009865002": "Calm Under Fire: once per turn after the bearer issues an Order to a SQUADRON unit, it can issue the same Order to one additional SQUADRON unit.",
        "000009865004": "Regimental Banner: add 3 to the bearer model's Objective Control characteristic.",
        "000009865005": "Veteran Crew: ranged attacks by models in the bearer's unit re-roll Hit rolls of 1.",
        ...
        "000009749002": "Dread Majesty (Aura): NECRONS units within 6\" (excluding MONSTER and TITANIC) re-roll Hit and Wound rolls of 1.",
        ...
        "000010574004": "Eye of Spite: bearer melee weapons gain +1 Attacks and +1 AP; when bearer's unit is selected to fight, optional spend 1 Pain token to gain +2 instead until end of phase.",
        "000010574005": "Crucible of Malediction: once per battle in your Shooting phase, enemy units within 12\" of bearer take Battle-shock tests; optional spend 1 Pain token for -1 to those tests; PSYKER units that fail suffer 3 mortal wounds.",
        "000009781002": "Archraider: in Declare Battle Formations, if bearer starts embarked in a Dedicated Transport, that transport gains Scouts 9\".",
        "000009781003": "Webway Walker: models in the bearer's unit gain Deep Strike; if that unit is set up using Deep Strike while losing the wager, it can re-roll Charge rolls until end of turn.",
        "000009781004": "Reaper's Cowl: models in the bearer's unit gain Stealth and Infiltrators.",
        "000009781005": "Conductor of Torment: in your Command phase, choose to gain 1 Pain token and switch the wager to DRUKHARI winning when DRUKHARI are losing, or spend 1 Pain token to switch to HARLEQUINS winning when DRUKHARI are winning.",
        "000010576002": "Phantasmal Smoke: while bearer unit is wholly within 6\" of a friendly DRUKHARI Transport, models in bearer unit have Stealth and Benefit of Cover against ranged attacks.",
        ...
        "000008815005": "Starflare Ignition System: at the end of your opponent's turn, if the bearer's unit is not within Engagement Range, optional confirmation places that unit into Strategic Reserves.",
        ...
        "000008572002": "Necromechanic: once per battle round, when a save fails for a friendly LEGIO CYBERNETICA or ADEPTUS MECHANICUS VEHICLE model within 12\" of the bearer, the attack's Damage is set to 0.",
        "000008572003": "Lord of Machines: start of opponent's Shooting phase, select a visible enemy VEHICLE within 12\"; Leadership test pass gives -1 to hit, fail makes it ineligible to shoot this phase.",
        "000008572004": "Emotionless Clarity: once per turn, when a friendly LEGIO CYBERNETICA or ADEPTUS MECHANICUS VEHICLE model with Deadly Demise is destroyed within 12\" of the bearer, its Deadly Demise automatically triggers.",
        "000008572005": "Arch-negator: ranged weapons equipped by the bearer gain [ANTI-VEHICLE 4+].",
        "000008564002": "Mechanicus Locum: once per battle, at the start of any phase, select a Battle-shocked friendly CULT MECHANICUS unit within 12\" of the bearer to clear Battle-shock.",
        "000008564003": "Mantle of the Gnosticarch: each time an attack is allocated to the bearer, that attack's Damage characteristic is set to 1.",
        "000008564004": "Data-blessed Autosermon: once per battle at the start of your Command phase, optional activation applies the non-selected Benediction of the Omnissiah to the bearer's unit until your next Command phase.",
        "000008564005": "Temporcopia: the bearer's unit has Fights First.",
        "000008568002": "Magos: at the end of your Command phase, if the bearer is within range of your Acquisition objective marker, roll one D6; on a 4+ you gain 1CP.",
        "000008568003": "Genetor: while the bearer is leading a unit that is within range of your Acquisition objective marker, models in that unit have a 4+ invulnerable save.",
        "000008568004": "Logis: while the bearer is leading a unit, each time a model in that unit makes an attack that targets a unit within range of your Acquisition objective marker, add 1 to the Hit roll.",
        "000008568005": "Artisan: while the bearer is leading a unit within range of your Acquisition objective marker, once per phase, you can change one Hit, Wound, or saving throw roll made for that unit to an unmodified 6.",
        "000009745002": "Transoracular Dyad Wafers: while the bearer is attached to Kastelan Robots, that unit gains HALO OVERRIDE and is excluded from Noospheric Transference unit selection; selected HALO OVERRIDE abilities apply to it.",
        ...
        "000008930005": "Veiled Blade: bearer melee weapons gain +2 Attacks; once per battle at the start of any Command phase, the bearer's Objective Control is tripled until end of turn.",
        "000008395002": "Auric Mantle: add 2 to the bearer's Wounds characteristic.",
        "000008395003": "Castellan's Mark: after deployment, redeploy up to two friendly ADEPTUS CUSTODES units (excluding ANATHEMA PSYKANA); selected units can be placed into Strategic Reserves regardless of limits.",
        "000008395004": "From the Hall of Armouries: bearer's melee weapons gain +1 Strength and +1 Damage.",
        "000008395005": "Panoptispex: while the bearer is leading, ranged weapons equipped by models in that unit gain [IGNORES COVER].",
        "000009753004": "Honoured Fallen (Aura): while a friendly ADEPTUS CUSTODES INFANTRY or MOUNTED unit is within 6\" of the bearer, models in that unit re-roll Hit rolls of 1.",
        "000009753005": "Veteran of the Kataphraktoi: in your Command phase, select one friendly ADEPTUS CUSTODES VEHICLE or MOUNTED unit within 6\" of the bearer (or None); selected unit can shoot after Falling Back until your next Command phase.",
        "000008921002": "Aegis Projector: once per turn, the first failed saving throw for the bearer's unit can set that attack's Damage to 0.",
        "000008921003": "Champion of the Imperium: the bearer's Revered Companions aura range (Null Aegis or Deadly Unity) increases to 9\".",
        "000008921004": "Gift of Terran Artifice: each time the bearer makes a melee attack, add 1 to the Wound roll.",
        "000008921005": "Radiant Mantle: each time an attack targets the bearer's unit, if the attacking model is within 12\", subtract 1 from the Hit roll.",
        "000008926002": "Enhanced Voidsheen Cloak: allocated attacks to the bearer are reduced by 1 Damage, or set to Damage 1 if the attacking model is PSYKER or Battle-shocked.",
        "000008926003": "Huntress' Eye: in your Command phase, select one enemy unit within 12\" of the bearer; that unit must take a Battle-shock test.",
        "000008926004": "Oblivion Knight: while the bearer is leading, models in that unit gain +1 to Hit, and gain +1 to Wound when targeting enemy PSYKER units.",
        "000008926005": "Raptor Blade: bearer melee weapons gain +1 Attacks/Strength/Damage, or +2 instead while the bearer is within Engagement Range of one or more Battle-shocked enemy PSYKER units.",
        ...
        "000010348005": "Tome of Forbidden Ways: while the bearer is on the battlefield or in Strategic Reserves, Gate of Infinity maximum selected units increases by 1.",
        "000010364004": "A Foot in the Future: when bearer unit is set up as reinforcements, optional CHOOSE_QUARRY decision (with None) can trigger a reactive Normal move up to D6\" and applies no-charge-until-end-of-turn to that unit.",
        "000010352002": "Eye of the Augurium: once per battle round, bearer unit can use Fire Overwatch or Heroic Intervention for 0CP and can still be targeted even if that Stratagem already targeted a different unit this turn.",
        ...
        "000008881003": "Brutal But Kunnin': in your Command phase, if the bearer is on the battlefield or embarked within a battlefield transport, roll one D6 and add 2 while the bearer's unit effectively counts as containing 10 or more models for enhancement evaluation; on 5+ gain 1CP.",
        ...
        "000008872003": "Speed Makes Right: in your Command phase, if the bearer or the battlefield transport it is embarked within is within 9\" of one or more enemy units, roll one D6 and gain 1CP on 3+.",
        "000009991002": "Da Gobshot Thunderbuss: ranged weapons equipped by the bearer gain [DEVASTATING WOUNDS] and [HAZARDOUS].",
        "000009991003": "Dead Shiny Shootas: ranged weapons equipped by models in the bearer's unit gain [RAPID FIRE 1].",
        ...
        "000009795002": "Skwad Leader: bearer can attach to Kommandos during Declare Battle Formations; while leading Kommandos, bearer has Infiltrators and Stealth.",
        "000009795003": "Mek Kaptin: bearer can attach to Flash Gitz during Declare Battle Formations; models in the bearer's unit can re-roll Hit rolls for ranged attacks.",
        "000009795004": "Mork's Kunnin': after deployment, redeploy up to three friendly ORKS units; selected units may be placed into Strategic Reserves regardless of current reserve unit count.",
        "000009795005": "Gob Boomer: when the bearer issues Taktiks, it can target an eligible unit within 18\" instead of 6\".",
        ...
        "000010308002": "Throne Mechanicum of Skulls: bearer unit re-rolls Charge rolls; once per battle in your Charge phase, optional activation allows charging in a turn in which the bearer unit Advanced.",
        "000010308003": "Blade of Celerity: bearer ranged weapons gain [ASSAULT]; once per battle in the Fight phase, optional activation grants Fights First to the bearer's unit until end of phase.",
        "000010308004": "Warp-borne Stalker: bearer gains Deep Strike; once per battle at the end of your opponent's turn while not within Engagement Range, optional move into Strategic Reserves.",
        ...
        "000008516005": "Veil of Medrengard: bearer has a 4+ invulnerable save against ranged attacks and a 5+ invulnerable save against melee attacks.",
        "000010312002": "Preyslayer's Mantle: bearer gains Super-heavy Walker via enhancement-aware Super-heavy Walker checks.",
        "000010312003": "Final Howl (Aura): friendly WAR DOG units within 6\" of the bearer re-roll Wound rolls of 1.",
        "000010312004": "Loping Predator: bearer ranged weapons gain [ASSAULT].",
        "000010312005": "Panoply of the Cursed Knight: attacks targeting the bearer worsen AP by 1.",
        "000009765002": "Profane Altar: when the bearer's unit makes a Dark Sacrifice, it gains both [LETHAL HITS] and [SUSTAINED HITS 1], and destroyed model count is fixed at 3 on a passed Leadership test or 6 on a failed test.",
        "000009765003": "Pave the Way: at the start of Declare Battle Formations, optional selection of up to three friendly DAMNED units grants selected units Scouts 6\" for the battle.",
        "000009765004": "Tyrant's Banner: while the bearer is on the battlefield, Dark Sacrifice can target a friendly DAMNED unit that is within 6\" of the source unit or visible to it.",
        "000009765005": "Diabolical Resilience: bearer gains Feel No Pain 6+ and can ignore any or all modifiers to Move, Advance, and Charge rolls via deterministic modifier-choice decisions.",
        "000008985002": "Forge's Blessing: in your Command phase, select one friendly HERETIC ASTARTES VEHICLE unit within 12\" of the bearer; selected unit gains Feel No Pain 6+ until the start of your next Command phase.",
        ...
        "000008972002": "Eater of Dread: at the start of your Command phase, while the bearer is on the battlefield, roll D6 with +1 per Battle-shocked enemy unit on the battlefield; on 5+ gain 1CP.",
        "000008972003": "Night's Shroud: models in the bearer's unit have Stealth.",
        "000008972004": "Warp-fuelled Thrusters: at the end of your opponent's turn, if the bearer's unit is not within Engagement Range, optional confirmation places that unit into Strategic Reserves.",
        "000008972005": "Willbreaker: in the Fight phase, after the bearer has made attacks, queue a model-scoped hit-target Battle-shock selection for one enemy unit hit by the bearer.",
        "000008976002": "Bastion Plate: once per battle round, when a saving throw is failed for the bearer's unit, optional activation can set that attack's Damage characteristic to 0.",
        "000008976003": "Iron Artifice: the bearer's ranged weapons gain [ANTI-VEHICLE 4+] and [ANTI-FORTIFICATION 4+].",
        "000008976004": "Ironbound Enmity: while the bearer is within range of an objective marker, add 1 to the bearer's Wound roll.",
        "000008976005": "Warp Tracer: in your Shooting phase, after the bearer has shot, queue a hit-target selection; selected target cannot have the Benefit of Cover until end of phase.",
        "000010641002": "Greyveil Hex: models in the bearer's unit have Stealth; while that unit is within range of an objective marker it controls, ranged attacks can only target it from within 18\".",
        "000010641003": "Warp-fuelled Thrusters: at the end of your opponent's Fight phase, if the bearer's unit is not within Engagement Range, optional confirmation places that unit into Strategic Reserves.",
        "000010641004": "Terrorglut Parasite: at the start of the Fight phase, each enemy unit within Engagement Range of the bearer takes a Battle-shock test at -1.",
        "000010641005": "Sorrowscent Vulture: models in the bearer's unit gain Scouts 6\", and the bearer can attach to Warp Talons in Declare Battle Formations.",
        "000010688002": "Voice of the Tyrant: while the bearer is alive, the bearer's unit gains both Tyrannical Motivation abilities (+1 to Hit and shoot/charge eligibility after Falling Back), including with phase-state handling.",
        "000010688003": "Raid Leader: while the bearer is alive, the bearer's unit remains eligible to charge after disembarking from a Transport that made a Normal move this turn.",
        "000010688004": "Dread Reputation: each time the bearer's unit is set up on the battlefield, enemy units within 6\" (or 12\" when the setup used Deep Strike) take Battle-shock tests.",
        "000010688005": "Eager for Bloodshed: while the bearer is alive, the bearer gains Infiltrators (consumed through unit infiltrate checks with cache invalidation).",
        ...
        "000008964002": "Cursed Fang: bearer melee weapons improve AP by 1 and gain [PRECISION].",
        "000008964003": "Falsehood: Declare Battle Formations choice to start bearer in Reserves; in a Reinforcements step choose a model in a friendly Legionaries/Chosen unit (2+ models, excluding Attached units), remove it, set up bearer near that model, and attach as Leader.",
        "000008964004": "Shroud of Obfuscation: bearer gains Stealth and Lone Operative.",
        "000008964005": "Soul Link: start of your Command phase optional selection of another friendly HERETIC ASTARTES INFANTRY CHARACTER (excluding EPIC HEROES); bearer gains PSYKER and uses selected model's datasheet abilities until your next Command phase.",
        ...
        "000008968005": "Tyrant's Lash: while the bearer is alive and on the battlefield, the bearer's unit can re-roll Advance rolls and can shoot in turns in which it Fell Back.",
        "000010694002": "Weaponised Hatred: once per battle round, after your Vendetta target is destroyed while the bearer is on the battlefield, optionally select one visible enemy unit (or None); the selected unit immediately becomes your Vendetta target.",
        "000010694003": "Eyes of the Hunter: while the bearer is alive, ranged weapons equipped by models in the bearer's unit gain [IGNORES COVER].",
        "000010694004": "Fratricidal Trophies: in a turn where the bearer's unit chose to Default to Doctrine, models in that unit can re-roll Hit rolls for attacks while the bearer is alive.",
        "000010694005": "Empyric Symbiote: add 1 to Advance and Charge rolls made for the bearer's unit while the bearer is alive.",
...
def _stratagem_support(
    ...
    *,
    ...
    notes = {
        ...
        "POUNCE ON THE PREY": "Your Movement phase reaction after a DRUKHARI INFANTRY unit disembarks from a friendly DRUKHARI TRANSPORT that made a Normal move this phase: selected unit can declare a charge this turn.",
        "SKYBORNE ANNIHILATION": "Your Shooting phase: selected DRUKHARI unit that disembarked from a TRANSPORT this turn and has not been selected to shoot gains [SUSTAINED HITS 1] on ranged weapons until end of phase, or [SUSTAINED HITS 2] if the selected unit is KABALITE WARRIORS or HAND OF THE ARCHON.",
        "SWOOPING MOCKERY": "Opponent Movement phase reaction after an enemy unit ends a Normal, Advance, or Fall Back move: selected DRUKHARI TRANSPORT within 9\" can make a Normal move up to 6\".",
        "VICIOUS BLADES": "Fight phase reaction after a friendly DRUKHARI TRANSPORT selects its targets: after that transport has fought, roll one D6 per embarked model (adding 1 for WRACKS models); each 5+ inflicts 1 mortal wound on a selected attacked enemy unit, to a maximum of 6.",
        "SCINTILLATING TEMPO": "Your Movement or Charge phase reaction when a friendly DRUKHARI or HARLEQUINS unit is selected to make a Normal/Advance/Fall Back move, is set up, or declares a charge: selected unit cannot be targeted by Fire Overwatch until end of turn.",
        "WRAITHLIKE RETREAT": "End of Fight phase: selected DRUKHARI INFANTRY unit that fought this phase can make a Normal move up to 6\" if not in Engagement Range, or a Fall Back move if engaged; non-WYCHES units must end wholly within 3\" horizontally and 5\" vertically of a friendly DRUKHARI TRANSPORT and then embark within it.",
        ...
        "INTERLOCKING MANOUEVRES": "End of Fight phase: selected T'AU EMPIRE unit that was eligible to fight this phase can make a Normal move up to 6\" if not in Engagement Range, or a Fall Back move if engaged; if that unit disembarked from a TRANSPORT this turn, it cannot embark after this move.",
        ...
        "GLIMMERSHIFT PORTAL": "End of opponent Fight phase: select up to two SCINTILLATING LEGIONS non-MONSTER units, or one SCINTILLATING LEGIONS MONSTER unit, that are each more than 6\" horizontally from all enemy units; selected units enter Strategic Reserves.",
        ...
        "MERCILESS RECLAMATION": "Shooting/Fight phase: NECRONS (non-MONSTER/non-TITANIC) unit not yet acted gets +1 to wound vs targets within objective range.",
        ...
        "MOB RULE": "End of your Command phase: select ORKS MOB (10+ models, not below half-strength) and a Battle-shocked ORKS INFANTRY within 6\" to clear Battle-shock.",
        ...
        "DRAG IT DOWN": "Fight phase: selected BEAST SNAGGA unit not yet selected to fight gains [SUSTAINED HITS 1] on melee weapons and scores Critical Hits on 5+ when targeting your Prey until end of phase.",
        "INSTINCTIVE HUNTERS": "End of opponent Fight phase: selected BEAST SNAGGA unit not within Engagement Range is removed from the battlefield and placed into Strategic Reserves.",
        "UNSTOPPABLE MOMENTUM": "Your Charge phase reaction after a BEAST SNAGGA MOUNTED unit ends a Charge move: select one enemy within Engagement Range, roll one D6 for each model in your unit (plus 3D6 if the enemy is your Prey), and inflict 1 mortal wound on each 4+, capped at 6.",
        "BASH AND GRAB": "Fight phase: selected ORKS unit not yet selected to fight can re-roll Wound rolls for attacks that target enemy units within range of the loot objective until end of phase.",
        "GRAB AND BASH": "Your Command phase: selected non-GRETCHIN ORKS unit within range of the active loot objective counts as Waaagh-active until the start of your next Command phase without consuming or altering army-level Waaagh call tracking.",
        "DECK FRAGGERS": "Shooting phase: selected ORKS unit not yet selected to shoot gains [BLAST] on ranged attacks that target INFANTRY units until end of phase.",
        "ROLLING LOOT-HEAP": "Shooting phase: selected FLASH GITZ unit not yet selected to shoot gains [ANTI-VEHICLE 4+] on ranged weapons until end of phase.",
        ...
        "SQUIG FLINGIN'": "Your Movement phase reaction after a friendly SPEED FREEKS or TRUKK unit ends a Normal/Advance/Fall Back move: select one enemy unit within 9\" to take a Battle-shock test at -1.",
        ...
        "SPESHUL SHELLS": "Shooting phase: selected ORKS unit not yet selected to shoot improves AP by 1 on ranged attacks that target the closest eligible target within 18\" until end of phase.",
        "DAT ONE'S EVEN BIGGA!": "Your Charge phase: selected BEAST SNAGGA unit can declare a charge after Advancing or Falling Back this turn, and uses prey-gated charge re-roll handling when one declared target is your Prey.",
        "STALKIN' TAKTIKS": "Opponent Shooting phase defensive reaction after enemy targets selected: targeted BEAST SNAGGA INFANTRY/MOUNTED unit gains Benefit of Cover against ranged attacks until end of phase, and gains Stealth as well if the unit has the INFANTRY keyword.",
        "STALKIN\u2019 TAKTIKS": "Opponent Shooting phase defensive reaction after enemy targets selected: targeted BEAST SNAGGA INFANTRY/MOUNTED unit gains Benefit of Cover against ranged attacks until end of phase, and gains Stealth as well if the unit has the INFANTRY keyword.",
        "SPEEDIEST FREEKS": "Opponent Shooting/Fight phase defensive reaction after enemy targets selected: targeted SPEED FREEKS/TRUKK unit gains 5+ invulnerable save until end of phase (improved to 4+ for VEHICLE units with unmodified Toughness 8 or less).",
        "EXTRA GUBBINZ": "Opponent Shooting phase defensive reaction after enemy targets selected: targeted ORKS WALKER/GROTS VEHICLE unit (excluding TITANIC) reduces incoming allocated attack Damage by 1 until end of phase.",
        ...
        "DATS OURS": "Command phase: selected ORKS unit within Engagement Range gains +1 Objective Control until the start of the next Command phase.",
        "TAKTIKAL RETREAT": "Your Movement phase reaction after a friendly ORKS unit Falls Back: that unit can shoot and declare a charge this turn despite Falling Back.",
        "HUGE SHOW OFFS": "Your Command phase: selected ORKS WALKER unit (excluding Killa Kans) gains +1 Move, +1 Leadership, +1 Objective Control, and +1 to Hit rolls until the start of your next Command phase.",
        ...
        "FOETID RESURGENCE": "Your Command phase: selected LEGIONES DAEMONICA NURGLE unit on the battlefield returns up to one destroyed model (or D3 if BATTLELINE), or if it is a MONSTER one model regains up to D3+1 lost wounds.",
        "FEVER VISIONS": "Your Shooting phase or the Fight phase: selected LEGIONES DAEMONICA NURGLE unit that has not been selected to shoot/fight this phase gains +1 to Hit rolls on its attacks, and after it finishes attacking you select one enemy unit it hit to take a Battle-shock test.",
        "MURKSHADOWS": "Your Movement phase: selected LEGIONES DAEMONICA NURGLE INFANTRY unit gains +5\" to Move for Normal moves this phase.",
        "PLAGUE OF WOES": "Opponent Command phase (before Melancholic Miasma target selection): selected LEGIONES DAEMONICA NURGLE unit gains a phase effect so that after Melancholic Miasma resolves one enemy Battle-shock test, one other enemy unit within 9\" of that selected unit is chosen to take a Battle-shock test.",
        "ROT AND RENEWAL": "Your Movement or Charge phase: selected LEGIONES DAEMONICA NURGLE unit can move through terrain features for move/advance/fall back (Movement phase) or charge (Charge phase) moves until end of phase.",
        "SEEPING VIRULENCE": "Fight phase: selected LEGIONES DAEMONICA NURGLE unit that has not been selected to fight this phase scores Critical Hits on unmodified Hit rolls of 5+ in melee until end of phase.",
        "ARCHAGONISTS": "Fight phase: select one LEGIONES DAEMONICA SLAANESH MONSTER that has not fought, or up to two LEGIONES DAEMONICA SLAANESH non-MONSTER units that have not fought; selected units gain +1 to melee Wound rolls until end of phase.",
        ...
        "CAVALCADE OF BLADES": "Your Charge phase reaction after a LEGIONES DAEMONICA SLAANESH unit ends a Charge move: select one enemy unit within Engagement Range and roll D6 for each model in your unit that is within Engagement Range of it (or six D6 if your unit is a MONSTER); each 4+ inflicts 1 mortal wound.",
        ...
        "PHANTASMAL LONGING": "Your Movement or Charge phase: selected LEGIONES DAEMONICA SLAANESH unit can move through terrain features for move/advance/fall back (Movement phase) or charge (Charge phase) moves until end of phase.",
        "SENSORY EXCRUCIATION": "Your Command phase: selected LEGIONES DAEMONICA SLAANESH MONSTER forces every on-battlefield unit within your Shadow of Chaos (friendly and enemy) to take a Battle-shock test, applying -1 to units that are Below Half-strength.",
        ...
        "OVERWHELMING EXCESS": "Opponent Shooting/Fight phase defensive reaction after enemy targets selected: targeted LEGIONES DAEMONICA SLAANESH unit imposes -1 to hit until end of phase.",
        "THIEVES OF PAIN": "Any phase reaction after an attack or mortal wound is allocated to a LEGIONES DAEMONICA SLAANESH non-MONSTER/non-VEHICLE unit: select another friendly SLAANESH unit within 9\" and visible; until end of phase, each wound that target unit would lose is replaced by 1 mortal wound on the selected unit while it remains on the battlefield.",
