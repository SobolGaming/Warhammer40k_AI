...
class BloodTitheDialog(BaseDialog):
    """Modal dialog to select a Blood Tithe ability to activate."""

    def __init__(self, screen_width: int, screen_height: int):
        super().__init__(screen_width, screen_height, width=640, height=420, draggable=True)
        self.title = "Blood Tithe"
    ...
    def draw(self, screen: pygame.Surface):
        ...
        header = f"Blood Tithe Points: {self.points}"
        if self.timing:
            header += f" | Timing: {self.timing.replace('_', ' ').title()}"
        header_surf = self.font_small.render(header, True, TEXT_SECONDARY)
        screen.blit(header_surf, (self.x + 15, self.y + self.title_bar_height + 8))

        if self.source:
            source_surf = self.font_tiny.render(f"Source: {self.source}", True, TEXT_SECONDARY)
            screen.blit(source_surf, (self.x + 15, self.y + self.title_bar_height + 28))
