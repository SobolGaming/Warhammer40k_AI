...
class DaemonPrimarchSlaaneshDialog(BaseDialog):
    """Modal dialog to select a Daemon Primarch of Slaanesh ability."""

    def __init__(self, screen_width: int, screen_height: int):
        super().__init__(screen_width, screen_height, width=680, height=460, draggable=True)
        self.title = "Daemon Primarch of Slaanesh"
    ...
    def draw(self, screen: pygame.Surface):
        ...
        header = "Select one Daemon Primarch of Slaanesh ability (active until your opponent's next Command phase):"
        self.draw_text_wrapped(
            screen,
            header,
            self.x + 15,
            self.y + self.title_bar_height + 8,
            self.width - 30,
            self.font_small,
            TEXT_SECONDARY,
            line_height=18,
        ...
        for i, entry in enumerate(self._option_entries):
            ...
            name = entry.get("label", "Daemon Primarch of Slaanesh")
