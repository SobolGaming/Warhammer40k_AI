...
class GrandCovenDialog(BaseDialog):
    """Modal dialog to select a Grand Coven (Kindred Sorcery) ability."""

    def __init__(self, screen_width: int, screen_height: int):
        super().__init__(screen_width, screen_height, width=560, height=360, draggable=True)
        self.title = "Kindred Sorcery"
    ...
    def draw(self, screen: pygame.Surface):
        ...
        header = self.font_small.render(
            "Select a Kindred Sorcery ability (each once per battle):",
            True,
            TEXT_SECONDARY,
        )
