...
class TemplarVowsDialog(BaseDialog):
    """Modal dialog to select a single Templar Vow at the start of battle round 1."""

    def __init__(self, screen_width: int, screen_height: int):
        super().__init__(screen_width, screen_height, width=560, height=420, draggable=True)
        self.title = "Templar Vows"
    ...
    def draw(self, screen: pygame.Surface):
        ...
        header = self.font_small.render("Select one Vow (cannot be changed):", True, TEXT_SECONDARY)
        ...
        list_w = self.width - 30
        for i, vow in enumerate(self.options):
            ...
            name = getattr(vow, "name", "Vow")
            summary = getattr(vow, "summary", "")
