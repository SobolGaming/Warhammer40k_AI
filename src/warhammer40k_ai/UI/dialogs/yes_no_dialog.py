...
class YesNoDialog(BaseDialog):
    ...
    def show(self, title: str, message: str, callback, *, decision_request):
        ...
        if self.decision_request is not None:
            ...
            for entry in option_entries(self.decision_request):
                ...
                if choice is True:
                    self._yes_option_id = entry.get("option_id", "")
                    self.yes_label = entry.get("label", "Yes") or "Yes"
                elif choice is False:
                    self._no_option_id = entry.get("option_id", "")
                    self.no_label = entry.get("label", "No") or "No"
        if not self._yes_option_id:
            self.yes_label = self.yes_label or "Yes"
        if not self._no_option_id:
            self.no_label = self.no_label or "No"
    ...
    def _handle_button_click(self, button_name: str) -> bool:
        if button_name == "yes":
            ...
            self.hide()
            if cb:
                cb(self._yes_option_id)
            return True
        if button_name == "no":
            cb = self.callback
            self.hide()
            if cb:
                cb(self._no_option_id)
            return True
        return False
