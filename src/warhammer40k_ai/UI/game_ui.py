...
class GameView:
    def __init__(self, screen, env, game, game_map, player1, player2, ui_interface=None):
        ...
        def _request_limb_from_limb_unit(player, game, on_chosen):
            ...
            _resolve_unit_selection_dialog(
                ...
                on_chosen=on_chosen,
                decision_type=DECISION_SELECT_OVERWATCH_SHOOTER,
                prompt="Select a BLOOD ANGELS unit for Limb From Limb.",
                title="Select Limb From Limb Unit",
                subtitle="BLOOD ANGELS unit that charged this turn.",
                enemy_unit=None,
                dialog=self.overwatch_shooter_dialog,
                allow_skip=True,
            )
        ...
        def _request_red_wrath_choice(player, game, unit, on_chosen):
            ...
            _resolve_option_selection_dialog(
                ...
                prompt="Select Red Wrath mode for a BLOOD ANGELS unit.",
                title="Red Wrath",
                header=f"{getattr(unit, 'name', 'Unit')} advanced.",
                subtitle="BLOOD ANGELS unit that Advanced; choose Shoot, Charge, or Red Thirst.",
                context={"unit_id": unit_id},
                allow_skip=True,
            )
        ...
        def _request_a_grim_warning_objective(player, game, candidates, on_chosen):
            ...
            _resolve_option_selection_dialog(
                ...
                prompt="Select an objective marker held by the destroyed BLOOD ANGELS unit.",
                title="A Grim Warning",
                header="Destroyed BLOOD ANGELS unit held an objective.",
                subtitle="Select one objective marker to remain under your control until broken.",
        ...
        # Horde Move prompt queue
        self._pending_horde_move_queue = []
        self._horde_move_flow_active = False
    ...
    def _on_decision_requested(self, request=None, game=None, **_kwargs):
        ...
        if decision_type == DECISION_CHOOSE_START_OF_BATTLE_KEYWORD:
            ...
            dlg = getattr(self, "start_of_battle_keyword_dialog", None)
            if dlg is None:
                try:
                    from .dialogs import QuarrySelectionDialog
                    self.start_of_battle_keyword_dialog = QuarrySelectionDialog(self.screen.get_width(), self.screen.get_height())
                    dlg = self.start_of_battle_keyword_dialog
                except Exception:
                    dlg = None
            if dlg is None:
                default_id = first_option_id(request)
            ...
            ctx = dict(getattr(request, "context", {}) or {})
            ability_name = str(ctx.get("ability_name", "") or "Select Keyword").strip()
            unit_name = ""
            model_name = ""
            try:
                unit_id = str(ctx.get("unit_id", "") or "")
            ...
            title = ability_name or "Select Keyword"
            header = "Select a keyword for this battle."
            if model_name:
                header = f"{model_name} selects a keyword for this battle."
            elif unit_name:
                header = f"{unit_name} selects a keyword for this battle."
            subtitle = "Re-roll Hit and Wound rolls of 1 vs units with the chosen keyword."
        ...
        if decision_type == DECISION_CHOOSE_QUARRY:
            ...
            if ability == "post_shoot_keyword_wound_bonus":
                from ..utility.decision_utils import resolve_decision_command
                from .decision_ui_utils import first_option_id

                if (
                    not hasattr(self, "post_shoot_keyword_wound_bonus_dialog")
                    or self.post_shoot_keyword_wound_bonus_dialog is None
                ):
                    try:
                        from .dialogs import QuarrySelectionDialog

                        self.post_shoot_keyword_wound_bonus_dialog = QuarrySelectionDialog(
                            self.screen.get_width(),
                            self.screen.get_height(),
                        )
                    except Exception:
                        self.post_shoot_keyword_wound_bonus_dialog = None
                dlg = self.post_shoot_keyword_wound_bonus_dialog
                if dlg is None:
                    return
                ...
                ability_name = str(ctx.get("ability_name", "") or "Wound Bonus").strip()
                keyword = str(ctx.get("keyword_phrase", "") or "").strip()
                attack_type = str(ctx.get("attack_type", "") or "any").strip().lower() or "any"
                try:
                ...
                if attack_type in ("ranged", "melee"):
                    attack_label = f"{attack_type} attacks"
                subtitle = f"{keyword_label} {attack_label} vs target: +{int(wound_bonus)} to wound until end of phase."
            ...
            if ability == "post_shoot_snare":
                ...
                if not hasattr(self, "post_shoot_snare_dialog") or self.post_shoot_snare_dialog is None:
                    try:
                        from .dialogs import QuarrySelectionDialog
                        self.post_shoot_snare_dialog = QuarrySelectionDialog(self.screen.get_width(), self.screen.get_height())
                    except Exception:
                        self.post_shoot_snare_dialog = None
                dlg = self.post_shoot_snare_dialog
                if dlg is None:
                    return
                ...
                ability_name = str(ctx.get("ability_name", "") or "Snared").strip()
                weapon_name = str(ctx.get("weapon_name", "") or "").strip()
                header = "Select an enemy unit."
                subtitle = "Target is snared until your next turn."
                if weapon_name:
                    subtitle = f"Hit by {weapon_name}: {subtitle}"
    ...
    def set_game(self, game, game_map, player1, player2) -> None:
        ...
        if self.ui_interface:
            self.ui_interface.game_view = self

        self._wire_map_providers()
        self._subscribe_event_hooks()
        self.phase_manager = PhaseManager(self)
        self.refresh_roster_panes()
        self.update_roster_pane_titles()
    ...
    def _open_voice_of_command_target_dialog(self, player, game, phase_name, trigger, officer, order):
        ...
        target_req = _require_pending_decision_request(self.game if self.game is not None else None,
            DECISION_CHOOSE_QUARRY,
    ...
    def _on_horde_move_prompt(self, player=None, unit=None, attacker_unit=None, game=None, **_kwargs):
        if player is None or unit is None:
            return
        try:
            if player is None or not getattr(player, "has_control", lambda: False)():
        ...
        if self._horde_move_flow_active:
            self._pending_horde_move_queue.append((player, unit, attacker_unit, game))
            return
        self._pending_horde_move_queue.append((player, unit, attacker_unit, game))
        self._open_next_horde_move_prompt(game or self.game)

    def _open_next_horde_move_prompt(self, game):
        q = list(getattr(self, "_pending_horde_move_queue", []) or [])
        if not q:
            self._pending_horde_move_queue = []
            self._horde_move_flow_active = False
            return
        player, unit, attacker_unit, game_ctx = q.pop(0)
        self._pending_horde_move_queue = q

        game_ctx = game_ctx or game or self.game
        if player is None or unit is None or game_ctx is None:
            self._open_next_horde_move_prompt(game_ctx)
            return

        try:
            if not unit.can_horde_move(game=game_ctx, game_map=getattr(game_ctx, "map", None)):
                self._open_next_horde_move_prompt(game_ctx)
                return
        except Exception:
            self._open_next_horde_move_prompt(game_ctx)
        ...
        def _start_horde_move():
            try:
                max_distance = int(game_ctx.roll_horde_move_distance(unit) or 0)
            except Exception:
                max_distance = 0
            if max_distance <= 0:
                _finish_and_next()
            ...
            def _move_done(_completed: bool):
                _finish_and_next()
    ...
    def _open_reverberating_summons_model_prompt(self, *, player, unit, ability_name, game_ctx, on_done):
        ...
        req = _require_pending_decision_request(game_ctx if game_ctx is not None else None,
            DECISION_ALLOCATE_DAMAGE,
    ...
    def _open_cabal_hud_use(self, player) -> None:
        ...
        caster_req = _require_pending_decision_request(self.game if self.game is not None else None,
            DECISION_SELECT_TARGET_MODEL,
            "Select Cabal caster.",
            player_id=getattr(player, "id", None),
        ...
        def _on_caster(option_id: str):
            ...
            ritual_req = _require_pending_decision_request(self.game if self.game is not None else None,
            ...
            def _on_ritual(option_id: str):
                ...
                def _on_target(target_option_id: str):
                    ...
                    channel_req = _require_pending_decision_request(self.game if self.game is not None else None,
                        DECISION_CONFIRM_YES_NO,
                        "Cabal of Sorcerers",
                        player_id=getattr(player, "id", None),
