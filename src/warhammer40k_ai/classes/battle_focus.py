...
class BattleFocusManager:
    ...
    def on_battle_round_start(self, battle_round: int, *, game=None) -> None:
        ...
        self._units_used_this_phase = set()
        self._maneuvers_used_this_phase = set()
        self._fallback_engagement_snapshot = {}
    ...
    def record_enemy_fall_back_start(self, moving_unit, game) -> None:
        ...
        for unit in enemy_units:
            ...
            if bool(getattr(unit, "is_titanic", False)):
                continue
            try:
                if game.map.is_within_engagement_range(moving_unit, unit):
                    eligible.append(unit)
            except Exception:
                continue
        self._fallback_engagement_snapshot[self._unit_id(moving_unit)] = eligible
    ...
    def maybe_trigger_opportunity_seized(self, moving_unit, game) -> None:
        ...
        eligible = self._fallback_engagement_snapshot.pop(self._unit_id(moving_unit), [])
        if not eligible:
            return
        eligible = [u for u in eligible if self._can_use_unit_this_phase(u)]
        if not eligible:
            return
        ...
        ctx = {
            "ability_name": "Battle Focus",
            "trigger": "opportunity_seized",
            "enemy_unit": getattr(moving_unit, "name", "") or "",
            "eligible_units": [getattr(u, "name", "") or "" for u in eligible],
            "tokens": int(self.tokens or 0),
        }

        chosen = None
        if len(eligible) == 1:
            if self._should_use(player, "BATTLE_FOCUS_OPPORTUNITY_SEIZED", ctx):
                chosen = eligible[0]
        else:
            chosen = self._choose_from_options(player, "BATTLE_FOCUS_OPPORTUNITY_SEIZED_UNIT", eligible, ctx)
        if chosen is None:
            return

        self._apply_reactive_move(chosen, self.MANEUVER_OPPORTUNITY, game)
