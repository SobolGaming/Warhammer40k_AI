...
class HarbingersOfDreadManager:
    ...
    def _army_has_harbingers(self) -> bool:
        ...
        try:
            ...
            faction_id = ""
        if faction_id and faction_id != "QT":
            return False
    ...
    def on_battle_round_start(self, battle_round: int, *, game=None) -> None:
        ...
        options = ["Roll 2D6 (randomly select two)"] + [d.name for d in self.get_available_dread_abilities()]
        ...
        if isinstance(choice, str):
            if choice.strip().lower().startswith("roll"):
                self.roll_dread_abilities(battle_round=br)
                return
            for dread in self.get_available_dread_abilities():
                if dread.name.strip().lower() == choice.strip().lower():
                    selected = dread
                    break
