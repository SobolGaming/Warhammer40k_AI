...
class VoiceOfCommandManager:
    ...
    def auto_issue_orders(self, game, player, *, phase_name: str = "", trigger: str = "") -> None:
        if game is None or player is None:
            return
        officers = self.get_eligible_officers(game=game, player=player, phase_name=phase_name, trigger=trigger)
        ...
        for officer in list(officers):
            ...
            while remaining > 0:
                ...
                self.issue_order(game, officer, target, order.key, phase_name=phase_name)
                remaining = self.orders_remaining(officer, battle_round)
