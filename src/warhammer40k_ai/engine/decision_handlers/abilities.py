...
def _validate_select_realm_of_chaos_units(game: object, request: DecisionRequest, result: DecisionResult) -> Sequence[str]:
    ...
    if ability_key == "subterranean_assault_trygon_character_selection":
        ...
        return ()
    if ability_key == "solar_spearhead_walker_character_selection":
        player = resolve_player(game, request.player_id)
        if player is None:
            return ("Auric Armour requires a player.",)
        army = getattr(player, "get_army", lambda: None)()
        if army is None:
            return ("Auric Armour requires an army.",)
        mgr = getattr(army, "adeptus_custodes_detachments", None)
        if mgr is None or not getattr(mgr, "is_solar_spearhead", lambda: False)():
            return ("Auric Armour requires the Solar Spearhead detachment.",)
        validate_fn = getattr(mgr, "solar_spearhead_walker_selection_is_valid", None)
        if not callable(validate_fn):
            return ("Auric Armour validation is unavailable.",)
        valid, reason = validate_fn(list(seen), game=game)
        if not bool(valid):
    ...
    if ability_key == "upon_wings_of_fire_end_of_opponent_turn":
        player = resolve_player(game, request.player_id)
        if player is None:
            return ("Upon Wings of Fire requires a player.",)
        army = getattr(player, "get_army", lambda: None)()
        if army is None:
            return ("Upon Wings of Fire requires an army.",)
        mgr = getattr(army, "space_marines_detachments", None)
        if mgr is None or not getattr(mgr, "is_the_angelic_host", lambda: False)():
            return ("Upon Wings of Fire requires The Angelic Host detachment.",)
        turn_ending_player_id = str(ctx.get("turn_ending_player_id", "") or "")
        turn_ending_player = resolve_player(game, turn_ending_player_id) if turn_ending_player_id else None
        candidates_fn = getattr(mgr, "upon_wings_of_fire_end_of_opponent_turn_candidates", None)
        if not callable(candidates_fn):
            return ("Upon Wings of Fire candidate resolver is unavailable.",)
        candidates = list(
            candidates_fn(
                game=game,
                turn_ending_player=turn_ending_player,
        ...
        for uid in seen:
            if uid not in candidate_ids:
                return ("Upon Wings of Fire selection contains an ineligible unit.",)
        max_units_fn = getattr(mgr, "upon_wings_of_fire_end_of_opponent_turn_max_units", None)
        max_units_allowed = int(max_units_fn(game=game) or 0) if callable(max_units_fn) else int(max_units or 0)
        if len(seen) > max(0, int(max_units_allowed)):
            return ("Too many units selected for Upon Wings of Fire.",)
        resolved_fn = getattr(mgr, "upon_wings_of_fire_phase_already_resolved", None)
        if callable(resolved_fn) and bool(resolved_fn(game=game, turn_ending_player_id=turn_ending_player_id)):
            return ("Upon Wings of Fire has already resolved this end-of-turn window.",)
    if ability_key == "hyperphasing_end_of_opponent_turn":
        player = resolve_player(game, request.player_id)
        if player is None:
            return ("Hyperphasing requires a player.",)
        army = getattr(player, "get_army", lambda: None)()
        if army is None:
            return ("Hyperphasing requires an army.",)
        mgr = getattr(army, "necrons_detachments", None)
        if mgr is None or not getattr(mgr, "is_hypercrypt_legion", lambda: False)():
            return ("Hyperphasing requires a Hypercrypt Legion detachment.",)
        turn_ending_player_id = str(ctx.get("turn_ending_player_id", "") or "")
        turn_ending_player = resolve_player(game, turn_ending_player_id) if turn_ending_player_id else None
        candidates_fn = getattr(mgr, "hyperphasing_end_of_opponent_turn_candidates", None)
        if not callable(candidates_fn):
            return ("Hyperphasing candidate resolver is unavailable.",)
        candidates = list(
            candidates_fn(
                game=game,
                turn_ending_player=turn_ending_player,
        ...
        for uid in seen:
            if uid not in candidate_ids:
                return ("Hyperphasing selection contains an ineligible unit.",)
        max_units_fn = getattr(mgr, "hyperphasing_end_of_opponent_turn_max_units", None)
        max_units_allowed = int(max_units_fn(game=game) or 0) if callable(max_units_fn) else int(max_units or 0)
        if len(seen) > max(0, int(max_units_allowed)):
            return ("Too many units selected for Hyperphasing.",)
...
def _apply_select_realm_of_chaos_units(game: object, request: DecisionRequest, result: DecisionResult):
    ...
    if ability_key == "solar_spearhead_walker_character_selection":
        player = resolve_player(game, request.player_id)
        if player is None:
            raise RuntimeError("Auric Armour player not found.")
        army = getattr(player, "get_army", lambda: None)()
        if army is None:
            raise RuntimeError("Auric Armour army not found.")
        mgr = getattr(army, "adeptus_custodes_detachments", None)
        if mgr is None:
            raise RuntimeError("Auric Armour detachment manager not found.")
        unit_ids = []
        if not is_skip_choice(request, result):
            unit_ids = sorted({str(uid or "").strip() for uid in list(result.payload.get("unit_ids") or []) if str(uid or "").strip()})
    ...
    if ability_key == "upon_wings_of_fire_end_of_opponent_turn":
        player = resolve_player(game, request.player_id)
        if player is None:
            raise RuntimeError("Upon Wings of Fire player not found.")
        army = getattr(player, "get_army", lambda: None)()
        if army is None:
            raise RuntimeError("Upon Wings of Fire army not found.")
        mgr = getattr(army, "space_marines_detachments", None)
        if mgr is None:
            raise RuntimeError("Upon Wings of Fire detachment manager not found.")
        ...
        if not is_skip_choice(request, result):
            ...
            for uid in unit_ids:
                ...
                sr["umbralefic_crystal_must_arrive_turn"] = int(current_turn + 1)
            ...
            if moved_units:
                labels = ", ".join(str(getattr(unit, "name", "Unit") or "Unit") for unit in list(moved_units or []))
                _log_action_for_players(game, player, f"Upon Wings of Fire: {labels} placed into Strategic Reserves.")

        mark_fn = getattr(mgr, "mark_upon_wings_of_fire_phase_resolved", None)
        if callable(mark_fn):
            mark_fn(game=game, turn_ending_player_id=turn_ending_player_id)
        return moved_units
    ...
    if ability_key == "hyperphasing_end_of_opponent_turn":
        player = resolve_player(game, request.player_id)
        if player is None:
            raise RuntimeError("Hyperphasing player not found.")
        army = getattr(player, "get_army", lambda: None)()
        if army is None:
            raise RuntimeError("Hyperphasing army not found.")
        mgr = getattr(army, "necrons_detachments", None)
        if mgr is None:
            raise RuntimeError("Hyperphasing detachment manager not found.")
...
def _validate_choose_quarry(game: object, request: DecisionRequest, result: DecisionResult) -> Sequence[str]:
    ...
    if ability == "renegade_warband_weaponised_hatred_target":
        ...
        army = _resolve_army(game, request, payload)
        if army is None:
            return ("Weaponised Hatred army not found.",)
        mgr = getattr(army, "chaos_space_marines_detachments", None)
        if mgr is None or not bool(getattr(mgr, "is_renegade_warband", lambda: False)()):
            return ("Weaponised Hatred requires Renegade Warband.",)
        player = _resolve_player(game, request, payload)
        if player is None:
            player = getattr(army, "player", None)
        can_select = getattr(mgr, "can_select_weaponised_hatred_target", None)
        if not callable(can_select) or not bool(can_select(game=game, player=player)):
            return ("Weaponised Hatred target cannot be selected right now.",)
        target_unit_id = str(
            payload.get("target_unit_id")
            or payload.get("unit_id")
            or ctx.get("target_unit_id")
        ...
        if not target_unit_id:
            return ("Weaponised Hatred selection requires target_unit_id.",)
        candidate_ids = {
            str(v or "").strip()
            for v in list(ctx.get("candidate_unit_ids", []) or [])
            if str(v or "").strip()
        ...
        if candidate_ids and target_unit_id not in candidate_ids:
            return ("Weaponised Hatred selection contains an ineligible target.",)
        validate_target = getattr(mgr, "weaponised_hatred_target_is_valid", None)
        if not callable(validate_target) or not bool(validate_target(target_unit_id, game=game, player=player)):
            return ("Weaponised Hatred selected target is invalid.",)
        return ()
    ...
    if ability == "librarius_psychic_disciplines":
        ...
        return ()
    if ability == "zealous_litanies":
        payload = _option_payload(request, result)
        army = _resolve_army(game, request, payload)
        if army is None:
            return ("Zealous Litanies army was not found.",)
        sm_mgr = getattr(army, "space_marines_detachments", None)
        if sm_mgr is None or not bool(getattr(sm_mgr, "is_wrathful_procession", lambda: False)()):
            return ("Zealous Litanies requires Wrathful Procession.",)
        can_select = getattr(sm_mgr, "can_select_zealous_litany", None)
        if not callable(can_select) or not bool(can_select(game=game)):
            return ("Zealous Litanies cannot be selected right now.",)
        battle_round = int(getattr(game, "turn", 0) or 0)
        try:
            required_round = int(ctx.get("battle_round", 0) or 0)
        except (TypeError, ValueError):
            required_round = 0
        if required_round > 0 and battle_round != required_round:
            return ("Zealous Litanies selection is no longer valid for this battle round.",)
        if is_skip_choice(request, result):
            return ()
        choice_key = str(payload.get("choice_key", "") or "").strip().upper()
        if not choice_key:
            return ("Zealous Litanies requires selecting a litany or None.",)
        allowed_keys = {
            str(val).strip().upper()
            for val in list(ctx.get("allowed_choice_keys", []) or [])
            if str(val).strip()
        ...
        if allowed_keys and choice_key not in allowed_keys:
            return ("Zealous Litanies selected litany is not an eligible choice.",)
        return ()
    ...
    if ability in (
        ...
        "space_marines_wolf_master_target",
    ):
        if is_skip_choice(request, result):
            if ability == "space_marines_wolf_master_target":
                return ("This Saga of the Hunter enhancement selection cannot be skipped.",)
            return ("This Ironstorm Spearhead enhancement selection cannot be skipped.",)
        payload = _option_payload(request, result)
        source_unit = resolve_unit(game, ctx.get("source_unit_id") or ctx.get("unit_id"))
        if source_unit is None:
            if ability == "space_marines_wolf_master_target":
                return ("Saga of the Hunter source unit was not found.",)
        ...
        if sm_mgr is None:
            return ("This selection requires a Space Marines army.",)
        source_sr = getattr(source_unit, "special_rules", None)
        if not isinstance(source_sr, dict):
            source_sr = {}
        if ability == "space_marines_wolf_master_target":
            if not bool(getattr(sm_mgr, "is_saga_of_the_hunter", lambda: False)()):
                return ("This selection requires a Space Marines Saga of the Hunter army.",)
            if not bool(source_sr.get("enhancement_wolf_master")):
                return ("Source unit does not have Wolf Master.",)
        else:
            if not bool(getattr(sm_mgr, "is_ironstorm_spearhead", lambda: False)()):
        ...
        if target_unit is None:
            if ability == "space_marines_wolf_master_target":
                return ("Saga of the Hunter target unit was not found.",)
            return ("Ironstorm Spearhead target unit was not found.",)
        source_root = source_unit.get_attached_unit_root() if hasattr(source_unit, "get_attached_unit_root") else source_unit
        target_root = target_unit.get_attached_unit_root() if hasattr(target_unit, "get_attached_unit_root") else target_unit
        if source_root is None or target_root is None:
            if ability == "space_marines_wolf_master_target":
                return ("Saga of the Hunter source/target root was not found.",)
            return ("Ironstorm Spearhead source/target root was not found.",)
        if source_root.get_parent_army() is not target_root.get_parent_army():
            if ability == "space_marines_wolf_master_target":
                return ("Saga of the Hunter target must be a friendly unit.",)
        ...
        if ability == "space_marines_wolf_master_target":
            if not bool(getattr(sm_mgr, "attached_unit_is_adeptus_astartes", lambda _u: False)(target_root)):
                return ("Saga of the Hunter target must be a friendly ADEPTUS ASTARTES unit.",)
            is_space_wolves = False
            check_fn = getattr(sm_mgr, "_pack_quarry_unit_is_space_wolves", None)
            if callable(check_fn):
                is_space_wolves = bool(check_fn(target_root))
            if not is_space_wolves:
                return ("Saga of the Hunter target must be a friendly SPACE WOLVES unit.",)
        else:
            if not bool(getattr(sm_mgr, "attached_unit_is_adeptus_astartes", lambda _u: False)(target_root)):
        ...
        if model is None:
            if ability == "space_marines_wolf_master_target":
                return ("Saga of the Hunter bearer model was not found.",)
            return ("Ironstorm Spearhead bearer model was not found.",)
        try:
            default_range = 9 if ability == "space_marines_wolf_master_target" else 6
            range_inches = float(ctx.get("range", default_range) or default_range)
        except (TypeError, ValueError):
            range_inches = 9.0 if ability == "space_marines_wolf_master_target" else 6.0
        in_range_fn = getattr(game, "_unit_within_range_of_model", None)
        if callable(in_range_fn):
            if not bool(in_range_fn(model, target_root, range_value=float(range_inches))):
                if ability == "space_marines_wolf_master_target":
                    return ("Saga of the Hunter target is out of range.",)
...
def _apply_choose_quarry(game: object, request: DecisionRequest, result: DecisionResult):
    ...
    if ability == "renegade_warband_vendetta_target":
        ...
        queue_fn = getattr(mgr, "queue_renegade_warband_weaponised_hatred_choice_request", None)
        if callable(queue_fn):
            queue_fn(game=game, player=player)
        return dict(outcome)
    if ability == "renegade_warband_weaponised_hatred_target":
        payload = _option_payload(request, result)
        army = _resolve_army(game, request, payload)
    ...
    if ability in (
        ...
        if player is None:
            try:
                ...
                player = None
        if ability == "space_marines_wolf_master_target":
            ability_name = str(ctx.get("ability_name", "") or "Wolf Master").strip() or "Wolf Master"
            apply_fn = getattr(game, "_apply_space_marines_wolf_master_effect", None)
            if callable(apply_fn):
                return apply_fn(
                    source_unit=source_unit,
                    target_unit=target_unit,
                    player=player,
                    ability_name=ability_name,
                    weapon_names=list(ctx.get("weapon_names", []) or []),
                )
            return None
    ...
    if ability == "zealous_litanies":
        ...
        apply_choice = getattr(sm_mgr, "select_zealous_litany", None)
        if not callable(apply_choice) or not bool(
            apply_choice(choice_key, battle_round=int(start_round or 0), player_id=player_id)
        ):
            return None

        ability_name = str(ctx.get("ability_name", "") or "Zealous Litanies").strip() or "Zealous Litanies"
        if player is None:
            player = getattr(army, "player", None)
        if not choice_key:
            _log_action_for_players(
                game,
                player,
                f"{ability_name}: no litany selected for battle round {int(start_round or 0)}.",
            )
            return {
                "choice_key": "",
                "choice_name": "None",
        ...
        label_fn = getattr(sm_mgr, "zealous_litany_label", None)
        if callable(label_fn):
            choice_name = str(label_fn(choice_key) or choice_key)
        else:
            choice_name = str(choice_key or "")
...
def _apply_choose_advance_modifier_ignores(game: object, request: DecisionRequest, result: DecisionResult):
    ...
    try:
        ...
        pass
    try:
        filt = getattr(unit, "_filter_firestorm_champion_of_humanity_roll_modifiers", None)
        if callable(filt):
            mods = filt(mods, kind="advance")
    except Exception:
        pass
