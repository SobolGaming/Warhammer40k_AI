...
from ..reserve_entry_rules import (
    evaluate_reserves_arrival_positions as _evaluate_reserves_arrival_positions_shared,
    validate_reserves_arrival_positions as _validate_reserves_arrival_positions_shared,
)
...
def _validate_cursed_circlet_positions(
    ...
    *,
    ...
    reactive_kind = str(context.get("reactive_move_kind", "") or "").strip().lower()
    reactive_move_type = str(context.get("reactive_move_movement_type", "") or "").strip().lower()
    if reactive_kind != "cursed_circlet" and reactive_move_type != "cursed_circlet":
        return ()
    if unit is None:
        return ("Move unit: Cursed Circlet requires a valid unit.",)
    game_map = getattr(game, "map", None)
    if game_map is None:
        return ("Move unit: Cursed Circlet requires a game map.",)

    try:
        from ...utility.calcs import MovementType, get_validation_rules, validate_final_position
    except ImportError:
        return ("Move unit: Cursed Circlet validation rules are unavailable.",)

    validation_rules = get_validation_rules(MovementType.BLOOD_SURGE, moving_unit=unit)
    validation_rules["closest_enemy_unit_reason"] = "Cursed Circlet"
    excluded_keywords = {
        str(value or "").strip().upper()
        for value in list(context.get("cursed_circlet_closest_enemy_exclude_keywords_any", ("AIRCRAFT",)) or ("AIRCRAFT",))
    ...
    for model in list(models or []):
        ...
        if not bool((validation or {}).get("valid", False)):
            reason = str((validation or {}).get("reason", "") or "invalid final position")
            return (f"Move unit: Cursed Circlet {reason}.",)
    return ()
...
def _validate_enfolding_nightmare_positions(
    ...
    *,
    ...
    reactive_kind = str(context.get("reactive_move_kind", "") or "").strip().lower()
    reactive_move_type = str(context.get("reactive_move_movement_type", "") or "").strip().lower()
    if reactive_kind != "enfolding_nightmare" and reactive_move_type != "enfolding_nightmare":
        return ()
    if unit is None:
        return ("Move unit: Enfolding Nightmare requires a valid unit.",)
    game_map = getattr(game, "map", None)
    if game_map is None:
        return ("Move unit: Enfolding Nightmare requires a game map.",)

    try:
        from ...utility.calcs import MovementType, get_validation_rules, validate_final_position
    except ImportError:
        return ("Move unit: Enfolding Nightmare validation rules are unavailable.",)

    validation_rules = get_validation_rules(MovementType.BLOOD_SURGE, moving_unit=unit)
    validation_rules["closest_enemy_unit_reason"] = "Enfolding Nightmare"
    excluded_keywords = {
        str(value or "").strip().upper()
        for value in list(
            context.get("enfolding_nightmare_closest_enemy_exclude_keywords_any", ("AIRCRAFT",))
            or ("AIRCRAFT",)
        )
        if str(value or "").strip()
    }
    ...
    for model in list(models or []):
        ...
        if not bool((validation or {}).get("valid", False)):
            reason = str((validation or {}).get("reason", "") or "invalid final position")
            return (f"Move unit: Enfolding Nightmare {reason}.",)
    return ()


def _validate_xenocreed_path_of_anguish_positions(
    game: object,
    unit: object,
    model_positions: object,
    *,
    ...
    reactive_kind = str(context.get("reactive_move_kind", "") or "").strip().lower()
    reactive_move_type = str(context.get("reactive_move_movement_type", "") or "").strip().lower()
    if reactive_kind != "xenocreed_path_of_anguish" and reactive_move_type != "xenocreed_path_of_anguish":
        return ()
    if unit is None:
        return ("Move unit: The Path of Anguish requires a valid unit.",)
    game_map = getattr(game, "map", None)
    if game_map is None:
        return ("Move unit: The Path of Anguish requires a game map.",)

    try:
        from ...utility.calcs import MovementType, get_validation_rules, validate_final_position
    except ImportError:
        return ("Move unit: The Path of Anguish validation rules are unavailable.",)

    validation_rules = get_validation_rules(MovementType.BLOOD_SURGE, moving_unit=unit)
    validation_rules["closest_enemy_unit_reason"] = "The Path of Anguish"
    excluded_keywords = {
        str(value or "").strip().upper()
        for value in list(
            context.get("xenocreed_path_of_anguish_closest_enemy_exclude_keywords_any", ("AIRCRAFT",))
            or ("AIRCRAFT",)
        )
        if str(value or "").strip()
    }
    ...
    for model in list(models or []):
        ...
        if not bool((validation or {}).get("valid", False)):
            reason = str((validation or {}).get("reason", "") or "invalid final position")
            return (f"Move unit: The Path of Anguish {reason}.",)
    return ()
...
def validate_move_unit_payload(
    ...
    *,
    ...
    if enfolding_nightmare_errors:
        return enfolding_nightmare_errors
    xenocreed_path_of_anguish_errors = _validate_xenocreed_path_of_anguish_positions(
        game,
        unit,
        model_positions,
        ctx=ctx,
...
def _validate_reserves_arrival_positions(
    ...
    *,
    ...
    return _validate_reserves_arrival_positions_shared(game, unit, model_positions, ctx=ctx)
...
def _evaluate_reserves_arrival_positions(
    ...
    *,
    ...
    return _evaluate_reserves_arrival_positions_shared(game, unit, model_positions, ctx=ctx)
...
def _validate_pick_point(game: object, request: DecisionRequest, result: DecisionResult) -> Sequence[str]:
    ...
    if ability_key == "cult_ambush_threatened_marker_relocation":
        ...
        source_unit_id = str(option_payload.get("source_unit_id", "") or "")
        if not marker_id:
    ...
    if ability_key == "regimental_reinforcements_marker_placement":
        validate_fn = getattr(game, "_validate_regimental_reinforcements_marker_placement", None)
        if not callable(validate_fn):
            return ("Regimental Reinforcements marker validation is unavailable.",)
        valid, reason = validate_fn(ctx, point=point)
        if not bool(valid):
            return (str(reason or "Regimental Reinforcements marker point is invalid."),)
...
def _apply_pick_point(game: object, request: DecisionRequest, result: DecisionResult) -> None:
    ...
    if ability_key == "cult_ambush_threatened_marker_relocation":
        ...
        return (x, y)
    if ability_key == "regimental_reinforcements_marker_placement":
        apply_fn = getattr(game, "_apply_regimental_reinforcements_marker_placement", None)
        if not callable(apply_fn):
            return None
        applied = bool(apply_fn(ctx, point=point))
        if not applied:
            return None
        if len(point) > 2:
            return (x, y, float(point[2]))
...
def _validate_pick_objective(game: object, request: DecisionRequest, result: DecisionResult) -> Sequence[str]:
    ...
    if ability == "corrupt_realspace":
        ...
        source_unit = get_unit(game, str(ctx.get("source_unit_id", "") or ctx.get("unit_id", "") or ""))
        if source_unit is None:
            return ("Corrupt Realspace source unit was not found.",)
        ...
        if objective is None:
            return ("Corrupt Realspace selected objective marker was not found.",)
        candidate_objective_ids = {
            str(value or "").strip()
            for value in list(ctx.get("candidate_objective_ids", []) or [])
            if str(value or "").strip()
        }
        if candidate_objective_ids and str(objective_id or "").strip() not in candidate_objective_ids:
            return ("Corrupt Realspace selected objective marker is not in this request's candidate list.",)
        live_candidates = list(getattr(stratagems, "_corrupting_taint_objective_candidates", lambda _unit: [])(source_root) or [])
        live_ids = {
            str(get_entity_id(candidate) or "").strip()
            for candidate in list(live_candidates or [])
            if candidate is not None
        }
        if str(objective_id or "").strip() not in live_ids:
            return ("Corrupt Realspace selected objective marker is no longer eligible.",)
        return ()
...
def _apply_pick_objective(game: object, request: DecisionRequest, result: DecisionResult) -> None:
    ...
    if is_skip_choice(request, result):
        if ability == "corrupt_realspace":
            player = resolve_player(game, getattr(request, "player_id", None))
            stratagems = getattr(player, "stratagems", None) if player is not None else None
            dequeue = getattr(stratagems, "_dequeue_reaction_by_name", None) if stratagems is not None else None
            if callable(dequeue):
    ...
    if ability == "corrupt_realspace":
        ...
        source_unit = get_unit(game, str(ctx.get("source_unit_id", "") or ctx.get("unit_id", "") or ""))
        if source_unit is None:
            raise RuntimeError("Corrupt Realspace source unit was not found.")
        source_root = source_unit.get_attached_unit_root() if hasattr(source_unit, "get_attached_unit_root") else source_unit
        if source_root is None:
            raise RuntimeError("Corrupt Realspace source unit was not found.")
        objective = get_objective(game, str(objective_id or ""))
        if objective is None:
            raise RuntimeError("Corrupt Realspace selected objective marker was not found.")
