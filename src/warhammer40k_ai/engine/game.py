...
class Game(
    ...
    def _maybe_prompt_end_of_opponent_turn_strategic_reserves(self, turn_ending_player=None) -> None:
        ...
        for opp in list(self.players or []):
            ...
            warp_candidates_fn = (
                getattr(csm_mgr, "warp_portals_end_of_opponent_turn_candidates", None)
                if csm_mgr is not None
                else None
            )
            warp_max_units_fn = (
                getattr(csm_mgr, "warp_portals_end_of_opponent_turn_max_units", None)
                if csm_mgr is not None
                else None
            )
            warp_resolved_fn = (
                getattr(csm_mgr, "warp_portals_phase_already_resolved", None)
                if csm_mgr is not None
                else None
            )
            if callable(warp_candidates_fn):
                resolved = False
                if callable(warp_resolved_fn):
                    resolved = bool(
                        warp_resolved_fn(
                            game=self,
                            turn_ending_player_id=turn_ending_player_id,
                ...
                if not resolved:
                    ...
                    max_units = int(warp_max_units_fn(game=self) or 0) if callable(warp_max_units_fn) else 0
                    max_units = max(0, min(int(max_units), len(warp_candidates)))
                    candidate_ids = [
                        str(get_entity_id(unit) or "")
                        for unit in list(warp_candidates or [])
                        if str(get_entity_id(unit) or "").strip()
                    ]
                    candidate_ids = sorted(set(candidate_ids))
                    if candidate_ids and max_units > 0:
                        ...
                        if queue is not None and hasattr(queue, "list"):
                            for req in list(queue.list() or []):
                                ...
                                if str(getattr(req, "player_id", "") or "") != str(getattr(opp, "id", "") or ""):
                                    continue
                                req_ctx = dict(getattr(req, "context", {}) or {})
                                if str(req_ctx.get("ability", "") or "").strip().lower() != "warp_portals_end_of_opponent_turn":
                                    continue
                                if str(req_ctx.get("turn_key", "") or "") != phase_key:
                                    continue
                                pending = True
                        ...
                        if not pending:
                            request = DecisionRequest.create(
                                DECISION_SELECT_REALM_OF_CHAOS_UNITS,
                                "Warp Portals: select units to place into Strategic Reserves.",
                                player_id=getattr(opp, "id", None),
                                options=[
                                    DecisionOption.create("Confirm selection", payload={"action": "confirm"}),
                                    DecisionOption.create("Do not use", payload={"action": "skip"}),
                                ...
                                context={
                                    "ability": "warp_portals_end_of_opponent_turn",
                                    "ability_name": "Warp Portals",
                                    "allowed_unit_ids": list(candidate_ids),
                                    "outside_shadow_unit_ids": [],
                                    "max_units": int(max_units),
                                    "selection_effect": "enter_strategic_reserves",
                                    "selection_effect_reason": "Warp Portals",
                                    "turn_key": phase_key,
                                    "turn_ending_player_id": turn_ending_player_id,
                                    "title": "Warp Portals",
                                    "subtitle": (
                                        f"Select up to {int(max_units)} HERETIC ASTARTES TERMINATOR, "
                                        "OBLITERATORS or MUTILATORS unit(s)"
                                    ),
                                    "instruction": (
                                        "Choose units to remove from the battlefield and place into Strategic Reserves, "
                                        "or select None to skip."
    ...
    def _on_unit_set_up_grey_knights_detachments(
        self,
        unit=None,
        set_up_as_reinforcements: bool = False,
        used_deep_strike: bool = False,
    ...
    def _cult_ambush_manager_for_player_id(self, player_id: str):
        key = str(player_id or "").strip()
        if not key:
            return None
        for player in list(self.players or []):
            if player is None or str(getattr(player, "id", "") or "") != key:
                continue
    ...
    def _handle_cult_ambush_threatened_markers(self, mgr, enemy_unit, threatened_markers) -> list[str]:
        if mgr is None or not threatened_markers:
            return []
        if not bool(getattr(self, "is_authoritative", True)):
            return []
        ...
        threatened.sort(key=lambda marker: str(getattr(marker, "marker_id", "") or ""))
        if not threatened:
            return []

        options = [DecisionOption.create("Skip", payload={"action": "skip"})]
        threatened_marker_ids: list[str] = []
        for idx, marker in enumerate(threatened, start=1):
            ...
            if can_shadow:
                for source_unit in list(shadow_sources or []):
                    ...
                    options.append(
                        DecisionOption.create(
                            f"Marker {idx} (In the Shadow of Iron: {source_name})",
                            payload={
                                "marker_id": marker_id,
        ...
        if can_evasive and not can_summon and not can_shadow:
            ...
            instruction = "Choose a threatened marker, then choose its relocation point, or Skip."
        elif can_shadow and not can_summon and not can_evasive:
            ability = "cult_ambush_threatened_marker_relocation"
            ability_name = "Cult Ambush Threatened Marker Relocation"
            prompt = (
                "In the Shadow of Iron: select one threatened Cult Ambush marker and a relocation point more than 9\" "
                "horizontally from all enemy units and wholly within 6\" of the selected vehicle, or Skip."
            )
            instruction = "Choose a threatened marker and source vehicle, then choose its relocation point, or Skip."
        elif relocation_mode_count > 1:
            ability = "cult_ambush_threatened_marker_relocation"
            ability_name = "Cult Ambush Threatened Marker Relocation"
            prompt = (
        ...
        self.request_decision(request)
        return list(threatened_marker_ids)

    def _validate_summon_the_cult_marker_relocation(self, context: dict, *, marker_id: str, point) -> tuple[bool, str]:
        mgr = self._cult_ambush_manager_for_player_id(str(context.get("owner_player_id", "") or ""))
        if mgr is None:
            return (False, "Summon the Cult manager is unavailable.")
        validate_fn = getattr(mgr, "validate_summon_the_cult_relocation", None)
        if not callable(validate_fn):
            return (False, "Summon the Cult validation is unavailable.")
        return validate_fn(str(marker_id or ""), point, game=self)

    def _apply_summon_the_cult_marker_relocation(self, context: dict, *, marker_id: str, point) -> bool:
        mgr = self._cult_ambush_manager_for_player_id(str(context.get("owner_player_id", "") or ""))
        if mgr is None:
            return False
        apply_fn = getattr(mgr, "apply_summon_the_cult_relocation", None)
        ...
        player = getattr(getattr(mgr, "army", None), "player", None)
        if player is not None:
            append_action(player, "Summon the Cult: relocated a threatened Cult Ambush marker.")
        return True

    def _skip_summon_the_cult_marker_relocation(self, context: dict) -> None:
        mgr = self._cult_ambush_manager_for_player_id(str(context.get("owner_player_id", "") or ""))
        if mgr is None:
            return
    ...
    def _validate_cult_ambush_threatened_marker_relocation(
        ...
        *,
        ...
        if mode == "in_the_shadow_of_iron":
            mgr = self._cult_ambush_manager_for_player_id(str(context.get("owner_player_id", "") or ""))
            if mgr is None:
                return (False, "In the Shadow of Iron manager is unavailable.")
            validate_fn = getattr(mgr, "validate_in_the_shadow_of_iron_relocation", None)
            if not callable(validate_fn):
                return (False, "In the Shadow of Iron validation is unavailable.")
            source_unit = self._resolve_unit_by_id(source_unit_id)
            return validate_fn(str(marker_id or ""), point, source_unit=source_unit, game=self)
    ...
    def _apply_cult_ambush_threatened_marker_relocation(
        ...
        *,
        ...
        if mode == "in_the_shadow_of_iron":
            ...
            if player is not None:
                append_action(player, "In the Shadow of Iron: relocated a threatened Cult Ambush marker.")
            return True
        return False
    ...
    def _validate_regimental_reinforcements_marker_placement(self, context: dict, *, point) -> tuple[bool, str]:
        mgr = self._cult_ambush_manager_for_player_id(str(context.get("owner_player_id", "") or ""))
        if mgr is None:
            return (False, "Regimental Reinforcements manager is unavailable.")
        validate_fn = getattr(mgr, "validate_regimental_reinforcements_marker_placement", None)
        if not callable(validate_fn):
            return (False, "Regimental Reinforcements validation is unavailable.")
        return validate_fn(point, game=self)

    def _apply_regimental_reinforcements_marker_placement(self, context: dict, *, point) -> bool:
        mgr = self._cult_ambush_manager_for_player_id(str(context.get("owner_player_id", "") or ""))
        if mgr is None:
            return False
        apply_fn = getattr(mgr, "place_regimental_reinforcements_marker", None)
        if not callable(apply_fn):
            return False
        marker = apply_fn(point, game=self)
        if marker is None:
            return False
        from ..utility.event_bus import append_action
        ...
        player = getattr(getattr(mgr, "army", None), "player", None)
        if player is not None:
            append_action(player, "Regimental Reinforcements: placed a Cult Ambush marker.")
        return True
    ...
    def _enter_command_context(self) -> None:
        return _command_service_for(self)._enter_command_context()

    def _exit_command_context(self) -> None:
        return _command_service_for(self)._exit_command_context()

    def in_command_context(self) -> bool:
        return _command_service_for(self).in_command_context()

    def _maybe_queue_post_command_tool_decisions(self, command: GameCommand, result) -> bool:
        return _command_service_for(self)._maybe_queue_post_command_tool_decisions(command, result)

    def apply_command(self, command: GameCommand):
        return _command_service_for(self).apply_command(command)

    def process_command_queue(self, *, limit: int | None = None):
        return _command_service_for(self).process_command_queue(limit=limit)

    def next_command(self) -> GameCommand | None:
        return _command_service_for(self).next_command()
    ...
    def _emergency_combat_embarkation_used_this_turn(self, *args, **kwargs):
        return _charge_service_for(self)._emergency_combat_embarkation_used_this_turn(*args, **kwargs)

    def _mark_emergency_combat_embarkation_used_this_turn(self, *args, **kwargs):
        return _charge_service_for(self)._mark_emergency_combat_embarkation_used_this_turn(*args, **kwargs)

    def _charge_declaration_candidate_targets(self, *args, **kwargs):
        return _charge_service_for(self)._charge_declaration_candidate_targets(*args, **kwargs)

    def _emergency_charge_retarget_candidates(self, *args, **kwargs):
        return _charge_service_for(self)._emergency_charge_retarget_candidates(*args, **kwargs)

    def _charge_model_base_radius(self, *args, **kwargs):
        return _charge_service_for(self)._charge_model_base_radius(*args, **kwargs)

    def _iter_charge_destination_candidates(self, *args, **kwargs):
        return _charge_service_for(self)._iter_charge_destination_candidates(*args, **kwargs)

    def _find_charge_destination(self, *args, **kwargs):
        return _charge_service_for(self)._find_charge_destination(*args, **kwargs)

    def _queue_emergency_combat_embarkation_interrupt(self, *args, **kwargs):
        return _charge_service_for(self)._queue_emergency_combat_embarkation_interrupt(*args, **kwargs)

    def _complete_charge_declaration(self, *args, **kwargs):
        return _charge_service_for(self)._complete_charge_declaration(*args, **kwargs)

    def _bind_charge_move_targets(self, *args, **kwargs):
        return _charge_service_for(self)._bind_charge_move_targets(*args, **kwargs)

    def continue_charge_after_emergency_combat_embarkation(self, *args, **kwargs):
        return _charge_service_for(self).continue_charge_after_emergency_combat_embarkation(*args, **kwargs)

    def resolve_emergency_combat_embarkation(self, *args, **kwargs):
        return _charge_service_for(self).resolve_emergency_combat_embarkation(*args, **kwargs)

    def declare_charge(self, *args, **kwargs):
        return _charge_service_for(self).declare_charge(*args, **kwargs)

    def _finalize_successful_charge_move(self, *args, **kwargs):
        return _charge_service_for(self)._finalize_successful_charge_move(*args, **kwargs)

    def roll_blood_surge_distance(self, *args, **kwargs):
        return _charge_service_for(self).roll_blood_surge_distance(*args, **kwargs)

    def roll_unhinged_vengeance_distance(self, *args, **kwargs):
        return _charge_service_for(self).roll_unhinged_vengeance_distance(*args, **kwargs)

    def roll_blistering_assault_distance(self, *args, **kwargs):
        return _charge_service_for(self).roll_blistering_assault_distance(*args, **kwargs)

    def roll_bestial_rage_distance(self, *args, **kwargs):
        return _charge_service_for(self).roll_bestial_rage_distance(*args, **kwargs)

    def roll_aggressive_leader_beast_distance(self, *args, **kwargs):
        return _charge_service_for(self).roll_aggressive_leader_beast_distance(*args, **kwargs)

    def roll_brazen_fury_distance(self, *args, **kwargs):
        return _charge_service_for(self).roll_brazen_fury_distance(*args, **kwargs)

    def roll_horde_move_distance(self, *args, **kwargs):
        return _charge_service_for(self).roll_horde_move_distance(*args, **kwargs)

    def roll_loping_speed_distance(self, *args, **kwargs):
        return _charge_service_for(self).roll_loping_speed_distance(*args, **kwargs)

    def attempt_charge(self, *args, **kwargs):
        return _charge_service_for(self).attempt_charge(*args, **kwargs)

    def _collect_charge_modifiers(self, *args, **kwargs):
        return _charge_service_for(self)._collect_charge_modifiers(*args, **kwargs)

    def _get_charge_roll_spec(self, *args, **kwargs):
        return _charge_service_for(self)._get_charge_roll_spec(*args, **kwargs)

    def get_charge_roll_modifiers(self, *args, **kwargs):
        return _charge_service_for(self).get_charge_roll_modifiers(*args, **kwargs)

    def get_max_charge_distance(self, *args, **kwargs):
        return _charge_service_for(self).get_max_charge_distance(*args, **kwargs)

    def _apply_charge_modifiers(self, *args, **kwargs):
        return _charge_service_for(self)._apply_charge_modifiers(*args, **kwargs)

    def get_eligible_charging_units(self, *args, **kwargs):
        return _charge_service_for(self).get_eligible_charging_units(*args, **kwargs)

    def is_charge_phase_complete(self, *args, **kwargs):
        return _charge_service_for(self).is_charge_phase_complete(*args, **kwargs)

    ###########################################################################
    # Fight Phase
    ###########################################################################

    def get_eligible_fighting_units(self, *args, **kwargs):
        return _fight_service_for(self).get_eligible_fighting_units(*args, **kwargs)

    def get_fight_first_units(self, *args, **kwargs):
        return _fight_service_for(self).get_fight_first_units(*args, **kwargs)

    def get_remaining_combatant_units(self, *args, **kwargs):
        return _fight_service_for(self).get_remaining_combatant_units(*args, **kwargs)

    def get_fight_phase_units_by_stage(self, *args, **kwargs):
        return _fight_service_for(self).get_fight_phase_units_by_stage(*args, **kwargs)

    def is_fight_phase_complete(self, *args, **kwargs):
        return _fight_service_for(self).is_fight_phase_complete(*args, **kwargs)
