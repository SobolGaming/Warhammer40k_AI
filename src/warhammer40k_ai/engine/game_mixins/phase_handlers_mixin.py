...
class GamePhaseHandlersMixin:
        ...
        if pname == "FIGHT_PHASE":
            ...
            for unit in list(army.units):
                ...
                for m in models:
                    if not getattr(m, "is_alive", True):
                        continue
                    get_specs = getattr(unit, "model_start_fight_phase_melee_attacks_set_invuln_specs", None)
                    specs = list(get_specs(m) or []) if callable(get_specs) else []
                    if not specs:
                        continue
                    for spec in specs:
                        key = str(spec.get("key") or "fight_phase_melee_attacks_set_invuln").strip().lower()
                        if not key:
                        ...
                        unit_id = maybe_entity_id(unit)
                        model_id = maybe_entity_id(m)
                        ability_name = str(spec.get("source", "") or "Fight phase invulnerable/attacks set").strip()
                        try:
                            invuln = int(spec.get("invuln", 0) or 0)
                        except Exception:
                            invuln = 0
                        try:
                            attacks_value = int(spec.get("attacks_value", 0) or 0)
                        except Exception:
                            attacks_value = 0
                        ...
                        self._queue_optional_ability_confirmation(
                            player=player,
                            ability_key="thrilling_spectacle",
                            ability_name=ability_name,
                            message=message,
                            context=ctx,
                            payload={
    ...
    def _on_phase_start_adaptive_instincts(self, player=None, phase=None, **_kwargs) -> None:
        pname = str(getattr(phase, "name", "") or "").strip().upper()
        if pname != "FIGHT_PHASE":
            return
        if not bool(getattr(self, "is_authoritative", True)):
        ...
        from ..decision_kinds import DECISION_CHOOSE_ADAPTIVE_INSTINCTS
        from ..decisions import DecisionOption, DecisionRequest
        ...
        for p in players:
            ...
            for unit in sorted(list(getattr(army, "units", []) or []), key=_unit_sort_key):
                ...
                has_fn = getattr(root, "has_adaptive_instincts", None)
                if not callable(has_fn) or not bool(has_fn()):
                    continue
                choice_fn = getattr(root, "_adaptive_instincts_choice", None)
                if callable(choice_fn) and choice_fn(game=self):
                    continue
                if unit_id and unit_id in pending_units:
                    continue
                options = [
                    DecisionOption.create(
                        ...
                        payload={
                            "choice": "AGGRESSION",
                            "summary": "Re-roll Hit rolls of 1 for this unit.",
                            "unit_id": unit_id,
                        },
                    ),
                    DecisionOption.create(
                        "Bioregeneration",
                        payload={
                            "choice": "BIOREGENERATION",
                            "summary": "Re-roll saving throws of 1 for this unit.",
                            "unit_id": unit_id,
                        },
                    ),
                ]
                ctx = {
                    "unit_id": unit_id,
                    "ability_name": "Adaptive Instincts",
                    "phase_name": pname,
                }
                req = DecisionRequest.create(
                    DECISION_CHOOSE_ADAPTIVE_INSTINCTS,
                    "Adaptive Instincts: select one imperative.",
                    player_id=getattr(p, "id", None),
                    options=options,
                    context=ctx,
                )
    ...
    def _on_phase_start_fight_phase_select_engagement_battleshock(self, player=None, phase=None, **_kwargs) -> None:
        """Fight phase: select one enemy unit in Engagement Range to take a Battle-shock test."""
        pname = str(getattr(phase, "name", "") or "").strip().upper()
        if pname != "FIGHT_PHASE":
            return
        if not bool(getattr(self, "is_authoritative", True)):
        ...
        for p in list(getattr(self, "players", []) or []):
            if p is None:
                raise RuntimeError("Fight phase select Engagement Battle-shock requires players.")
            army = p.get_army()
            if army is None:
                raise RuntimeError(f"Fight phase select Engagement Battle-shock requires an army for {p.name}.")
            ...
            for unit in sorted(list(getattr(army, "units", []) or []), key=_unit_sort_key):
                ...
                spec_fn = getattr(root, "model_start_fight_phase_select_engagement_battleshock_specs", None)
        ...
        for p in list(getattr(self, "players", []) or []):
            ...
            for candidate in list(army.units or []):
                ...
                try:
                    ...
                    croot = candidate
                if croot is None or not croot.is_alive() or not getattr(croot, "deployed", True):
                    continue
                try:
                    if croot.is_in_reserves() or croot.is_embarked:
                        continue
                except Exception:
                    pass
                cid = str(get_entity_id(croot) or "")
                if not cid or cid in seen_friendly:
                    continue
            ...
            for unit in sorted(list(army.units or []), key=_unit_sort_key):
                ...
                for model in sorted([m for m in models if getattr(m, "is_alive", True)], key=_model_sort_key):
                    ...
                    if model_id and model_id in pending_models:
                        continue
                    spec_fn = getattr(root, "model_start_fight_phase_friendly_melee_ws_bonus_specs", None)
                    if not callable(spec_fn):
                        continue
                    specs = spec_fn(model) or []
                    if not specs:
                        continue
                    spec = specs[0]
                    ability_name = str(spec.get("source", "") or "Accelerator Mandible").strip() or "Accelerator Mandible"
                    keyword = str(spec.get("keyword", "") or "canoptek").strip()
                    try:
                        range_value = int(spec.get("range", 3) or 3)
                    except Exception:
                        range_value = 3
                    try:
                        ws_bonus = int(spec.get("ws_bonus", 1) or 1)
                    except Exception:
                        ws_bonus = 1
                    ...
                    for candidate_root in list(friendly_roots):
                        ...
                        if keyword:
                            try:
                                keyword_match = bool(candidate_root.has_any_keyword(keyword))
                            except Exception:
                                keyword_match = False
                            if not keyword_match:
                                try:
                                    keyword_match = bool(candidate_root.has_keyword(keyword))
                                except Exception:
                                    keyword_match = False
                        if not keyword_match:
                            continue
                        ...
                        try:
                            in_range = bool(
                                float(game_map.get_distance_between_units(root, candidate_root))
                                <= float(range_value)
                            )
                        except Exception:
                            in_range = False
                    ...
                    request = DecisionRequest.create(
                        DECISION_CHOOSE_QUARRY,
                        f"{ability_name}: select a friendly {keyword.upper()} unit (or None).",
                        player_id=getattr(p, "id", None),
                        options=options,
                        context={
                            "ability": "accelerator_mandible",
                            "ability_name": ability_name,
                            "source_unit_id": get_entity_id(root),
    ...
    def _on_phase_start_ironstorm_spearhead_enhancements(self, player=None, phase=None, **_kwargs) -> None:
        ...
        def _unit_has_vehicle(unit) -> bool:
            if unit is None:
                return False
            try:
                if bool(unit.has_any_keyword("VEHICLE")):
                    return True
            except Exception:
                pass
            try:
                if bool(unit.has_keyword("VEHICLE")):
                    return True
            except Exception:
                pass
            keywords = list(getattr(unit, "keywords", []) or []) + list(getattr(unit, "faction_keywords", []) or [])
            return any(str(v or "").strip().upper() == "VEHICLE" for v in keywords)
        ...
        if own_player_id:
            for root in list(all_roots):
                ...
                if not isinstance(sr, dict):
                    continue
                if str(sr.get("enhancement_target_augury_web_owner", "") or "") == own_player_id:
                    for key in (
                        "enhancement_target_augury_web_active",
                        "enhancement_target_augury_web_owner",
                        "enhancement_target_augury_web_source",
                        "enhancement_target_augury_web_source_unit_id",
                    ):
                        sr.pop(key, None)
                if str(sr.get("enhancement_master_of_machine_war_owner", "") or "") == own_player_id:
                    for key in (
                        "enhancement_master_of_machine_war_active",
                        "enhancement_master_of_machine_war_owner",
                        "enhancement_master_of_machine_war_source",
                        "enhancement_master_of_machine_war_source_unit_id",
                    ):
                        sr.pop(key, None)
                root.special_rules = sr
        ...
        def _queue_or_apply(
            *,
            ...
            if source_root is None or source_unit is None:
                return
            if not bool(source_sr.get(enhancement_flag, False)):
                return
            if not _unit_active(source_root):
                return
            bearer = getattr(source_unit, "_get_enhancement_bearer_model", lambda: None)()
            ...
            if bearer is None or not bool(getattr(bearer, "is_alive", True)):
                return
            try:
                range_inches = int(source_sr.get(range_key, 6) or 6)
            except (TypeError, ValueError):
                range_inches = 6
            range_inches = max(1, int(range_inches))
            candidates: list = []
            for target_root in list(all_roots):
                if not _unit_active(target_root):
                    continue
                if not _unit_has_vehicle(target_root):
                    continue
                if not bool(sm_mgr.attached_unit_is_adeptus_astartes(target_root)):
                    continue
                if not _model_in_unit_range(bearer, target_root, float(range_inches)):
                    continue
                candidates.append(target_root)
            if not candidates:
                return
            candidates.sort(key=lambda u: str(get_entity_id(u) or ""))
            ability_name = str(source_sr.get(source_key, "") or default_name).strip() or default_name
            apply_fn = getattr(self, apply_fn_name, None)
            if len(candidates) == 1 and callable(apply_fn):
                apply_fn(
                    source_unit=source_unit,
                    target_unit=candidates[0],
        ...
        for source_root in list(all_roots):
            ...
            for source_unit in list(members or []):
                ...
                _queue_or_apply(
                    ...
                    source_unit=source_unit,
                    source_sr=source_sr,
                    enhancement_flag="enhancement_target_augury_web",
                    range_key="enhancement_target_augury_web_range",
                    source_key="enhancement_target_augury_web_source",
                    default_name="Target Augury Web",
                    ability_key="space_marines_target_augury_web_target",
                    apply_fn_name="_apply_space_marines_target_augury_web_effect",
                )
                _queue_or_apply(
                    source_root=source_root,
                    source_unit=source_unit,
                    source_sr=source_sr,
                    enhancement_flag="enhancement_master_of_machine_war",
                    range_key="enhancement_master_of_machine_war_range",
                    source_key="enhancement_master_of_machine_war_source",
                    default_name="Master of Machine War",
                    ability_key="space_marines_master_of_machine_war_target",
                    apply_fn_name="_apply_space_marines_master_of_machine_war_effect",
                )

    def _apply_space_marines_target_augury_web_effect(
        self,
        *,
        source_unit=None,
        target_unit=None,
        player=None,
        ability_name: str = "Target Augury Web",
    ) -> bool:
        if source_unit is None or target_unit is None:
            return False
        from ...utility.entity_ids import get_entity_id
        ...
        if source_root is None or target_root is None:
            return False
        source_sr = getattr(source_unit, "special_rules", None)
        if not isinstance(source_sr, dict) or not bool(source_sr.get("enhancement_target_augury_web")):
            return False
        try:
            if source_root.get_parent_army() is not target_root.get_parent_army():
                return False
        ...
        if sm_mgr is not None and not bool(getattr(sm_mgr, "attached_unit_is_adeptus_astartes", lambda _u: False)(target_root)):
            return False
        try:
            if not bool(target_root.has_any_keyword("VEHICLE") or target_root.has_keyword("VEHICLE")):
                return False
        except Exception:
            return False
        owner_id = str(getattr(player, "id", "") or "")
        ...
        if not isinstance(sr, dict):
            sr = {}
        sr["enhancement_target_augury_web_active"] = True
        sr["enhancement_target_augury_web_owner"] = owner_id
        sr["enhancement_target_augury_web_source"] = str(ability_name or "Target Augury Web").strip() or "Target Augury Web"
        source_unit_id = str(get_entity_id(source_unit) or "")
        if not source_unit_id:
            source_unit_id = str(get_entity_id(source_root) or "")
        sr["enhancement_target_augury_web_source_unit_id"] = source_unit_id
        target_root.special_rules = sr
        try:
            append_action(
                player,
                f"{sr['enhancement_target_augury_web_source']}: {getattr(target_root, 'name', 'Unit')} gains [LETHAL HITS] until your next Command phase.",
            )
        except Exception:
            pass
        return True

    def _apply_space_marines_master_of_machine_war_effect(
        self,
        *,
        source_unit=None,
        target_unit=None,
        player=None,
        ability_name: str = "Master of Machine War",
    ) -> bool:
        if source_unit is None or target_unit is None:
            return False
        from ...utility.entity_ids import get_entity_id
        ...
        if source_root is None or target_root is None:
            return False
        source_sr = getattr(source_unit, "special_rules", None)
        if not isinstance(source_sr, dict) or not bool(source_sr.get("enhancement_master_of_machine_war")):
            return False
        try:
            if source_root.get_parent_army() is not target_root.get_parent_army():
                return False
        ...
        if not isinstance(sr, dict):
            sr = {}
        sr["enhancement_master_of_machine_war_active"] = True
        sr["enhancement_master_of_machine_war_owner"] = owner_id
        sr["enhancement_master_of_machine_war_source"] = str(ability_name or "Master of Machine War").strip() or "Master of Machine War"
        source_unit_id = str(get_entity_id(source_unit) or "")
        if not source_unit_id:
            source_unit_id = str(get_entity_id(source_root) or "")
        sr["enhancement_master_of_machine_war_source_unit_id"] = source_unit_id
        target_root.special_rules = sr
        try:
            append_action(
                player,
                f"{sr['enhancement_master_of_machine_war_source']}: {getattr(target_root, 'name', 'Unit')} can shoot after Advancing or Falling Back until your next Command phase.",
            )
        except Exception:
            pass
        return True
    ...
    def _on_phase_end_cleanup(self, player=None, phase=None, **_kwargs) -> None:
        ...
        for p in list(self.players or []):
            ...
            for u in list(army.units):
                ...
                if exp and exp == pname:
                    for k in (
                        "accelerator_mandible_ws_bonus_active",
                        "accelerator_mandible_ws_bonus",
                        "accelerator_mandible_ws_bonus_owner",
                        "accelerator_mandible_ws_bonus_turn",
                        "accelerator_mandible_ws_bonus_source",
                        "accelerator_mandible_ws_bonus_expires_phase",
                    ):
                        sr.pop(k, None)
                ...
                if pname == "FIGHT_PHASE" and active_name:
                    ...
                    try:
                        turn = int(sr.get("a_foot_in_the_future_no_charge_turn", 0) or 0)
                    except Exception:
                        turn = 0
                    if owner and owner == active_name:
                        if int(turn or 0) == int(getattr(self, "turn", 0) or 0):
                            for k in (
                                "a_foot_in_the_future_no_charge_turn_owner",
