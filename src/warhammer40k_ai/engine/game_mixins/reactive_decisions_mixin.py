...
class GameReactiveDecisionsMixin:
    ...
    def _queue_movement_phase_advance_redeploy(
        self,
        *,
        player,
        unit,
        ...
        try:
            specs = list(unit.unit_movement_phase_advance_redeploy_specs() or [])
        except Exception:
            specs = []
        if not specs:
            return None
        ...
        for spec in specs:
            ...
            message = (
                ...
                f"instead of making an Advance move?"
            )
            return self._queue_optional_ability_confirmation(
                player=player,
    ...
    def _queue_start_phase_select_enemy_battleshock(
        self,
        *,
        player,
        source_unit,
    ...
    def _queue_start_fight_phase_select_enemy_melee_hit_penalty(
        self,
        *,
        player,
        source_unit,
        ...
        ability_name = str(spec.get("source", "") or "Fight phase enemy hit penalty").strip()
        ability_name = ability_name or "Fight phase enemy hit penalty"
        ability_context = str(
            spec.get("context_ability", "") or spec.get("ability", "") or "fight_phase_select_enemy_melee_hit_penalty"
        ).strip().lower()
        if not ability_context:
            ability_context = "fight_phase_select_enemy_melee_hit_penalty"
        ability_key = str(spec.get("ability_key", "") or "").strip().lower()
        if not ability_key:
            source_key = re.sub(r"[^a-z0-9]+", "_", ability_name.lower()).strip("_")
            if not source_key:
        ...
        try:
            hit_penalty = int(spec.get("hit_penalty", 1) or 1)
        except (TypeError, ValueError):
            hit_penalty = 1
        ...
        phase_label = phase_key.replace("_", " ").title()
        none_clause = " (or None)" if optional else ""
        request = DecisionRequest.create(
            DECISION_CHOOSE_QUARRY,
            (
                f"{ability_name}: select one enemy unit within Engagement Range{none_clause}. "
                f"Until end of {phase_label}, that unit suffers -{int(hit_penalty)} to Hit rolls for melee attacks."
            ),
            player_id=getattr(player, "id", None),
            options=options,
            context=ctx,
    ...
    def _maybe_queue_reactive_move_followup(self, request: DecisionRequest, result: DecisionResult) -> None:
        ...
        if decision_type == DECISION_CONFIRM_YES_NO:
            ...
            if kind == "unhinged_vengeance":
                ...
                return
            if kind == "blistering_assault":
                if not unit.can_blistering_assault(game=self, game_map=getattr(self, "map", None)):
                    return
                max_distance = int(self.roll_blistering_assault_distance(unit) or 0)
                if max_distance <= 0:
                    return
                attacker_unit_id = str(ctx.get("reactive_move_attacker_unit_id") or "")
                attacker_unit = self._resolve_unit_by_id(attacker_unit_id)
    ...
    def _maybe_apply_optional_ability_confirmation(self, request: DecisionRequest, result: DecisionResult) -> None:
        ...
        if ability_key not in (
            ...
            "deeds_of_heroism",
            "alchemicus_familiar",
            "fight_selected_target_keyword_melee_weapon_keyword",
        ...
        if ability_key == "aeldari_strength_from_death_lethal_surge":
            ...
            return
        if ability_key == "advance_redeploy":
            unit_id = str(payload.get("unit_id") or ctx.get("unit_id") or "")
            if not unit_id:
                return
            unit = self._resolve_unit_by_id(unit_id)
            if unit is None or not getattr(unit, "is_alive", lambda: False)():
                return
            if not choice:
                prepare = getattr(unit, "prepare_advance", None)
                if callable(prepare):
                    prepare()
                return
            try:
            ...
            if queue is not None and hasattr(queue, "list"):
                for req in list(queue.list() or []):
                    ...
                    if str(req_ctx.get("placement_kind", "") or "") != "advance_redeploy_9h":
                        continue
                    if str(req_ctx.get("unit_id", "") or "") != root_id:
                        continue
                    return

            ability_name = str(ctx.get("ability_name", "") or "Advance redeploy").strip() or "Advance redeploy"
            request_move = DecisionRequest.create(
                ...
                options=[
                    DecisionOption.create(
                        "Confirm",
                        payload={"unit_id": root_id, "movement_type": "advance", "action": "confirm"},
                    )
                ],
                context={
                    "unit_id": root_id,
        ...
        if ability_key == "student_of_the_codex":
            unit_id = str(payload.get("unit_id") or ctx.get("unit_id") or "")
            if not unit_id:
                return
            unit = self._resolve_unit_by_id(unit_id)
            ...
            if source_member is None:
                find_source = getattr(self, "_attached_member_with_enhancement_flag", None)
                if callable(find_source):
                    _root, source_member, _source_sr = find_source(root, "enhancement_student_of_the_codex")
            if source_member is None:
                return
            sr = getattr(source_member, "special_rules", None)
            if not isinstance(sr, dict) or not bool(sr.get("enhancement_student_of_the_codex", False)):
                return
            bearer = getattr(source_member, "_get_enhancement_bearer_model", lambda: None)()
            if bearer is None:
                return
            alive = getattr(bearer, "is_alive", True)
            if not bool(alive() if callable(alive) else alive):
                return
            bearer_model_id = str(payload.get("model_id") or ctx.get("model_id") or "")
            if bearer_model_id:
                model_id = str(get_entity_id(bearer) or "")
                if model_id and model_id != bearer_model_id:
                    return
            doctrine = str(
                payload.get("doctrine")
                or ctx.get("doctrine")
                or sr.get("enhancement_student_of_the_codex_doctrine")
                or "TACTICAL"
            ).strip().upper()
            if doctrine not in {"DEVASTATOR", "TACTICAL", "ASSAULT"}:
                doctrine = "TACTICAL"
            try:
                current_round = int(getattr(self, "turn", 0) or 0)
            except (TypeError, ValueError):
            ...
            sr["enhancement_student_of_the_codex_active"] = True
            sr["enhancement_student_of_the_codex_owner_id"] = owner_id
            sr["enhancement_student_of_the_codex_turn_started"] = int(current_round)
            sr["enhancement_student_of_the_codex_expires_round"] = int(expires_round)
            sr["enhancement_student_of_the_codex_doctrine"] = doctrine
            source_member.special_rules = sr
            return
        ...
        if ability_key == "fight_selected_target_keyword_melee_weapon_keyword":
            if choice is False:
                return
            unit_id = str(payload.get("unit_id") or ctx.get("unit_id") or "")
            model_id = str(payload.get("model_id") or ctx.get("model_id") or "")
            ...
            if getattr(model, "parent_unit", None) not in members:
                return
            buff_key = str(
                payload.get("buff_key") or ctx.get("buff_key") or "fight_selected_target_keyword_melee_weapon_keyword"
            ).strip().lower()
            if not buff_key:
                return
            if bool(getattr(model, "has_used_once_per_battle", lambda _k: False)(buff_key)):
                return
            ability_name = str(
                payload.get("ability_name") or ctx.get("ability_name") or "Fight-selected melee weapon keyword bonus"
            ).strip()
            ability_name = ability_name or "Fight-selected melee weapon keyword bonus"
            keywords = [
                str(value or "").strip().upper()
                for value in list(payload.get("keywords") or ctx.get("keywords") or [])
                if str(value or "").strip()
            ]
            if not keywords:
                return
            ...
            for idx, weapon_name in enumerate(weapon_names):
                set_fn(
                    key=f"{buff_key}:{get_entity_id(model)}:{idx}",
                    weapon_name=weapon_name,
            ...
            try:
                ...
                if player is not None:
                    append_action(
                        player,
                        f"{ability_name}: {getattr(model, 'name', 'Model')} gains [{'] ['.join(keywords)}] on melee weapons this phase.",
                    )
            except (ImportError, AttributeError, TypeError, ValueError):
                pass
            return
    ...
    def _issue_bodyguard_loss_request(
        self,
        *,
        leader_unit=None,
        bodyguard_unit=None,
        ...
        if not candidates:
            return None, None
        sorted_candidates = [model for model in list(candidates or []) if model is not None]
        sorted_candidates.sort(key=lambda model: str(maybe_entity_id(model) or ""))
        if len(sorted_candidates) == 1:
            chosen_model = sorted_candidates[0]
        ...
        if not leader_id or not bodyguard_id:
            return None, None
        options = []
        for model in list(sorted_candidates or []):
            model_id = maybe_entity_id(model)
            if not model_id:
                continue
            options.append(
    ...
    def resolve_bodyguard_loss_immediately(
        ...
        *,
        ...
        from ...utility.decision_utils import decision_request_is_pending, resolve_or_reuse_decision_value

        chosen_model, request = self._issue_bodyguard_loss_request(
            leader_unit=leader_unit,
            bodyguard_unit=bodyguard_unit,
            ability_name=ability_name,
            player=player,
            leader_unit_id=leader_unit_id,
        )
        if chosen_model is not None or request is None:
            return chosen_model
        resolved_player = player
        if resolved_player is None:
            resolved_player = self._resolve_player_by_id(getattr(request, "player_id", None))
        ...
        if decision_request_is_pending(self, request):
            ...
            try:
                ...
                if resolved_player is not None and callable(getattr(resolved_player, "has_control", None)) and resolved_player.has_control() and callable(provider):
                    provider_choice = provider(
                        ...
                        leader_unit=leader_unit,
                        bodyguard_unit=bodyguard_unit,
                        candidates=[self._resolve_model_by_id(str((opt.payload or {}).get("model_id") or "")) for opt in list(getattr(request, "options", []) or [])],
                        ability_name=str((getattr(request, "context", {}) or {}).get("ability_name", "") or ability_name or "Bodyguard Loss"),
                    )
            except Exception:
                provider_choice = None
            selected_id = str(maybe_entity_id(provider_choice) or provider_choice or "").strip()
            if not selected_id and resolved_player is not None:
                choose_value = getattr(resolved_player, "_choose_optional_value", None)
                if callable(choose_value):
                    selected = choose_value(
                        str(selection_key or "BODYGUARD_LOSS"),
                        [
                            str((getattr(option, "payload", {}) or {}).get("model_id") or "")
                            for option in list(getattr(request, "options", []) or [])
                        ],
                        {
                            "ability_name": str((getattr(request, "context", {}) or {}).get("ability_name", "") or ""),
                            "bodyguard_unit_id": str((getattr(request, "context", {}) or {}).get("bodyguard_unit_id", "") or ""),
                            "leader_unit_id": str((getattr(request, "context", {}) or {}).get("leader_unit_id", "") or ""),
                        },
                    )
            ...
            if selected_id:
                for option in list(getattr(request, "options", []) or []):
                    ...
                    if str(payload.get("model_id", "") or "") == selected_id:
                        choice_option_id = str(getattr(option, "option_id", "") or "")
                        break
        else:
            options = list(getattr(request, "options", []) or [])
            if options:
                choice_option_id = str(getattr(options[0], "option_id", "") or "")
        if not choice_option_id:
            return None
        value, apply_result = resolve_or_reuse_decision_value(
            self,
            request,
            choice_option_id,
            player_id=getattr(resolved_player, "id", None),
        )
    ...
    def queue_bodyguard_loss(
        ...
        *,
        ...
        self._issue_bodyguard_loss_request(
            leader_unit=leader_unit,
            bodyguard_unit=bodyguard_unit,
            ability_name=ability_name,
            player=player,
