...
class GameSetupDeploymentReservesMixin:
    ...
    def _reserves_denial_ranges_for_unit(self, unit) -> list[dict]:
        ...
        circle_denial_fn = (
            getattr(gk_mgr, "banishers_circle_of_sanctuary_reserves_denial", None)
            if gk_mgr is not None
            else None
        )
        if callable(circle_denial_fn):
            entry = circle_denial_fn(root, game=self)
            if isinstance(entry, dict):
                try:
                    denial_range = float(entry.get("range", 0.0) or 0.0)
                except (TypeError, ValueError):
    ...
    def can_place_unit_arriving_from_reserves(self, unit: 'Unit', position: Tuple[float, float, float],
        ...
        from ..reserve_entry_rules import can_place_unit_arriving_from_reserves
    ...
    def get_distance_to_battlefield_edge(self, position: Tuple[float, float, float], 
        ...
        from ..reserve_entry_geometry import distance_to_battlefield_edge

        return float(distance_to_battlefield_edge(position, battlefield_edge, game=self))
    ...
    def execute_muster_armies_phase(
        ...
        if len(self.players) >= 2:
            ...
            for player in list(self.players[:2]):
                ...
                queue_steel_hammer_fn = getattr(
                    am_mgr,
                    "queue_steel_hammer_titanic_character_selection_request",
                    None,
