...
def next_phase(game: "Game") -> None:
    ...
    if next_phase_value == 0:  # If we've wrapped around to COMMAND_PHASE
        ...
        # This means we've finished all phases for the current player. Switch to the next player
        # before publishing the new Command phase start.
        ...
        if game.battle_round_starting_player_index is None:
            game.battle_round_starting_player_index = (game.current_player_index - 1) % len(players)

        completed_battle_round = game.current_player_index == game.battle_round_starting_player_index
        if completed_battle_round:
            try:
