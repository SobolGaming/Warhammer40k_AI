...
class SpaceMarinesDetachmentManager(DetachmentManagerBase):
    ...
    @staticmethod
    def _vanguard_strike_from_the_shadows_keys() -> tuple[str, ...]:
        return (
            "space_marines_vanguard_strike_from_the_shadows_active",
            "space_marines_vanguard_strike_from_the_shadows_turn",
            "space_marines_vanguard_strike_from_the_shadows_expires_phase",
            "space_marines_vanguard_strike_from_the_shadows_source",
            "space_marines_vanguard_strike_from_the_shadows_player_id",
            "space_marines_vanguard_strike_from_the_shadows_selected_target_ids",
            "space_marines_vanguard_strike_from_the_shadows_qualified_target_ids",
        )

    def clear_vanguard_strike_from_the_shadows(self, unit) -> None:
        root = self._attached_unit_root(unit)
        if root is None:
            return
        sr = getattr(root, "special_rules", None)
        if not isinstance(sr, dict):
            return
        for key in self._vanguard_strike_from_the_shadows_keys():
            sr.pop(key, None)
        root.special_rules = sr

    def set_vanguard_strike_from_the_shadows(
        self,
        unit,
        *,
        battle_round=None,
        player_id: str = "",
        source: str = "Strike from the Shadows",
    ) -> bool:
        if not self.is_vanguard_spearhead():
            return False
        root = self._attached_unit_root(unit)
        ...
        if not isinstance(sr, dict):
            sr = {}
        sr["space_marines_vanguard_strike_from_the_shadows_active"] = True
        sr["space_marines_vanguard_strike_from_the_shadows_expires_phase"] = "SHOOTING_PHASE"
        sr["space_marines_vanguard_strike_from_the_shadows_turn"] = int(battle_round or 0)
        sr["space_marines_vanguard_strike_from_the_shadows_player_id"] = str(player_id or "").strip()
        sr["space_marines_vanguard_strike_from_the_shadows_selected_target_ids"] = []
        sr["space_marines_vanguard_strike_from_the_shadows_qualified_target_ids"] = []
        sr["space_marines_vanguard_strike_from_the_shadows_source"] = (
            str(source or "Strike from the Shadows").strip() or "Strike from the Shadows"
        )
        root.special_rules = sr
        return True

    def _vanguard_strike_from_the_shadows_bonus(
        self,
        attacker_model,
        target_unit=None,
        *,
        ...
        if root is None or not self.attached_unit_is_adeptus_astartes(root):
            return 0, ""
        sr = getattr(root, "special_rules", None)
        if not isinstance(sr, dict) or not bool(sr.get("space_marines_vanguard_strike_from_the_shadows_active")):
            return 0, ""
        ...
        current_phase = str(getattr(getattr(game_obj, "phase", None), "name", "") or "").strip().upper().replace(" ", "_")
        expected_phase = str(
            sr.get("space_marines_vanguard_strike_from_the_shadows_expires_phase", "") or ""
        ).strip().upper()
        if expected_phase and current_phase and current_phase != expected_phase:
            self.clear_vanguard_strike_from_the_shadows(root)
            return 0, ""
        current_turn = int(getattr(game_obj, "turn", 0) or 0)
        marked_turn = int(sr.get("space_marines_vanguard_strike_from_the_shadows_turn", 0) or 0)
        if marked_turn and current_turn and marked_turn != current_turn:
            self.clear_vanguard_strike_from_the_shadows(root)
            return 0, ""
        ...
        if target_id:
            qualified_ids = [
                str(value or "").strip()
                for value in list(sr.get("space_marines_vanguard_strike_from_the_shadows_qualified_target_ids", []) or [])
                if str(value or "").strip()
            ]
            if target_id not in qualified_ids:
                qualified_ids.append(target_id)
                sr["space_marines_vanguard_strike_from_the_shadows_qualified_target_ids"] = qualified_ids
                root.special_rules = sr
        source = str(
            sr.get("space_marines_vanguard_strike_from_the_shadows_source", "") or "Strike from the Shadows"
        ).strip() or "Strike from the Shadows"
        return 1, source

    def vanguard_strike_from_the_shadows_ranged_skill_bonus(
        self,
        attacker_model,
        target_unit=None,
        *,
        ...
        return self._vanguard_strike_from_the_shadows_bonus(
            attacker_model,
            target_unit,
    ...
    def vanguard_strike_from_the_shadows_ranged_ap_bonus(
        self,
        attacker_model,
        target_unit=None,
        *,
