...
class NecronsStratagemMixin:
    ...
    def _necrons_current_turn_owner_id(self) -> str:
    ...
    def _is_pantheon_of_woe(self) -> bool:
        mgr = self._get_necrons_mgr()
        if mgr is None:
            return False
        return bool(mgr.is_pantheon_of_woe())
    ...
    def _pantheon_turn_key(self) -> str:
        return f"{int(self._necrons_current_turn())}:{self._necrons_current_turn_owner_id()}"

    def _pantheon_battle_round_key(self) -> str:
        return str(int(self._necrons_current_turn()) or 0)

    def _pantheon_mass_transmogrification_used_turn_keys(self) -> set[str]:
        used = getattr(self, "_pantheon_mass_transmogrification_used_turn_keys_set", None)
        if not isinstance(used, set):
            used = set()
            self._pantheon_mass_transmogrification_used_turn_keys_set = used
        return used

    def _pantheon_mass_transmogrification_used_this_turn(self) -> bool:
        key = self._pantheon_turn_key()
        return bool(key) and key in self._pantheon_mass_transmogrification_used_turn_keys()

    def _mark_pantheon_mass_transmogrification_used(self) -> None:
        key = self._pantheon_turn_key()
        if key:
            self._pantheon_mass_transmogrification_used_turn_keys().add(key)

    def _pantheon_molecular_erosion_used_round_keys(self) -> set[str]:
        used = getattr(self, "_pantheon_molecular_erosion_used_round_keys_set", None)
        if not isinstance(used, set):
            used = set()
            self._pantheon_molecular_erosion_used_round_keys_set = used
        return used

    def _pantheon_molecular_erosion_used_this_battle_round(self) -> bool:
        key = self._pantheon_battle_round_key()
        return bool(key) and key in self._pantheon_molecular_erosion_used_round_keys()

    def _mark_pantheon_molecular_erosion_used(self) -> None:
        key = self._pantheon_battle_round_key()
        if key:
            self._pantheon_molecular_erosion_used_round_keys().add(key)

    def _pantheon_unit_eligible(
        self,
        unit: Any,
        *,
        ...
        if not self._is_pantheon_of_woe():
            return False
        root = self._necrons_root(unit)
        if root is None:
            return False
        ...
        if not bool(mgr.unit_is_necrons(root)):
            return False
        if require_monster and not self._necrons_unit_contains_keyword(root, "MONSTER"):
            return False
        if exclude_monster and self._necrons_unit_contains_keyword(root, "MONSTER"):
            return False
        round_state = getattr(root, "round_state", None)
        if require_not_shot and bool(getattr(round_state, "shot_this_round", False)):
            return False
    ...
    def _pantheon_monster_candidates(self, *, require_visible_unravelling_enemy: bool = False) -> list[Any]:
        if not self._is_pantheon_of_woe():
            return []
        army = self.player.get_army()
        units = list(getattr(army, "units", []) or []) if army is not None else []
        ...
        for unit in units:
            ...
            if not unit_id or unit_id in seen:
                continue
            seen.add(unit_id)
            if not self._pantheon_unit_eligible(root, require_monster=True):
                continue
            if require_visible_unravelling_enemy and not self._pantheon_visible_unravelling_enemy_candidates(root):
                continue
            results.append(root)
        results.sort(key=lambda unit_obj: str(get_entity_id(unit_obj) or ""))
        return results

    def _pantheon_non_monster_candidates(
        self,
        *,
        require_not_shot: bool = False,
        require_not_fought: bool = False,
        require_reanimation: bool = False,
        within_unit: Any = None,
        range_inches: float = 0.0,
    ) -> list[Any]:
        if not self._is_pantheon_of_woe():
            return []
        army = self.player.get_army()
        units = list(getattr(army, "units", []) or []) if army is not None else []
        anchor_root = self._necrons_root(within_unit)
        results: list[Any] = []
        seen: set[str] = set()
        for unit in units:
            root = self._necrons_root(unit)
            ...
            if not unit_id or unit_id in seen:
                continue
            seen.add(unit_id)
            if not self._pantheon_unit_eligible(
                root,
                ...
                continue
            if anchor_root is not None and float(range_inches or 0.0) > 0.0:
                if not self._necrons_units_within_distance(anchor_root, root, range_inches=float(range_inches)):
                    continue
            results.append(root)
        results.sort(key=lambda unit_obj: str(get_entity_id(unit_obj) or ""))
        return results

    def _pantheon_visible_unravelling_enemy_candidates(self, source_unit: Any) -> list[Any]:
        source_root = self._necrons_root(source_unit)
        if source_root is None or not self._is_pantheon_of_woe():
            return []
        mgr = self._get_necrons_mgr()
        game_map = getattr(self.game, "map", None) if self.game is not None else None
        if mgr is None or game_map is None:
            return []
        los_checker = getattr(source_root, "_attacking_unit_has_any_los_to_target_unit", None)
        if not callable(los_checker):
            return []
        results: list[Any] = []
        seen: set[str] = set()
        for unit in list(getattr(game_map, "units", []) or []):
            ...
            if not bool(getattr(enemy_root, "deployed", False)):
                continue
            if not bool(mgr.pantheon_unit_is_unravelling(enemy_root, game=self.game)):
                continue
            if not bool(los_checker(enemy_root, game_map)):
                continue
            results.append(enemy_root)
        results.sort(key=lambda unit_obj: str(get_entity_id(unit_obj) or ""))
        return results

    def _pantheon_phase_melding_candidates(self, enemy_unit: Any) -> list[Any]:
        enemy_root = self._necrons_root(enemy_unit)
        if enemy_root is None or not self._is_pantheon_of_woe():
            return []
        game_map = getattr(self.game, "map", None) if self.game is not None else None
        army = self.player.get_army()
        units = list(getattr(army, "units", []) or []) if army is not None else []
        ...
        for unit in units:
            ...
            if not unit_id or unit_id in seen:
                continue
            seen.add(unit_id)
            if not self._pantheon_unit_eligible(root):
                continue
            engaged = False
            if game_map is not None and callable(getattr(game_map, "is_within_engagement_range", None)):
                engaged = bool(game_map.is_within_engagement_range(root, enemy_root))
            if not engaged:
                engaged = self._necrons_units_within_distance(root, enemy_root, range_inches=1.0)
            if not engaged:
                continue
            results.append(root)
        results.sort(key=lambda unit_obj: str(get_entity_id(unit_obj) or ""))
    ...
    def _pantheon_unit_in_candidates(self, unit: Any, candidates: list[Any]) -> bool:
        root = self._necrons_root(unit)
        if root is None:
            return False
        unit_id = str(get_entity_id(root) or "")
    ...
    def _pantheon_reaction_already_queued(
        self,
        *,
        event_name: str,
        stratagem_name: str,
        ...
        for reaction in list(self._pending_reactions or []):
            ...
            if enemy_root is not None:
                reaction_enemy = self._necrons_root(
                    reaction.get("enemy_unit") or reaction.get("attacking_unit") or reaction.get("moving_unit")
                )
                if reaction_enemy is not enemy_root:
                    continue
            if source_root is not None:
                reaction_source = self._necrons_root(reaction.get("source_unit"))
                if reaction_source is not source_root:
                    continue
            if destroyed_model_id and str(reaction.get("destroyed_model_id", "") or "") != destroyed_model_id:
                continue
            return True
        return False

    def _queue_pantheon_phase_start_stratagem(
        self,
        *,
        stratagem_name: str,
        phase_name: str,
        ...
        if str(getattr(stratagem, "name", "") or "").strip().upper() in self._used_stratagems_this_phase:
            return
        if self._pantheon_reaction_already_queued(
    ...
    def _queue_pantheon_phase_start_reactions(self, *, player: Any, phase: Any) -> None:
        if not self._is_pantheon_of_woe():
            return
        mgr = self._get_necrons_mgr()
        snapshot_fn = getattr(mgr, "snapshot_pantheon_unravelling_phase_state", None) if mgr is not None else None
        if callable(snapshot_fn):
            snapshot_fn(game=self.game)
        phase_key = str(getattr(phase, "name", "") or "").strip().upper()
        active_player = getattr(self.game, "get_current_player", lambda: None)() if self.game is not None else None
        if phase_key == "COMMAND_PHASE":
            ...
            enemy_candidates_by_unit = {
                str(get_entity_id(root) or ""): list(self._pantheon_visible_unravelling_enemy_candidates(root))
                for root in list(candidates or [])
            }
            self._queue_pantheon_phase_start_stratagem(
                stratagem_name="MOLECULAR EROSION",
                phase_name="Command phase",
                candidates=candidates,
                extra_payload={"enemy_candidates_by_unit": enemy_candidates_by_unit},
            )
            return
        if phase_key == "SHOOTING_PHASE" and player is self.player and active_player is self.player:
            self._queue_pantheon_phase_start_stratagem(
                stratagem_name="ENTROPHASIC AURA TARGETING",
                phase_name="Shooting phase",
                candidates=self._pantheon_non_monster_candidates(require_not_shot=True),
            )
            return
        if phase_key == "FIGHT_PHASE":
            self._queue_pantheon_phase_start_stratagem(
                stratagem_name="ENTROPHASIC AURA TARGETING",
                phase_name="Fight phase",
                candidates=self._pantheon_non_monster_candidates(require_not_fought=True),
            )

    def _queue_pantheon_fight_targets_selected_reactions(
        self,
        *,
        attacking_unit: Any,
        target_units: list[Any],
    ) -> None:
        if self.game is None or attacking_unit is None or not self._is_pantheon_of_woe():
            return
        if str(self._current_phase_name or "").strip().lower() != "fight phase":
            return
        attacker_root = self._necrons_root(attacking_unit)
        if attacker_root is None or self._necrons_owned_by_player(attacker_root):
            return
        stratagem = self.get_by_name("CHRONODISTORTION")
        if stratagem is None:
            return
        if int(getattr(self.player, "command_points", 0) or 0) < int(getattr(stratagem, "cp_cost", 0) or 0):
            return
        ...
        for unit in list(target_units or []):
            ...
            if not unit_id or unit_id in seen:
                continue
            seen.add(unit_id)
            if root is None or not self._pantheon_unit_eligible(root):
                continue
    ...
    def _queue_pantheon_move_started_reactions(self, *, unit: Any, action: str) -> None:
        if self.game is None or unit is None or not self._is_pantheon_of_woe():
            return
        ...
        enemy_root = self._necrons_root(unit)
        mgr = self._get_necrons_mgr()
        if enemy_root is None or self._necrons_owned_by_player(enemy_root) or mgr is None:
            return
        if not bool(mgr.pantheon_unit_is_unravelling(enemy_root, game=self.game)):
            return
        stratagem = self.get_by_name("PHASE MELDING")
        if stratagem is None:
            return
        if int(getattr(self.player, "command_points", 0) or 0) < int(getattr(stratagem, "cp_cost", 0) or 0):
            return
        if str(getattr(stratagem, "name", "") or "").strip().upper() in self._used_stratagems_this_phase:
            return
        candidates = self._pantheon_phase_melding_candidates(enemy_root)
        if not candidates:
            return
        if self._pantheon_reaction_already_queued(
    ...
    def _queue_pantheon_unit_destroyed_reactions(self, *, destroyed_unit: Any, destroyed_by_unit: Any) -> None:
        if self.game is None or destroyed_unit is None or destroyed_by_unit is None or not self._is_pantheon_of_woe():
            return
        phase_name = str(self._current_phase_name or "").strip().lower()
        if phase_name not in {"shooting phase", "fight phase"}:
            return
        ...
        mgr = self._get_necrons_mgr()
        destroyed_root = self._necrons_root(destroyed_unit)
        source_root = self._necrons_root(destroyed_by_unit)
        if destroyed_root is None or source_root is None or mgr is None:
            return
        if self._necrons_owned_by_player(destroyed_root) or not self._necrons_owned_by_player(source_root):
            return
        if not self._pantheon_unit_eligible(source_root, require_monster=True):
            return
        if not bool(mgr.pantheon_unit_was_unravelling_at_phase_start(destroyed_root, game=self.game)):
            return
        stratagem = self.get_by_name("MASS TRANSMOGRIFICATION")
        if stratagem is None:
            return
        if int(getattr(self.player, "command_points", 0) or 0) < int(getattr(stratagem, "cp_cost", 0) or 0):
            return
    ...
    def _queue_pantheon_model_destroyed_reactions(self, *, unit: Any, model: Any) -> None:
        if self.game is None or unit is None or model is None or not self._is_pantheon_of_woe():
            return
        mgr = self._get_necrons_mgr()
        root = self._necrons_root(unit)
        if root is None or mgr is None:
        ...
        if not self._necrons_owned_by_player(root):
            return
        if not bool(mgr.unit_is_necrons(root)) or not self._necrons_unit_contains_keyword(root, "MONSTER"):
            return
        ...
        if not (isinstance(deadly_demise, tuple) and bool(deadly_demise[0])):
            return
        model_alive_attr = getattr(model, "is_alive", None)
        if callable(model_alive_attr) and bool(model_alive_attr()):
            return
        stratagem = self.get_by_name("DISHARMONISATION CASCADE")
        if stratagem is None:
            return
        if int(getattr(self.player, "command_points", 0) or 0) < int(getattr(stratagem, "cp_cost", 0) or 0):
            return
        if str(getattr(stratagem, "name", "") or "").strip().upper() in self._used_stratagems_this_phase:
            return
        phase_label = str(self._current_phase_name or "").strip() or "Any phase"
        if self._pantheon_reaction_already_queued(
    ...
    def _cleanup_pantheon_phase_end_effects(self, *, phase: Any) -> None:
        if not self._is_pantheon_of_woe():
            return
        phase_name = str(getattr(phase, "name", "") or "").strip().upper()
        if phase_name not in {"MOVEMENT_PHASE", "SHOOTING_PHASE", "FIGHT_PHASE"}:
            return
        army = self.player.get_army()
        units = list(getattr(army, "units", []) or []) if army is not None else []
        seen: set[str] = set()
        for unit in units:
            ...
            changed = False
            if phase_name in {"SHOOTING_PHASE", "FIGHT_PHASE"}:
                expires_phase = str(special_rules.get("pantheon_entrophasic_aura_targeting_expires_phase", "") or "").strip().upper()
                if bool(special_rules.get("pantheon_entrophasic_aura_targeting_active")) and (not expires_phase or expires_phase == phase_name):
                    for key in list(special_rules.keys()):
                        if key.startswith("pantheon_entrophasic_aura_targeting_"):
                            special_rules.pop(key, None)
                            changed = True
            if phase_name == "FIGHT_PHASE":
                expires_phase = str(special_rules.get("pantheon_chronodistortion_expires_phase", "") or "").strip().upper()
                if bool(special_rules.get("pantheon_chronodistortion_active")) and (not expires_phase or expires_phase == phase_name):
                    for key in list(special_rules.keys()):
                        if key.startswith("pantheon_chronodistortion_"):
                            special_rules.pop(key, None)
                            changed = True
            if phase_name == "MOVEMENT_PHASE" and bool(special_rules.get("enemy_fallback_desperate_escape")):
                source_name = str(special_rules.get("enemy_fallback_desperate_escape_source", "") or "").strip().upper()
                expires_phase = str(special_rules.get("enemy_fallback_desperate_escape_expires_phase", "") or "").strip().upper()
                if source_name == "PHASE MELDING" and (not expires_phase or expires_phase == phase_name):
                    for key in (
    ...
    def _use_pantheon_chronodistortion(self, stratagem: Any, **kwargs) -> bool:
        unit = kwargs.get("unit") or kwargs.get("target_unit")
        pending = self._pantheon_pending_context("CHRONODISTORTION", unit=unit)
        attacking_unit = kwargs.get("attacking_unit") or kwargs.get("enemy_unit")
        target_units = list(kwargs.get("target_units") or [])
        candidates = list(kwargs.get("candidates") or [])
        if isinstance(pending, dict):
        ...
        if unit is None and len(candidates) == 1:
            unit = candidates[0]
        if unit is None:
            logger.error("ERROR: CHRONODISTORTION: no target unit provided")
            return False
        root = self._necrons_root(unit)
        attacker_root = self._necrons_root(attacking_unit)
        if root is None or attacker_root is None or not self._is_pantheon_of_woe():
            return False
        phase_name = str(kwargs.get("phase_name") or self._current_phase_name or "").strip().lower().replace("_", " ")
        if phase_name != "fight phase":
            logger.error("ERROR: CHRONODISTORTION: wrong phase")
            return False
        if self._necrons_owned_by_player(attacker_root):
            logger.error("ERROR: CHRONODISTORTION: trigger requires an enemy attacking unit")
            return False
        if not candidates:
            seen: set[str] = set()
            for target in list(target_units or []):
                ...
                if not target_id or target_id in seen:
                    continue
                seen.add(target_id)
                if self._pantheon_unit_eligible(target_root):
                    candidates.append(target_root)
        if not self._pantheon_unit_in_candidates(root, candidates):
            logger.error("ERROR: CHRONODISTORTION: target must be a friendly NECRONS unit selected by that attacker")
            return False
        if not stratagem.can_use(
            self.player,
            self.game,
            ...
            logger.error("ERROR: CHRONODISTORTION: cannot be used in current state")
            return False
        if not self._necrons_spend_cp(stratagem, target_unit=root):
            return False
        source = str(getattr(stratagem, "name", "") or "CHRONODISTORTION").strip() or "CHRONODISTORTION"
        special_rules = dict(getattr(root, "special_rules", None) or {})
        special_rules["pantheon_chronodistortion_active"] = True
        special_rules["pantheon_chronodistortion_threshold"] = 4
        special_rules["pantheon_chronodistortion_bonus_if_attacker_unravelling"] = 1
        special_rules["pantheon_chronodistortion_expires_phase"] = "FIGHT_PHASE"
        special_rules["pantheon_chronodistortion_turn"] = int(self._necrons_current_turn())
        special_rules["pantheon_chronodistortion_source"] = source
        root.special_rules = special_rules
        self._necrons_finalize_stratagem_use(stratagem, dequeue=kwargs.get("dequeue") is True)
        logger.info(
            "INFO: CHRONODISTORTION: %s can fight on death this phase, improving from 4+ to 3+ against unravelling attackers.",
            getattr(root, "name", "Unit"),
        )
        return True

    def _use_pantheon_disharmonisation_cascade(self, stratagem: Any, **kwargs) -> bool:
        unit = kwargs.get("unit") or kwargs.get("target_unit") or kwargs.get("destroyed_unit")
        destroyed_model = kwargs.get("destroyed_model") or kwargs.get("target_model") or kwargs.get("model")
        pending = self._pantheon_pending_context("DISHARMONISATION CASCADE", unit=unit, destroyed_model=destroyed_model)
        if isinstance(pending, dict):
            if unit is None:
                unit = pending.get("unit") or pending.get("target_unit") or pending.get("destroyed_unit")
            if destroyed_model is None:
                destroyed_model = pending.get("destroyed_model")
        root = self._necrons_root(unit)
        if root is None or destroyed_model is None or not self._is_pantheon_of_woe():
            return False
        if not self._necrons_owned_by_player(root):
            logger.error("ERROR: DISHARMONISATION CASCADE: target must be a friendly NECRONS MONSTER model")
            return False
        if not self._necrons_unit_contains_keyword(root, "MONSTER"):
            logger.error("ERROR: DISHARMONISATION CASCADE: target must be a friendly NECRONS MONSTER model")
            return False
        has_deadly_demise = getattr(root, "has_deadly_demise", None)
        deadly_demise = has_deadly_demise() if callable(has_deadly_demise) else (False, None)
        if not (isinstance(deadly_demise, tuple) and bool(deadly_demise[0])):
            logger.error("ERROR: DISHARMONISATION CASCADE: target model does not have Deadly Demise")
            return False
        model_alive_attr = getattr(destroyed_model, "is_alive", None)
        if callable(model_alive_attr) and bool(model_alive_attr()):
            logger.error("ERROR: DISHARMONISATION CASCADE: target model is not destroyed")
            return False
        ...
        if not stratagem.can_use(
            ...
            logger.error("ERROR: DISHARMONISATION CASCADE: cannot be used in current state")
            return False
        game_map = getattr(self.game, "map", None)
        if game_map is None:
            logger.error("ERROR: DISHARMONISATION CASCADE: map context unavailable")
            return False
        if not self._necrons_spend_cp(stratagem, target_unit=root):
            return False
        setattr(destroyed_model, "_pantheon_disharmonisation_trigger_threshold_once", 3)
        ...
        self._necrons_finalize_stratagem_use(stratagem, dequeue=kwargs.get("dequeue") is True)
        logger.info(
            "INFO: DISHARMONISATION CASCADE: %s now triggers Deadly Demise on 3+ this phase.",
            getattr(root, "name", "Unit"),
        )
        return True

    def _use_pantheon_entrophasic_aura_targeting(self, stratagem: Any, **kwargs) -> bool:
        unit = kwargs.get("unit") or kwargs.get("target_unit")
        pending = self._pantheon_pending_context("ENTROPHASIC AURA TARGETING", unit=unit)
        candidates = list(kwargs.get("candidates") or [])
        if isinstance(pending, dict) and not candidates:
            candidates = list(pending.get("candidates") or [])
        if unit is None and len(candidates) == 1:
            unit = candidates[0]
        if unit is None:
            logger.error("ERROR: ENTROPHASIC AURA TARGETING: no target unit provided")
            return False
        root = self._necrons_root(unit)
        if root is None or not self._is_pantheon_of_woe():
            return False
        phase_name = str(kwargs.get("phase_name") or self._current_phase_name or "").strip().lower().replace("_", " ")
        if phase_name not in {"shooting phase", "fight phase"}:
            logger.error("ERROR: ENTROPHASIC AURA TARGETING: wrong phase")
            return False
        if phase_name == "shooting phase" and getattr(self.game, "get_current_player", lambda: None)() is not self.player:
            logger.error("ERROR: ENTROPHASIC AURA TARGETING: only usable in your Shooting phase")
            return False
        valid_candidates = candidates or self._pantheon_non_monster_candidates(
            require_not_shot=phase_name == "shooting phase",
            require_not_fought=phase_name == "fight phase",
        )
        if not self._pantheon_unit_in_candidates(root, valid_candidates):
            logger.error(
                "ERROR: ENTROPHASIC AURA TARGETING: target must be a friendly non-MONSTER NECRONS unit that has not acted this phase"
            )
            return False
        phase_label = "Shooting phase" if phase_name == "shooting phase" else "Fight phase"
        if not stratagem.can_use(
            ...
            logger.error("ERROR: ENTROPHASIC AURA TARGETING: cannot be used in current state")
            return False
        if not self._necrons_spend_cp(stratagem, target_unit=root):
            return False
        current_player = getattr(self.game, "get_current_player", lambda: None)() if self.game is not None else None
        source = str(getattr(stratagem, "name", "") or "ENTROPHASIC AURA TARGETING").strip() or "ENTROPHASIC AURA TARGETING"
        special_rules = dict(getattr(root, "special_rules", None) or {})
        special_rules["pantheon_entrophasic_aura_targeting_active"] = True
        special_rules["pantheon_entrophasic_aura_targeting_expires_phase"] = (
            "SHOOTING_PHASE" if phase_name == "shooting phase" else "FIGHT_PHASE"
        )
        special_rules["pantheon_entrophasic_aura_targeting_turn"] = int(self._necrons_current_turn())
        special_rules["pantheon_entrophasic_aura_targeting_turn_owner"] = str(getattr(current_player, "id", "") or "")
        special_rules["pantheon_entrophasic_aura_targeting_source"] = source
        root.special_rules = special_rules
        self._necrons_finalize_stratagem_use(stratagem, dequeue=kwargs.get("dequeue") is True)
        logger.info(
            "INFO: ENTROPHASIC AURA TARGETING: %s re-rolls Hit rolls of 1, and re-rolls Wound rolls of 1 against unravelling targets, this phase.",
            getattr(root, "name", "Unit"),
        )
        return True

    def _use_pantheon_mass_transmogrification(self, stratagem: Any, **kwargs) -> bool:
        unit = kwargs.get("unit") or kwargs.get("target_unit")
        source_unit = kwargs.get("source_unit")
        destroyed_unit = kwargs.get("destroyed_unit") or kwargs.get("enemy_unit")
        pending = self._pantheon_pending_context("MASS TRANSMOGRIFICATION", unit=unit)
        candidates = list(kwargs.get("candidates") or [])
        if isinstance(pending, dict):
            if source_unit is None:
                source_unit = pending.get("source_unit")
            if destroyed_unit is None:
        ...
        if unit is None and len(candidates) == 1:
            unit = candidates[0]
        if unit is None:
            logger.error("ERROR: MASS TRANSMOGRIFICATION: no target unit provided")
            return False
        root = self._necrons_root(unit)
        source_root = self._necrons_root(source_unit)
        destroyed_root = self._necrons_root(destroyed_unit)
        if root is None or source_root is None or destroyed_root is None or not self._is_pantheon_of_woe():
            return False
        phase_name = str(kwargs.get("phase_name") or self._current_phase_name or "").strip().lower().replace("_", " ")
        if phase_name not in {"shooting phase", "fight phase"}:
            logger.error("ERROR: MASS TRANSMOGRIFICATION: wrong phase")
            return False
        if phase_name == "shooting phase" and getattr(self.game, "get_current_player", lambda: None)() is not self.player:
            logger.error("ERROR: MASS TRANSMOGRIFICATION: only usable in your Shooting phase")
            return False
        if self._pantheon_mass_transmogrification_used_this_turn():
            logger.error("ERROR: MASS TRANSMOGRIFICATION: already used this turn")
            return False
        if self._necrons_owned_by_player(destroyed_root) or not self._necrons_owned_by_player(source_root):
            logger.error("ERROR: MASS TRANSMOGRIFICATION: trigger requires a friendly NECRONS MONSTER to destroy an enemy unit")
            return False
        if not self._pantheon_unit_eligible(source_root, require_monster=True):
            logger.error("ERROR: MASS TRANSMOGRIFICATION: source must be a friendly NECRONS MONSTER")
            return False
        valid_candidates = candidates or self._pantheon_non_monster_candidates(
            within_unit=source_root,
            range_inches=6.0,
        ...
        if not self._pantheon_unit_in_candidates(root, valid_candidates):
            logger.error("ERROR: MASS TRANSMOGRIFICATION: target must be a friendly non-MONSTER NECRONS unit within 6\" of the triggering MONSTER")
            return False
        mgr = self._get_necrons_mgr()
        if mgr is None or not bool(mgr.pantheon_unit_was_unravelling_at_phase_start(destroyed_root, game=self.game)):
            logger.error("ERROR: MASS TRANSMOGRIFICATION: destroyed enemy was not unravelling at the start of the phase")
            return False
        phase_label = "Shooting phase" if phase_name == "shooting phase" else "Fight phase"
        if not stratagem.can_use(
            self.player,
            ...
            logger.error("ERROR: MASS TRANSMOGRIFICATION: cannot be used in current state")
            return False
        if not self._necrons_spend_cp(stratagem, target_unit=root):
            return False
        roll = int(dice_module.get_roll("D3") or 0)
        ...
        self._mark_pantheon_mass_transmogrification_used()
        self._necrons_finalize_stratagem_use(stratagem, dequeue=kwargs.get("dequeue") is True)
        logger.info(
            "INFO: MASS TRANSMOGRIFICATION: %s triggers Reanimation Protocols for %d wound(s).",
            getattr(root, "name", "Unit"),
            int(roll),
        )
        return True

    def _use_pantheon_molecular_erosion(self, stratagem: Any, **kwargs) -> bool:
        unit = kwargs.get("unit") or kwargs.get("target_unit")
        enemy_unit = kwargs.get("enemy_unit")
        pending = self._pantheon_pending_context("MOLECULAR EROSION", unit=unit)
        candidates = list(kwargs.get("candidates") or [])
        enemy_candidates_by_unit = dict(kwargs.get("enemy_candidates_by_unit") or {})
        if isinstance(pending, dict):
            if not candidates:
        ...
        if unit is None and len(candidates) == 1:
            unit = candidates[0]
        if unit is None:
            logger.error("ERROR: MOLECULAR EROSION: no source unit provided")
            return False
        root = self._necrons_root(unit)
        if root is None or not self._is_pantheon_of_woe():
            return False
        ...
        if phase_name != "command phase":
            logger.error("ERROR: MOLECULAR EROSION: wrong phase")
            return False
        if self._pantheon_molecular_erosion_used_this_battle_round():
            logger.error("ERROR: MOLECULAR EROSION: already used this battle round")
            return False
        valid_candidates = candidates or self._pantheon_monster_candidates(require_visible_unravelling_enemy=True)
        if not self._pantheon_unit_in_candidates(root, valid_candidates):
            logger.error("ERROR: MOLECULAR EROSION: source must be a friendly NECRONS MONSTER with a visible unravelling enemy")
            return False
        root_id = str(get_entity_id(root) or "")
        enemy_candidates = list(enemy_candidates_by_unit.get(root_id) or [])
        if not enemy_candidates:
            enemy_candidates = self._pantheon_visible_unravelling_enemy_candidates(root)
        if enemy_unit is None and len(enemy_candidates) == 1:
            enemy_unit = enemy_candidates[0]
        enemy_root = self._necrons_root(enemy_unit)
        if enemy_root is None:
            logger.error("ERROR: MOLECULAR EROSION: no visible unravelling enemy selected")
            return False
        enemy_valid = any(self._necrons_root(candidate) is enemy_root for candidate in list(enemy_candidates or []))
        if not enemy_valid:
            logger.error("ERROR: MOLECULAR EROSION: selected enemy must be visible and unravelling to the source MONSTER")
            return False
        if not stratagem.can_use(
            self.player,
            self.game,
            ...
            logger.error("ERROR: MOLECULAR EROSION: cannot be used in current state")
            return False
        if not self._necrons_spend_cp(stratagem, target_unit=root):
            return False
        source = str(getattr(stratagem, "name", "") or "MOLECULAR EROSION").strip() or "MOLECULAR EROSION"
        current_turn = int(self._necrons_current_turn())
        was_battle_shocked = False
        is_battle_shocked = getattr(enemy_root, "is_battle_shocked", None)
        ...
        event_group = f"necrons_pantheon_molecular_erosion:{str(get_entity_id(enemy_root) or '')}:{current_turn}"

        def _capture_battle_shock_result(unit: Any = None, passed: Any = None, **_event_kwargs: Any) -> None:
            if self._necrons_root(unit) is not enemy_root:
                return
        ...
        if result["captured"] is True and result["passed"] is False:
            failed_test = True
        elif hasattr(enemy_root, "_last_leadership_test_passed"):
            failed_test = not bool(getattr(enemy_root, "_last_leadership_test_passed", True))
        ...
        self._necrons_finalize_stratagem_use(stratagem, dequeue=kwargs.get("dequeue") is True)
        logger.info(
            "INFO: MOLECULAR EROSION: %s takes a Battle-shock test at -1%s.",
            getattr(enemy_root, "name", "Enemy"),
            f" and suffers {int(mortal_wounds)} mortal wounds on failure" if mortal_wounds > 0 else "",
        )
        return True

    def _use_pantheon_phase_melding(self, stratagem: Any, **kwargs) -> bool:
        unit = kwargs.get("unit") or kwargs.get("target_unit")
        enemy_unit = kwargs.get("enemy_unit") or kwargs.get("moving_unit")
        pending = self._pantheon_pending_context("PHASE MELDING", unit=unit)
        candidates = list(kwargs.get("candidates") or [])
        if isinstance(pending, dict):
            if enemy_unit is None:
                enemy_unit = pending.get("enemy_unit") or pending.get("moving_unit")
        ...
        if unit is None and len(candidates) == 1:
            unit = candidates[0]
        if unit is None:
            logger.error("ERROR: PHASE MELDING: no target unit provided")
            return False
        root = self._necrons_root(unit)
        enemy_root = self._necrons_root(enemy_unit)
        if root is None or enemy_root is None or not self._is_pantheon_of_woe():
            return False
        phase_name = str(kwargs.get("phase_name") or self._current_phase_name or "").strip().lower().replace("_", " ")
        if phase_name != "movement phase":
            logger.error("ERROR: PHASE MELDING: wrong phase")
            return False
        active_player = getattr(self.game, "get_current_player", lambda: None)() if self.game is not None else None
        if active_player is self.player:
            logger.error("ERROR: PHASE MELDING: not opponent's Movement phase")
            return False
        action_key = str(kwargs.get("action") or "fall_back").strip().lower()
        if action_key not in {"fall_back", "fallback"}:
            logger.error("ERROR: PHASE MELDING: trigger requires an enemy Fall Back move")
            return False
        mgr = self._get_necrons_mgr()
        if mgr is None or not bool(mgr.pantheon_unit_is_unravelling(enemy_root, game=self.game)):
            logger.error("ERROR: PHASE MELDING: triggering enemy unit is not unravelling")
            return False
        valid_candidates = candidates or self._pantheon_phase_melding_candidates(enemy_root)
        if not self._pantheon_unit_in_candidates(root, valid_candidates):
            logger.error("ERROR: PHASE MELDING: target must be a friendly NECRONS unit within Engagement Range of the triggering enemy")
            return False
        if not stratagem.can_use(
            self.player,
            self.game,
            ...
            logger.error("ERROR: PHASE MELDING: cannot be used in current state")
            return False
        if not self._necrons_spend_cp(stratagem, target_unit=root):
            return False
        source_name = str(getattr(stratagem, "name", "") or "PHASE MELDING").strip() or "PHASE MELDING"
        sr = getattr(root, "special_rules", None)
        if not isinstance(sr, dict):
            sr = {}
        ...
        sr["enemy_fallback_desperate_escape_target_enemy_id"] = str(get_entity_id(enemy_root) or "")
        ...
        sr["enemy_fallback_desperate_escape_turn"] = int(self._necrons_current_turn())
        sr["enemy_fallback_desperate_escape_source"] = source_name
        root.special_rules = sr
        self._necrons_finalize_stratagem_use(stratagem, dequeue=kwargs.get("dequeue") is True)
        logger.info(
            "INFO: PHASE MELDING: %s will force Desperate Escape tests on %s.",
            getattr(root, "name", "Unit"),
            getattr(enemy_root, "name", "Enemy"),
        )
        return True
    ...
    def _use_necrons_stratagem(self, stratagem: Any, **kwargs) -> Optional[bool]:
        ...
        if name_u == "CHRONODISTORTION" and self._is_pantheon_of_woe():
            return self._use_pantheon_chronodistortion(stratagem, **kwargs)
        if name_u == "DISHARMONISATION CASCADE" and self._is_pantheon_of_woe():
            return self._use_pantheon_disharmonisation_cascade(stratagem, **kwargs)
        if name_u == "ENTROPHASIC AURA TARGETING" and self._is_pantheon_of_woe():
            return self._use_pantheon_entrophasic_aura_targeting(stratagem, **kwargs)
        if name_u == "MASS TRANSMOGRIFICATION" and self._is_pantheon_of_woe():
            return self._use_pantheon_mass_transmogrification(stratagem, **kwargs)
        if name_u == "MOLECULAR EROSION" and self._is_pantheon_of_woe():
            return self._use_pantheon_molecular_erosion(stratagem, **kwargs)
        if name_u == "PHASE MELDING" and self._is_pantheon_of_woe():
            return self._use_pantheon_phase_melding(stratagem, **kwargs)
