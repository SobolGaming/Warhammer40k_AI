...
class UnitRoundState:
    ...
    charge_roll_id: Optional[int] = None  # DiceRollManager roll id for the active charge roll
    charge_move_resolved_this_round: bool = False  # Track whether the declared charge's move step has resolved
...
from .unit_mixins import (
    ...
    combat_runtime_mixin as _combat_runtime_mixin,
...
class Unit(
        ...
        *,
        ...
        if ckey == "objective_control":
            ...
            try:
                sm_mgr = getattr(army, "space_marines_detachments", None) if army is not None else None
                bonus_fn = (
                    getattr(sm_mgr, "unyielding_might_objective_control_bonus", None)
                    if sm_mgr is not None
                    else None
                )
                if callable(bonus_fn):
                    ...
                    bonus, source = bonus_fn(model, unit=self, game=game)
                    if int(bonus or 0) > 0:
                        source_name = str(source or "Unyielding Might").strip() or "Unyielding Might"
                        mods.append(Modifier(ModifierOp.ADD, int(bonus), source=f"detachment:{source_name}"))
            except Exception:
                pass
            try:
    ...
    _CHARGE_END_MODEL_WEAPON_ATTACKS_BONUS_RE = re.compile(
        r"each\s+time\s+this\s+model\s+ends?\s+a\s+charge\s+move\s+until\s+the\s+end\s+of\s+the\s+turn\s+"
        r"add\s+(?P<bonus>\d+)\s+to\s+the\s+attacks\s+characteristic\s+of\s+(?:this\s+model\s+s|this\s+models|its)\s+"
        r"(?P<weapon>[a-z0-9 ]+?)\s+weapon",
        re.IGNORECASE,
    )
