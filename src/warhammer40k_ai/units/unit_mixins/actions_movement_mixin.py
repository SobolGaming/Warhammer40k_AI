...
class ActionsMovementMixin:
    ...
    def can_shoot_after_advance(self, profile) -> bool:
        ...
        try:
            army = self.get_parent_army()
            mgr = getattr(army, "astra_militarum_detachments", None) if army is not None else None
            apply_fn = getattr(mgr, "can_shoot_after_advance", None) if mgr is not None else None
            if callable(apply_fn):
                game = getattr(getattr(army, "player", None), "game", None) if army is not None else None
                if bool(apply_fn(self, profile=profile, game=game)):
                    return True
    ...
    def _is_siege_regiment_callous_sacrifice_active(
        self,
        *,
        game=None,
        phase_name: Optional[str] = None,
        ...
        if game is None:
            ...
            game = getattr(getattr(army, "player", None), "game", None) if army is not None else None
        owner = str(sr.get("siege_regiment_callous_sacrifice_turn_owner", "") or "")
        if owner:
            army = getattr(root, "get_parent_army", lambda: None)()
        ...
        if exp:
            ...
            if phase_key and phase_key != exp:
                return False
        turn = int(sr.get("siege_regiment_callous_sacrifice_turn", 0) or 0)
        if turn:
            if game is None:
                return False
            if int(getattr(game, "turn", 0) or 0) != turn:
