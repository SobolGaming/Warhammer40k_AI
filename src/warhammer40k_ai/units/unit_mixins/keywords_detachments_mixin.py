...
class KeywordsDetachmentsMixin:
    ...
    def get_melee_fight_on_death_after_attacks_rule(self, model: Optional['Model'] = None) -> Optional[dict]:
        ...
        # Temporary effect hook: Renegade Warband (Undying Hatred).
        try:
            sr = getattr(self, "special_rules", None)
            if isinstance(sr, dict) and sr.get("renegade_warband_undying_hatred_active"):
                phase_name = ""
                current_turn = 0
                current_owner_id = ""
                try:
                ...
                if game is not None:
                    ...
                    current_owner_id = str(getattr(current_player, "id", "") or "").strip()
                try:
                    marked_turn = int(sr.get("renegade_warband_undying_hatred_turn", 0) or 0)
                except Exception:
                    marked_turn = 0
                marked_owner_id = str(sr.get("renegade_warband_undying_hatred_turn_owner", "") or "").strip()
                exp = str(sr.get("renegade_warband_undying_hatred_expires_phase", "") or "").strip().upper()
                if phase_name == "FIGHT_PHASE" and (not exp or exp == "FIGHT_PHASE"):
                    if not (marked_turn and current_turn and marked_turn != current_turn):
                        if not (marked_owner_id and current_owner_id and marked_owner_id != current_owner_id):
                            source = str(
                                sr.get("renegade_warband_undying_hatred_source", "") or "Undying Hatred"
                            ).strip() or "Undying Hatred"
                            try:
                                threshold = int(sr.get("renegade_warband_undying_hatred_threshold", 4) or 4)
                            except Exception:
                                threshold = 4
                            return {
                                "threshold": max(2, min(6, int(threshold))),
                ...
                if phase_name and phase_name != "FIGHT_PHASE":
                    for key in (
                        "renegade_warband_undying_hatred_active",
                        "renegade_warband_undying_hatred_threshold",
                        "renegade_warband_undying_hatred_expires_phase",
                        "renegade_warband_undying_hatred_source",
                        "renegade_warband_undying_hatred_turn_owner",
                        "renegade_warband_undying_hatred_turn",
                    ):
                        sr.pop(key, None)
                    self.special_rules = sr
        except Exception:
        ...
        # Temporary effect hook: UNDYING SPITE (Death Lord's Chosen).
        try:
            sr = getattr(self, "special_rules", None)
            if isinstance(sr, dict) and sr.get("death_guard_undying_spite_active"):
                phase_name = ""
                current_turn = 0
                current_owner_id = ""
                try:
                ...
                if game is not None:
                    ...
                    current_owner_id = str(getattr(current_player, "id", "") or "").strip()
                try:
                    marked_turn = int(sr.get("death_guard_undying_spite_turn", 0) or 0)
                except Exception:
                    marked_turn = 0
                marked_owner_id = str(sr.get("death_guard_undying_spite_turn_owner", "") or "").strip()
                exp = str(sr.get("death_guard_undying_spite_expires_phase", "") or "").strip().upper()
                if phase_name == "FIGHT_PHASE" and (not exp or exp == "FIGHT_PHASE"):
                    if not (marked_turn and current_turn and marked_turn != current_turn):
                        if not (marked_owner_id and current_owner_id and marked_owner_id != current_owner_id):
                            source = str(sr.get("death_guard_undying_spite_source", "") or "Undying Spite").strip()
                            source = source or "Undying Spite"
                            try:
                                threshold = int(sr.get("death_guard_undying_spite_threshold", 4) or 4)
                            except Exception:
                                threshold = 4
                            return {
                                "threshold": max(2, min(6, int(threshold))),
                ...
                if phase_name and phase_name != "FIGHT_PHASE":
                    for key in (
                        "death_guard_undying_spite_active",
                        "death_guard_undying_spite_threshold",
                        "death_guard_undying_spite_expires_phase",
                        "death_guard_undying_spite_source",
                        "death_guard_undying_spite_turn_owner",
                        "death_guard_undying_spite_turn",
                    ):
                        sr.pop(key, None)
                    self.special_rules = sr
        except Exception:
        ...
        tyr_rule_fn = (
            getattr(tyr_mgr, "crusher_enraged_reserves_fight_on_death_rule", None)
            if tyr_mgr is not None
            else None
        )
        if callable(tyr_rule_fn):
            tyr_rule = tyr_rule_fn(self, model=model, game=game)
            if isinstance(tyr_rule, dict):
                try:
                    threshold = int(tyr_rule.get("threshold", 0) or 0)
                except (TypeError, ValueError):
                    threshold = 0
                if 2 <= threshold <= 6:
    ...
    def _precursive_judgement_source_unit(self):
        try:
            root = self.get_attached_unit_root()
        except Exception:
            root = self
        ...
        for member in members:
            if member is None:
                continue
            sr = getattr(member, "special_rules", None)
            if not isinstance(sr, dict) or not bool(sr.get("enhancement_precursive_judgement", False)):
                continue
            bearer = None
            bearer_id = str(
                sr.get("enhancement_precursive_judgement_bearer_model_id", "")
                or sr.get("enhancement_bearer_model_id", "")
                or ""
            ).strip()
            if bearer_id:
    ...
    def get_precursive_judgement_overwatch_rule(self) -> Optional[dict]:
        try:
            root = self.get_attached_unit_root()
        except Exception:
            root = self
        cache_key = "precursive_judgement_overwatch_rule"
        if cache_key in getattr(root, "_ability_cache", {}):
            return root._ability_cache[cache_key]

        rule = None
        source_unit, source_sr = root._precursive_judgement_source_unit()
        if source_unit is not None and isinstance(source_sr, dict):
            enhancement = getattr(source_unit, "enhancement", None)
            source = str(
                source_sr.get("enhancement_precursive_judgement_source", "")
                or getattr(enhancement, "name", "")
                or "Precursive Judgement"
            ).strip() or "Precursive Judgement"
            stratagems = [
                str(v or "").strip().upper()
                for v in list(
                    source_sr.get(
                        "enhancement_precursive_judgement_stratagems",
            ...
            if not stratagems:
                stratagems = ["OVERWATCH", "FIRE OVERWATCH"]
            try:
                base_threshold = int(source_sr.get("enhancement_precursive_judgement_overwatch_hit_threshold", 5) or 5)
            except Exception:
                base_threshold = 0
            try:
                transport_range_in = float(source_sr.get("enhancement_precursive_judgement_transport_range", 6.0) or 6.0)
            except Exception:
                transport_range_in = 6.0
            if base_threshold > 0 and transport_range_in > 0:
                rule = {
                    "source": source,
                    "stratagems": tuple(stratagems),
                    "base_threshold": int(base_threshold),
                    "transport_range_in": float(transport_range_in),
                }
                try:
                    source_id = get_entity_id(source_unit)
                except Exception:
    ...
    def _precursive_judgement_transport_proximity_ok(self, game=None) -> bool:
        _ = game
        try:
            root = self.get_attached_unit_root()
        except Exception:
            root = self
        rule = root.get_precursive_judgement_overwatch_rule()
        if not rule:
            return False
        try:
            army = root.get_parent_army()
        ...
        within_fn = getattr(mgr, "brandfast_unit_wholly_within_transport_range", None) if mgr is not None else None
        if not callable(within_fn):
            return False
        try:
            transport_range = float(rule.get("transport_range_in", 6.0) or 6.0)
        except Exception:
            transport_range = 6.0
        return bool(within_fn(root, range_in=float(transport_range)))

    def can_use_precursive_judgement_overwatch(self, game=None, *, stratagem_name: str = "") -> bool:
        try:
            root = self.get_attached_unit_root()
        except Exception:
            root = self
        ...
        if name_u and allowed and name_u not in allowed:
            return False
        return bool(root._precursive_judgement_transport_proximity_ok(game=game))

    def get_precursive_judgement_overwatch_hit_threshold(self, *, enemy_unit=None, game=None) -> int:
        _ = enemy_unit
        try:
            root = self.get_attached_unit_root()
        except Exception:
            root = self
        if not root.can_use_precursive_judgement_overwatch(game=game, stratagem_name="OVERWATCH"):
            return 0
        rule = root.get_precursive_judgement_overwatch_rule()
        if not rule:
            return 0
    ...
    def _death_guard_temp_effect_target_matches(
        ...
        *,
        ...
        excluded_keywords = [
            ...
            for keyword in list(entry.get("exclude_keywords_any", []) or [])
