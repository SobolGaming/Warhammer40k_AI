...
class LateGameplayMixin:
    ...
    def get_ranged_targeting_restriction(
        ...
        *,
        ...
        try:
            sr = getattr(root, "special_rules", None)
            if isinstance(sr, dict) and sr.get("thousand_sons_fractal_disjunction_active"):
                applies = True
                game = None
                try:
                    game = getattr(getattr(root.get_parent_army(), "player", None), "game", None)
                    ...
                    game = None
                owner_id = str(sr.get("thousand_sons_fractal_disjunction_turn_owner", "") or "").strip()
                if owner_id:
                    unit_owner = ""
                    try:
                        unit_owner = str(
                ...
                if applies and game is not None:
                    ...
                    if owner_id and current_owner and current_owner == owner_id:
                        applies = False
                    exp_phase = str(sr.get("thousand_sons_fractal_disjunction_expires_phase", "") or "").strip().upper()
                    current_phase = str(getattr(getattr(game, "phase", None), "name", "") or "").strip().upper()
                    if exp_phase and current_phase and current_phase != exp_phase:
                        applies = False
                    try:
                        effect_turn = int(sr.get("thousand_sons_fractal_disjunction_turn", 0) or 0)
                    except Exception:
                        effect_turn = 0
                    try:
                        current_turn = int(getattr(game, "turn", 0) or 0)
                ...
                if applies:
                    try:
                        dist = float(sr.get("thousand_sons_fractal_disjunction_targeting_range", 18) or 18)
                    except Exception:
                        dist = 18.0
                    source = str(sr.get("thousand_sons_fractal_disjunction_source", "") or "").strip() or "FRACTAL DISJUNCTION"
                    _consider(dist, source)
        except Exception:
            pass
        ...
        try:
            sr = getattr(root, "special_rules", None)
            if isinstance(sr, dict) and sr.get("pactbound_festering_miasma_active"):
                applies = True
                game = None
                try:
                    game = getattr(getattr(root.get_parent_army(), "player", None), "game", None)
                    ...
                    game = None
                owner_id = str(sr.get("pactbound_festering_miasma_turn_owner", "") or "").strip()
                if owner_id:
                    unit_owner = ""
                    try:
                        unit_owner = str(
                    ...
                    if unit_owner and unit_owner != owner_id:
                        applies = False
                if applies and game is not None:
                    exp_phase = str(sr.get("pactbound_festering_miasma_phase", "") or "").strip().upper()
                    current_phase = str(getattr(getattr(game, "phase", None), "name", "") or "").strip().upper()
                    if exp_phase and current_phase and current_phase != exp_phase:
                        applies = False
                    try:
                        effect_turn = int(sr.get("pactbound_festering_miasma_turn", 0) or 0)
                    except Exception:
                        effect_turn = 0
                    try:
                        current_turn = int(getattr(game, "turn", 0) or 0)
                ...
                if applies:
                    try:
                        dist = float(sr.get("pactbound_festering_miasma_targeting_range", 18) or 18)
                    except Exception:
                        dist = 18.0
                    source = str(sr.get("pactbound_festering_miasma_source", "") or "").strip() or "FESTERING MIASMA"
                    _consider(dist, source)
        except Exception:
            pass
