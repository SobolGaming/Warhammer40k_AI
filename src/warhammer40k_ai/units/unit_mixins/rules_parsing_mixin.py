...
class RulesParsingMixin:
    ...
    def _scan_end_of_opponent_turn_strategic_reserves_ability(self):
        ...
        if isinstance(sr, dict) and bool(sr.get("enhancement_fulgus_magna")):
            bearer_alive = False
            ...
            if bearer_id:
                ...
                if bearer is not None:
                    ...
                    bearer_alive = bool(alive_attr() if callable(alive_attr) else alive_attr)
            if bearer_alive:
                ability_name = str(sr.get("enhancement_fulgus_magna_source", "") or "Fulgus Magna").strip() or "Fulgus Magna"
                ability_key = str(sr.get("enhancement_fulgus_magna_once_key", "") or "fulgus_magna").strip().lower()
                if not ability_key:
                    ability_key = "fulgus_magna"
                return {
                    "name": ability_name,
                    "description": "",
                    "once_per_battle": True,
        ...
        if isinstance(sr, dict) and bool(sr.get("enhancement_warprot_talisman")):
            bearer_alive = False
            bearer_id = str(
                sr.get("enhancement_warprot_talisman_bearer_model_id", "")
                or sr.get("enhancement_bearer_model_id", "")
                or ""
            ).strip()
            if bearer_id:
                ...
                if bearer is not None:
                    alive_attr = getattr(bearer, "is_alive", True)
                    bearer_alive = bool(alive_attr() if callable(alive_attr) else alive_attr)
            requires_alive = bool(sr.get("enhancement_warprot_talisman_requires_bearer_alive", True))
            if bearer_alive or not requires_alive:
                ability_name = str(sr.get("enhancement_warprot_talisman_source", "") or "Warprot Talisman").strip()
                if not ability_name:
                    ability_name = "Warprot Talisman"
                ability_key = str(
                    sr.get("enhancement_warprot_talisman_ability_key", "") or "warprot_talisman"
                ).strip().lower()
                if not ability_key:
                    ability_key = "warprot_talisman"
                trigger_phase = str(
                    sr.get("enhancement_warprot_talisman_trigger_phase", "") or "OPPONENT_TURN_END"
                ).strip().upper()
                ...
                return {
                    "name": ability_name,
                    "description": "",
                    "once_per_battle": bool(sr.get("enhancement_warprot_talisman_once_per_battle", True)),
                    "ability_key": ability_key,
    ...
    def _get_return_on_death_specs(self) -> list[dict]:
        ...
        if isinstance(sr, dict) and sr.get("enhancement_indomitable_fury"):
            ...
            try:
                roll_min = int(sr.get("enhancement_indomitable_fury_roll_min", 2) or 2)
            except (TypeError, ValueError):
                roll_min = 2
            wounds_spec = sr.get("enhancement_indomitable_fury_wounds", "full")
            if isinstance(wounds_spec, str):
                ...
                try:
                    ...
                    wounds_spec = "full"
            key = str(sr.get("enhancement_indomitable_fury_key", "indomitable_fury") or "indomitable_fury").strip().lower()
            specs.append(
                {
                    "name": str(sr.get("enhancement_indomitable_fury_source", "Indomitable Fury") or "Indomitable Fury"),
                    "roll_min": int(max(2, roll_min)),
                    "wounds": wounds_spec,
                    "skip_deadly_demise": False,
                    "key": key if key else "indomitable_fury",
                    "bearer_model_id": bearer_id,
                }
            )
        ...
        if isinstance(sr, dict) and sr.get("enhancement_indomitable_champion"):
            ...
            try:
                roll_min = int(sr.get("enhancement_indomitable_champion_roll_min", 2) or 2)
            except (TypeError, ValueError):
                roll_min = 2
            try:
                wounds = int(sr.get("enhancement_indomitable_champion_wounds", 3) or 3)
            except (TypeError, ValueError):
                wounds = 3
            key = str(sr.get("enhancement_indomitable_champion_key", "indomitable_champion") or "indomitable_champion").strip().lower()
            specs.append(
                {
                    "name": str(sr.get("enhancement_indomitable_champion_source", "Indomitable Champion") or "Indomitable Champion"),
                    "roll_min": int(max(2, roll_min)),
                    "wounds": int(max(1, wounds)),
                    "skip_deadly_demise": False,
                    "key": key if key else "indomitable_champion",
                    "bearer_model_id": bearer_id,
                }
            )
