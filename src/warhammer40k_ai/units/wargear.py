...
class WargearProfile:
    ...
    def _aeldari_focused_firepower_ap_bonus(self, attack_instance: Dict) -> tuple[int, str]:
        ...
        try:
            ...
            root = attacker_unit
        sr = getattr(root, "special_rules", None)
        if not isinstance(sr, dict) or not bool(sr.get("aeldari_focused_firepower_active")):
            return 0, ""

        applies = True
        exp_phase = str(sr.get("aeldari_focused_firepower_expires_phase", "") or "").strip().upper()
        owner_id = str(sr.get("aeldari_focused_firepower_turn_owner", "") or "").strip()
        try:
            effect_turn = int(sr.get("aeldari_focused_firepower_turn", 0) or 0)
        except Exception:
            effect_turn = 0
        game = None
        try:
        ...
        try:
            bonus = int(sr.get("aeldari_focused_firepower_ap_bonus", 1) or 1)
        except Exception:
            bonus = 1
        if bonus <= 0:
            return 0, ""
        source = (
            str(sr.get("aeldari_focused_firepower_source", "") or "FOCUSED FIREPOWER").strip()
    ...
    def _rad_zone_lethal_dosage_active(self, attacker: 'Model') -> bool:
        ...
        return self._phase_effect_special_rules(
            attacker,
            active_key="rad_zone_lethal_dosage_active",
            expires_key="rad_zone_lethal_dosage_expires_phase",
            owner_key="rad_zone_lethal_dosage_turn_owner",
            turn_key="rad_zone_lethal_dosage_turn",
        ) is not None
                ...
                if pa_a_bonus:
                    atk_mods.append(
                        Modifier(
                            ModifierOp.ADD,
                            int(pa_a_bonus),
                            source="enhancement:plasma_accelerator_rifle_attacks_add",
                        )
                    )
    ...
    def _hit_target_with_tracking(
        ...
        *,
        ...
        try:
            ...
            army = unit.get_parent_army() if unit is not None else None
            tau_mgr = getattr(army, "tau_empire_detachments", None) if army is not None else None
            if tau_mgr is not None and callable(getattr(tau_mgr, "hunters_instincts_hit_bonus", None)):
                bonus, source = tau_mgr.hunters_instincts_hit_bonus(
                    attacker,
                    target,
                    weapon_profile=self,
                    attack_instance=attack_instance,
        ...
        # Space Marines (Saga of the Bold): Heroes All
        # selected to shoot/fight: pre-saga (Space Wolves CHARACTER only) one of Hit/Wound/Damage;
        # post-saga one Hit, one Wound, and one Damage.
        try:
            if rerolls_allowed and "reroll" not in hit_result:
                action = "shoot" if attack_is_ranged else "fight" if attack_is_melee else ""
                unit = getattr(attacker, "parent_unit", None)
                army = unit.get_parent_army() if unit is not None else None
                sm_mgr = getattr(army, "space_marines_detachments", None) if army is not None else None
                can_use = getattr(sm_mgr, "heroes_all_reroll_is_available", None) if sm_mgr is not None else None
                consume = getattr(sm_mgr, "consume_heroes_all_reroll", None) if sm_mgr is not None else None
                if action and callable(can_use) and callable(consume) and bool(can_use(unit, "hit", action=action)):
                    try:
                        success = (dice_roll != 1) and (self.skill > 0) and (dice_roll >= final_needed)
                    except Exception:
                        success = False
                    ...
                    if is_human and callable(provider):
                        ...
                        do_reroll = (not success)
                    if do_reroll and bool(consume(unit, "hit", action=action)):
                        rr = _reroll_hit()
                        hit_result.setdefault("special_effects", []).append("Heroes All: re-roll Hit roll")
                        hit_result["reroll"] = rr
                        dice_roll = rr
                        reroll_used = True
        except Exception:
        ...
        # selected to shoot/fight: if near a friendly THOUSAND SONS PSYKER, one Hit + one Wound + one Damage reroll;
        # otherwise choose one of Hit/Wound/Damage.
        try:
            if rerolls_allowed and "reroll" not in hit_result:
                action = "shoot" if attack_is_ranged else "fight" if attack_is_melee else ""
                unit = getattr(attacker, "parent_unit", None)
                ...
                can_use = getattr(ts_mgr, "warpfire_infusion_reroll_is_available", None) if ts_mgr is not None else None
                consume = getattr(ts_mgr, "consume_warpfire_infusion_reroll", None) if ts_mgr is not None else None
                game_local = getattr(getattr(army, "player", None), "game", None) if army is not None else None
                if (
                    action
                    and callable(can_use)
                    and callable(consume)
                    and bool(can_use(unit, "hit", action=action, game=game_local))
                ):
                    try:
                        success = (dice_roll != 1) and (self.skill > 0) and (dice_roll >= final_needed)
                    except Exception:
                    ...
                    if is_human and callable(provider):
                        ...
                        do_reroll = (not success)
                    if do_reroll and bool(consume(unit, "hit", action=action, game=game_local)):
                        rr = _reroll_hit()
                        hit_result.setdefault("special_effects", []).append("Warpfire Infusion: re-roll Hit roll")
                        hit_result["reroll"] = rr
                        dice_roll = rr
                        reroll_used = True
        except Exception:
        ...
        # Space Marines (Ironstorm Spearhead): Armoured Wrath
        # once per phase per ADEPTUS ASTARTES unit, re-roll one Hit/Wound/Damage roll.
        try:
            if rerolls_allowed and "reroll" not in hit_result:
                unit = getattr(attacker, "parent_unit", None)
                army = unit.get_parent_army() if unit is not None else None
                sm_mgr = getattr(army, "space_marines_detachments", None) if army is not None else None
                can_use = getattr(sm_mgr, "armoured_wrath_reroll_is_available", None) if sm_mgr is not None else None
                consume = getattr(sm_mgr, "consume_armoured_wrath_reroll", None) if sm_mgr is not None else None
                game_local = getattr(getattr(army, "player", None), "game", None) if army is not None else None
                if callable(can_use) and callable(consume) and bool(can_use(unit, "hit", game=game_local)):
                    try:
                        success = (dice_roll != 1) and (self.skill > 0) and (dice_roll >= final_needed)
                    except Exception:
                        success = False
                    ...
                    if is_human and callable(provider):
                        ...
                        do_reroll = (not success)
                    if do_reroll and bool(consume(unit, "hit", game=game_local)):
                        rr = _reroll_hit()
                        hit_result.setdefault("special_effects", []).append("Armoured Wrath: re-roll Hit roll")
                        hit_result["reroll"] = rr
                        dice_roll = rr
                        reroll_used = True
        except Exception:
        ...
        try:
            unit = getattr(attacker, "parent_unit", None)
            root = unit.get_attached_unit_root() if unit is not None and hasattr(unit, "get_attached_unit_root") else unit
            context_fn = getattr(root, "_gsc_primed_and_readied_context", None) if root is not None else None
        ...
        if orks_mgr is not None:
            ...
            if callable(loot_fn):
                freebooter_loot_sustained_value = int(loot_fn(attacker, target_unit=target) or 0)
        freebooter_loot_sustained = bool(freebooter_loot_sustained_value)
        ...
        *,
        ...
        try:
            if self.parent_wargear and self.parent_wargear.is_ranged() and isinstance(strength, int):
                ...
                army = unit.get_parent_army() if unit is not None else None
                sm_mgr = getattr(army, "space_marines_detachments", None) if army is not None else None
                if sm_mgr is not None and callable(getattr(sm_mgr, "close_range_eradication_strength_bonus", None)):
                    s_bonus, source = sm_mgr.close_range_eradication_strength_bonus(
                        attacker,
                        target,
                        weapon_profile=self,
                        attack_instance=attack_instance,
        ...
        try:
            ...
            pass
        # Space Marines: Vindication Task Force (Purge and Sanctify) defensive wound penalty.
        try:
            target_army = target.get_parent_army()
            sm_mgr = getattr(target_army, "space_marines_detachments", None) if target_army is not None else None
            if sm_mgr is not None and callable(getattr(sm_mgr, "purge_and_sanctify_wound_roll_penalty", None)):
                game = getattr(getattr(target_army, "player", None), "game", None) if target_army is not None else None
                penalty, source = sm_mgr.purge_and_sanctify_wound_roll_penalty(
                    target,
                    strength=strength,
                    target_toughness=target_toughness,
                    game=game,
                ...
                if penalty:
                    dice_modifier -= int(penalty)
                    source_name = str(source or "Purge and Sanctify").strip() or "Purge and Sanctify"
                    wound_result["modifiers"].append(f"-{int(penalty)} to wound from {source_name}")
        except Exception:
            pass
        ...
        try:
            ...
            if tau_mgr is not None and callable(getattr(tau_mgr, "hunters_instincts_wound_bonus", None)):
                wound_bonus, source = tau_mgr.hunters_instincts_wound_bonus(
                    attacker,
                    target,
                    weapon_profile=self,
                    attack_instance=attack_instance,
        ...
        try:
            if rerolls_allowed and "reroll" not in wound_result:
                ...
                can_use = getattr(sm_mgr, "heroes_all_reroll_is_available", None) if sm_mgr is not None else None
                consume = getattr(sm_mgr, "consume_heroes_all_reroll", None) if sm_mgr is not None else None
                if action and callable(can_use) and callable(consume) and bool(can_use(unit, "wound", action=action)):
                    needed = 0
                    try:
                        s_val = strength
                        t_val = target_toughness
                    ...
                    if is_human and callable(provider):
                        ...
                        do_reroll = (not success)
                    if do_reroll and bool(consume(unit, "wound", action=action)):
                        rr = _reroll_wound()
                        wound_result.setdefault("special_effects", []).append("Heroes All: re-roll Wound roll")
                        wound_result["reroll"] = rr
                        dice_roll = rr
                        reroll_used = True
        except Exception:
        ...
        try:
            if rerolls_allowed and "reroll" not in wound_result:
                ...
                if (
                    ...
                    and callable(can_use)
                    and callable(consume)
                    and bool(can_use(unit, "wound", action=action, game=game_local))
                ):
                    needed = 0
                    try:
                        s_val = strength
                    ...
                    if is_human and callable(provider):
                        ...
                        do_reroll = (not success)
                    if do_reroll and bool(consume(unit, "wound", action=action, game=game_local)):
                        rr = _reroll_wound()
                        wound_result.setdefault("special_effects", []).append("Warpfire Infusion: re-roll Wound roll")
                        wound_result["reroll"] = rr
                        dice_roll = rr
                        reroll_used = True
        except Exception:
        ...
        try:
            if rerolls_allowed and "reroll" not in wound_result:
                ...
                consume = getattr(sm_mgr, "consume_armoured_wrath_reroll", None) if sm_mgr is not None else None
                game_local = getattr(getattr(army, "player", None), "game", None) if army is not None else None
                if callable(can_use) and callable(consume) and bool(can_use(unit, "wound", game=game_local)):
                    needed = 0
                    try:
                        s_val = strength
                        t_val = target_toughness
                    ...
                    if is_human and callable(provider):
                        ...
                        do_reroll = (not success)
                    if do_reroll and bool(consume(unit, "wound", game=game_local)):
                        rr = _reroll_wound()
                        wound_result.setdefault("special_effects", []).append("Armoured Wrath: re-roll Wound roll")
                        wound_result["reroll"] = rr
                        dice_roll = rr
                        reroll_used = True
        except Exception:
    ...
    def _save_with_tracking(
        ...
        *,
        ...
        if self.parent_wargear is not None and callable(getattr(self.parent_wargear, "is_ranged", None)):
            is_ranged_attack = bool(self.parent_wargear.is_ranged())
        elif getattr(self, "range", None) is not None:
            try:
                is_ranged_attack = int(getattr(self.range, "max", 0) or 0) > 0
            except (TypeError, ValueError):
                is_ranged_attack = False
        if t_unit is not None and attacker_unit is not None and is_ranged_attack:
            ...
            game_map = getattr(game, "map", None) if game is not None else None
            protector_units = list(getattr(t_army, "units", []) or []) if t_army is not None else []
            get_bonus = getattr(game_map, "get_selfless_protector_bonus_for_ranged_attack", None) if game_map is not None else None
            if callable(get_bonus):
                bonus = get_bonus(
                ...
                if bool(bonus.get("applies")):
                    source_name = str(
                        ...
                        or "Selfless Protector"
                    ).strip() or "Selfless Protector"
                    if bool(bonus.get("grants_benefit_of_cover")):
                        attack_instance.setdefault("benefit_of_cover", True)
                        ...
                        elif source_name.lower() not in {p.strip().lower() for p in existing_source.split(",") if p.strip()}:
        ...
        try:
            ...
            focused_ap_bonus, focused_source = 0, ""
        if int(focused_ap_bonus or 0) > 0:
            effective_ap = int(effective_ap) - int(focused_ap_bonus)
            source_name = str(focused_source or "FOCUSED FIREPOWER").strip() or "FOCUSED FIREPOWER"
            save_result["special_effects"].append(
                f"{source_name}: AP improved by {int(focused_ap_bonus)}"
            )
        try:
        ...
        *,
        ...
        try:
            if rerolls_allowed and isinstance(self.damage, DiceCollection) and "reroll" not in damage_result:
                ...
                can_use = getattr(sm_mgr, "heroes_all_reroll_is_available", None) if sm_mgr is not None else None
                consume = getattr(sm_mgr, "consume_heroes_all_reroll", None) if sm_mgr is not None else None
                if action and callable(can_use) and callable(consume) and bool(can_use(unit, "damage", action=action)):
                    do_reroll = False
                    try:
                        player = getattr(army, "player", None) if army is not None else None
                        is_human = bool(getattr(player, "has_control", lambda: False)())
                    ...
                    if do_reroll and bool(consume(unit, "damage", action=action)):
                        ...
                        damage_result['damage_rolled'] = new_val
                        damage_result.setdefault('special_effects', []).append(
                            "Heroes All: re-roll Damage roll"
                        )
                        damage_result['reroll'] = new_val
        except Exception:
            pass
        ...
        try:
            if rerolls_allowed and isinstance(self.damage, DiceCollection) and "reroll" not in damage_result:
                ...
                if (
                    ...
                    and callable(can_use)
                    and callable(consume)
                    and bool(can_use(unit, "damage", action=action, game=game_local))
                ):
                    do_reroll = False
                    try:
                        player = getattr(army, "player", None) if army is not None else None
                    ...
                    if do_reroll and bool(consume(unit, "damage", action=action, game=game_local)):
                        ...
                        damage_result['damage_rolled'] = new_val
                        damage_result.setdefault('special_effects', []).append(
                            "Warpfire Infusion: re-roll Damage roll"
                        )
                        damage_result['reroll'] = new_val
        except Exception:
            pass
        ...
        try:
            if rerolls_allowed and isinstance(self.damage, DiceCollection) and "reroll" not in damage_result:
                ...
                consume = getattr(sm_mgr, "consume_armoured_wrath_reroll", None) if sm_mgr is not None else None
                game_local = getattr(getattr(army, "player", None), "game", None) if army is not None else None
                if callable(can_use) and callable(consume) and bool(can_use(unit, "damage", game=game_local)):
                    do_reroll = False
                    try:
                        player = getattr(army, "player", None) if army is not None else None
                        is_human = bool(getattr(player, "has_control", lambda: False)())
                ...
                if pa_d_bonus:
                    damage_mods.append(
                        Modifier(
                            ModifierOp.ADD,
                            int(pa_d_bonus),
                            source="enhancement:plasma_accelerator_rifle_damage_add",
                        )
                    )
                    source_name = str(pa_source or "Plasma Accelerator Rifle").strip() or "Plasma Accelerator Rifle"
