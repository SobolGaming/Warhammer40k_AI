...
def test_load_all_datasheet_units():
    ...
    for unit_name, unit_id in waha_helper.get_all_datasheet_names():
        ...
        for model in u.models:
            logger.info(f"{model.name}: {model.wargear}")
        ...
        try:
            ...
            try:
                ...
                logger.info(f"No cost found for {unit_name}")
                cost = 1000
        logger.info(f"Ranged Threat level: {ranged_threat_level}, Melee Threat level: {melee_threat_level}, Ranged Threat per Cost: {ranged_threat_level/cost}, Melee Threat per Cost: {melee_threat_level/cost}")
        logger.info(f"------------------------------------------------------------------\n")
