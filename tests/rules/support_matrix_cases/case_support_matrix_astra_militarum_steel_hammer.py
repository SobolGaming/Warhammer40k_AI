...
def test_imposing_arrival_stratagem_is_supported():
    import scripts.generate_ability_support_matrix as gsm
    ...
    row = next(
        item
        for item in stratagems
        if str(item.get("id", "") or "").strip() == "000010788003"
    )
    ...
    assert canonical_name == "IMPOSING ARRIVAL"
    assert status in {"Implemented", "Supported"}
    notes_l = str(notes or "").lower()
    assert "reinforcements step" in notes_l
    assert "titanic" in notes_l
