...
def test_taktikal_brigade_lissen_ere_detachment_ability_is_supported():
    gsm, detachment_abilities = _seed_support_maps()
    row = next(
        item
        for item in detachment_abilities
        if str(item.get("id", "") or "").strip() == "000009794"
    )
    ...
    assert "mortal wound" in notes_l
    assert "battle-shock" in notes_l or "battle shock" in notes_l
    assert "stormboyz" in notes_l
