...
class TestAdeptaSororitasTheEmperorsGrace(unittest.TestCase):
    def _make_game(self):
    ...
    def test_the_emperors_grace_queues_and_applies_invulnerable_bonus(self):
        ...
        ability_desc = (
            ...
            "If it does, until the end of the phase, this model has a 2+ invulnerable save."
        )
        ability = Ability("The Emperor's Grace", "AS", ability_desc, "Datasheet", "")
        canoness = _make_unit(
            "Canoness",
            player.army,
            abilities=[ability],
        ...
        canoness_model = _make_model("Canoness", canoness)
        canoness_model.abilities = {"The Emperor's Grace": ability}
        ...
        invuln_value, _source = canoness_model.get_temporary_invulnerable_save()
        self.assertEqual(int(invuln_value or 0), 2)
