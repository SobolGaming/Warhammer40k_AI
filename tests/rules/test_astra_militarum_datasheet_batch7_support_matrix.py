...
def test_furious_barrage_support_matrix_classifies_supported():
    ...
    assert "storm eagle rockets" in str(notes or "").lower()
    assert "Objective Control" in str(notes or "")


def test_command_rod_support_matrix_classifies_supported():
    gsm = _seed_support_maps()
    status, notes = gsm._classify_ability(
        "Command Rod",
