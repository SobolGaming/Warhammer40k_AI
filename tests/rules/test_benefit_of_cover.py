...
class TestBenefitOfCover:
    ...
    def test_foul_spores_aura_grants_cover_to_tyranids_monsters(self, monkeypatch):
        import warhammer40k_ai.units.wargear as wargear_mod
        import warhammer40k_ai.utility.aura_effects as aura_mod
        ...
        game_map = Map(width=48, height=72)
        aura_source = create_unit("Venomthropes", 10.0, 10.0, faction="A", model_count=1, abilities=aura_ability)
        monster_target = create_unit("Carnifexes", 12.0, 10.0, faction="A", model_count=1)
        attacker = create_unit("Attacker", 30.0, 10.0, faction="B", model_count=1)

        army_a, army_b = attach_to_armies(game_map, [aura_source, monster_target], [attacker])
        game = SimpleNamespace(map=game_map)
        army_a.player = SimpleNamespace(game=game, name="P1", id="P1")
        army_b.player = SimpleNamespace(game=game, name="P2", id="P2")

        monster_target.has_any_keyword = lambda kw: str(kw).strip().lower() in {"tyranids", "monster"}
        ...
        attack_instance = {"mortal_wound": False}
        ranged_wp._save_with_tracking(monster_target.models[0], attack_instance, ap=0)
        assert attack_instance.get("benefit_of_cover") is True
        assert "Foul Spores" in str(attack_instance.get("benefit_of_cover_source", ""))
