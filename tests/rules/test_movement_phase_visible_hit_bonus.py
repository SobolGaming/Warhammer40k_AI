...
class TestMovementPhaseVisibleHitBonus(unittest.TestCase):
    ...
    def test_movement_phase_visible_hit_bonus(self):
        ...
        ability_desc = (
            ...
            "targets that enemy unit, add 1 to the Hit roll. Each unit can only be selected for this ability once per turn."
        )
        ability = Ability("Guide (Psychic)", "AE", ability_desc, "Datasheet", "")

        unit = self._make_unit("Farseer", army, faction_keywords=["AELDARI"])
        model = self._make_model("Farseer", unit)
        model.abilities = {"Guide": ability}
        unit.models = [model]
        ...
        game._on_phase_end_movement_phase_visible_hit_bonus(player=player, phase=game.phase)
        ...
        self.assertEqual(ctx.get("ability"), "movement_phase_visible_hit_bonus")
        ...
        self.assertTrue(sr.get("movement_phase_visible_hit_bonus_active"))
        self.assertEqual(int(sr.get("movement_phase_visible_hit_bonus_value", 0) or 0), 1)
        ...
        ranged_parent = SimpleNamespace(name="Test Weapon", is_melee=lambda: False)
        ...
        try:
            ...
            hit_res = profile._hit_target_with_tracking(target_unit, model, attack_instance)
            self.assertTrue(any("Guide" in mod for mod in hit_res.get("modifiers", [])))
        finally:
