...
class _RollManagerStub:
    def __init__(self, roll_state) -> None:
        self._roll_state = roll_state

    def get_roll(self, _roll_id: int):
        return self._roll_state
...
def test_move_units_followup_waits_for_advance_reroll_to_finalize() -> None:
    game, unit = _build_flow_game()
    unit.round_state.advance_roll = 4
    game.roll_manager = _RollManagerStub(SimpleNamespace(final=False))
    option = DecisionOption.create("No re-roll", payload={"action_id": "none"})
...
def test_move_units_followup_queues_advance_move_after_reroll_declined() -> None:
    game, unit = _build_flow_game()
    unit.round_state.advance_roll = 4
    game.roll_manager = _RollManagerStub(SimpleNamespace(final=True))
    option = DecisionOption.create("No re-roll", payload={"action_id": "none"})
    request = DecisionRequest.create(
        DECISION_SELECT_DICE_REROLL,
        "Re-roll options: Advance roll",
    ...
    assert float(queued.context["max_distance"]) == 10.0
