...
def _build_game():
    ...
    sm_army = Army.with_detachment("Space Marines", "Gladius Task Force")
...
def test_gladius_task_force_stratagem_descriptors_registered():
    expected = {
        "000008352005": ("Adaptive Strategy", "unit_specific_combat_doctrine_override"),
        "000008352004": ("Honour the Chapter", "melee_weapons_gain_lance_and_conditional_assault_ap_bonus"),
        "000008352003": ("Only in Death Does Duty End", "fight_on_death_after_attacks"),
        "000008352007": ("Squad Tactics", "reactive_normal_move_with_tactical_fixed_six"),
        "000008352006": ("Storm of Fire", "ranged_weapons_gain_ignores_cover_and_conditional_devastator_ap_bonus"),
    }
...
def test_honour_the_chapter_grants_melee_lance_and_assault_ap_bonus():
    ...
    pending = _pending_by_name(sm_player.stratagems, "HONOUR THE CHAPTER")
    assert pending is not None

    ok = sm_player.stratagems.use("HONOUR THE CHAPTER", unit=intercessors, dequeue=True)
    assert ok
    ...
    assert any("HONOUR THE CHAPTER" in str(reason).upper() for reason in list(ap_reasons or []))
    ...
    assert any("HONOUR THE CHAPTER" in str(modifier).upper() for modifier in list(during.get("modifiers", []) or []))
...
def test_storm_of_fire_grants_ignores_cover_and_devastator_ap_bonus():
    game, sm_player, _enemy_player, sm_army, enemy_army = _build_game()
    ...
    pending = _pending_by_name(sm_player.stratagems, "STORM OF FIRE")
    assert pending is not None

    ok = sm_player.stratagems.use("STORM OF FIRE", unit=hellblasters, dequeue=True)
    assert ok
    assert int(sm_player.command_points or 0) == 9
    ...
    assert int(ap_bonus or 0) == 1
    assert any("STORM OF FIRE" in str(reason).upper() for reason in list(ap_reasons or []))
...
def test_squad_tactics_queues_reaction_and_rolls_d6_move_distance():
    ...
    pending = _pending_by_name(sm_player.stratagems, "SQUAD TACTICS")
    assert pending is not None
    ...
    assert str(context.get("reactive_move_kind", "") or "") == "squad_tactics"
    assert int(context.get("reactive_move_range", 0) or 0) == 9


def test_squad_tactics_uses_fixed_six_under_tactical_doctrine():
    game, sm_player, enemy_player, sm_army, enemy_army = _build_game()
    intercessors = _make_unit(
        "Intercessor Squad",
        keywords=["INFANTRY"],
...
def test_only_in_death_does_duty_end_reaction_defers_fight_on_death_and_cleans_up():
    game, sm_player, enemy_player, sm_army, enemy_army = _build_game()
    target = _make_unit(
    ...
    pending = _pending_by_name(sm_player.stratagems, "ONLY IN DEATH DOES DUTY END")
    assert pending is not None
    ...
    assert bool(target.special_rules.get("deathless_duty_active"))
    assert "ONLY IN DEATH DOES DUTY END" in str(target.special_rules.get("deathless_duty_source", "")).upper()

    target.round_state.fought_this_phase = False
    model = target.models[0]
    model._wounds = 0
    target._handle_model_destroyed(model, game.map)
    pending_models = list(getattr(target, "_deathless_duty_pending_models", []) or [])
    ...
    assert not list(getattr(target, "_deathless_duty_pending_models", []) or [])

    game.event_system.publish("phase_end", player=enemy_player, phase=SimpleNamespace(name="FIGHT_PHASE"))
    assert not bool(target.special_rules.get("deathless_duty_active"))


def test_adaptive_strategy_overrides_unit_doctrine_until_next_command_phase():
    ...
    pending = _pending_by_name(sm_player.stratagems, "ADAPTIVE STRATEGY")
    assert pending is not None
