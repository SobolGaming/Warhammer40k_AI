...
def test_more_dakka_dakka_dakka_dakka_detachment_ability_is_supported():
    gsm, detachment_abilities = _seed_support_maps()
    row = next(
        item
        for item in detachment_abilities
        if str(item.get("id", "") or "").strip() == "000009990"
    )
    ...
    assert "sustained hits" in notes_l
    assert "waaagh" in notes_l
