...
class TestSpaceMarinesBladeOfUltramarEnhancements(unittest.TestCase):
    def test_armour_of_antoninus_applies_bearer_only_save_and_fnp(self):
        ...
        self.assertIn("Antoninus", str(save_source or ""))
    ...
    def test_oath_of_macragge_scales_bearer_melee_attacks_and_strength_with_assault_doctrine(self):
        ...
        self.assertTrue(any("+1S from Oath of Macragge" in str(reason) for reason in list(dev_wound.get("modifiers", []) or [])))
        ...
        self.assertTrue(any("+2S from Oath of Macragge" in str(reason) for reason in list(assault_wound.get("modifiers", []) or [])))

    def test_student_of_the_codex_optional_activation_sets_unit_doctrine_override_until_next_command_phase(self):
    ...
    def test_veteran_of_behemoth_grants_ranged_sustained_hits_and_devastator_advance_reroll(self):
        game, sm_army, enemy_army, _sm_player, _enemy_player = _build_game()
        bodyguard = _make_unit(
            "Intercessors",
            keywords=["ADEPTUS ASTARTES", "INFANTRY"],
        ...
        self.assertTrue(any("Veteran of Behemoth" in str(effect) for effect in list(dev_hit_result.get("special_effects", []) or [])))
